...
public class SupportBundleJob {
  ...
  public SupportBundleJob(Set<SupportBundleTaskFactory> supportBundleTaskFactories,
    ...
    this.maxThreadTimeout = cConf.getInt(Constants.SupportBundle.MAX_THREAD_TIMEOUT);
   ...
   */
  public void executeTask(SupportBundleTask supportBundleTask, String basePath, String taskName,
      String taskType) {
    ...
    while (!runningTaskStateQueue.isEmpty()) {
      ...
      try {
        ...
        long timeLeftBeforeTimeout =
            futureStartTime == 0L ? maxThreadTimeoutToMill
                : maxThreadTimeoutToMill - (currentTime - futureStartTime);
   ...
   */
  private void executeTask(SupportBundleTaskStatus taskStatus, SupportBundleTask supportBundleTask,
      String basePath,
    ...
    Future<SupportBundleTaskStatus> futureService = executor.submit(() -> {
      try {
        ...
        latestTaskStatus.set(
            updateTask(latestTaskStatus.get(), basePath, CollectionState.IN_PROGRESS));
        supportBundleTask.collect();
        latestTaskStatus.set(
            updateTask(latestTaskStatus.get(), basePath, CollectionState.FINISHED));
      } catch (Exception e) {
        LOG.warn("Failed to execute task with supportBundleTask {} ", taskName, e);
        executeTaskAgainAfterFailed(supportBundleTask, taskName, taskType, latestTaskStatus.get(),
            basePath,
            retryCount + 1);
      }
      return latestTaskStatus.get();
    });
    RunningTaskState runningTaskState = new RunningTaskState(futureService, startTimeStore,
        taskStatus);
   ...
   */
  private void executeTaskAgainAfterFailed(SupportBundleTask supportBundleTask, String taskName,
      String taskType,
    ...
    if (retryCount >= maxRetries) {
      ...
      SupportBundleTaskStatus updatedTaskStatus =
          SupportBundleTaskStatus.builder(taskStatus).setRetries(retryCount)
              .setStatus(CollectionState.QUEUED).build();
      supportBundleStatus.getTasks().remove(taskStatus);
  ...
  private void updateFailedTask(SupportBundleTaskStatus supportBundleTaskStatus,
      Future<SupportBundleTaskStatus> future,
      String basePath) {
