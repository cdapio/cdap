...
public class StreamHandlerTestRun extends GatewayTestBase {
  ...
  public void testStreamInfo() throws Exception {
    ...
    Id.Stream streamId = Id.Stream.from(Constants.DEFAULT_NAMESPACE, "stream_info");
    StreamProperties streamProperties = new StreamProperties(streamId, 2L, formatSpecification, 20);
  ...
  public void testPutStreamConfigDefaults() throws Exception {
    ...
    Id.Stream streamId = Id.Stream.from(Constants.DEFAULT_NAMESPACE, "stream_defaults");
    StreamProperties streamProperties = new StreamProperties(streamId, 2L, formatSpecification, 20);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.OK.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
    ...
    StreamProperties expected = new StreamProperties(streamId, 2L, StreamConfig.DEFAULT_STREAM_FORMAT, 20);
  ...
  public void testPutInvalidStreamConfig() throws Exception {
    ...
    Id.Stream streamId = Id.Stream.from(Constants.DEFAULT_NAMESPACE, "stream_badconf");
    StreamProperties streamProperties = new StreamProperties(streamId, -1L, null, 20);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.BAD_REQUEST.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
    ...
    streamProperties = new StreamProperties(streamId, 2L, formatSpec, 20);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.BAD_REQUEST.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
    ...
    urlConn = openURL(String.format("http://%s:%d/v2/streams/stream_badconf/config",
                                    HOSTNAME, port), HttpMethod.PUT);
    urlConn.setDoOutput(true);
    streamProperties = new StreamProperties(streamId, 2L, null, -20);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.BAD_REQUEST.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
  }
