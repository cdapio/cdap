...
class KubeJobCleaner implements Runnable {
  ...
  @Override
  public void run() {
    LOG.info("### Starting k8s job cleaner");
        ...
        for (V1Job job : jobs.getItems()) {
          ...
          if (jobStatus != null && (jobStatus.getSucceeded() != null || jobStatus.getFailed() != null)) {
            ...
            try {
              ...
              LOG.info("### Cleaning job {} in kubernetes", jobName);
              jobDeletionCount++;
              ...
              if (e.getCode() == 404) {
                ...
                LOG.debug("### Ignoring job deletion for job {} because job was not found.", jobName, e);
              } else {
