...
public class WorkloadIdentityUtil {
  ...
  private static final String WORKLOAD_IDENTITY_CREDENTIAL_KSA_SOURCE_PATH
      = WorkloadIdentityUtil.WORKLOAD_IDENTITY_CREDENTIAL_DIR + "/"
  ...
  /**
   * Helper method for testing whether to mount a workload identity config map. Workload identity
   * should be mounted in the following two scenarios: - A program is running in the CDAP
   * installation namespace - A program has a workload identity email set This helper method helps
   * keep the mounting logic consistent between KubeTwillPreparer and KubeMasterEnvironment.
   *
   ...
   */
  public static boolean shouldMountWorkloadIdentity(String installK8sNamespace,
      String programRuntimeNamespace,
      String workloadIdentityServiceAccountEmail) {
    return installK8sNamespace.equals(programRuntimeNamespace)
        || (workloadIdentityServiceAccountEmail != null
  ...
  public static void findOrCreateWorkloadIdentityConfigMap(CoreV1Api coreV1Api, String k8sNamespace,
    ...
    try {
      ...
      if (e.getCode() == HttpURLConnection.HTTP_NOT_FOUND) {
        LOG.debug("Creating workload identity config map for kubernetes namespace {}",
            k8sNamespace);
      } else {
    ...
    String workloadIdentityAudience = String.format(WORKLOAD_IDENTITY_AUDIENCE_FORMAT,
        workloadIdentityPool,
        workloadIdentityProvider);
    String workloadIdentityImpersonationURL = String.format(
        WORKLOAD_IDENTITY_IMPERSONATION_URL_FORMAT,
        workloadIdentityGCPServiceAccountEmail);
    GCPWorkloadIdentityCredential credential =
        new GCPWorkloadIdentityCredential(
            GCPWorkloadIdentityCredential.CredentialType.EXTERNAL_ACCOUNT,
    ...
    V1ConfigMap configMap = new V1ConfigMap()
        .metadata(new V1ObjectMeta().namespace(k8sNamespace)
            .name(WorkloadIdentityUtil.WORKLOAD_IDENTITY_CONFIGMAP_NAME))
   ...
   */
  public static V1Volume generateWorkloadIdentityVolume(
    ...
    return new V1Volume().name(WORKLOAD_IDENTITY_PROJECTED_VOLUME_NAME)
        .projected(new V1ProjectedVolumeSource().defaultMode(420)
            .addSourcesItem(
                new V1VolumeProjection().serviceAccountToken(serviceAccountTokenProjection))
  ...
  public static V1EnvVar generateWorkloadIdentityEnvVar() {
    return new V1EnvVar().name(WORKLOAD_IDENTITY_ENV_VAR_KEY)
   ...
   */
  public static long convertWorkloadIdentityTTLFromString(@Nullable String ttlStr)
