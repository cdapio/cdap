...
public class MemStatsFlowlet extends MetricsCollector {

  private org.slf4j.Logger LOG = LoggerFactory.getLogger(CPUStatsFlowlet.class);

  public static final String NAME = "MEM_STAT_FLOWLET";
  public static final String DESC = "Memory usage stats flowlet";
  ...
  public TimeseriesTable.Entry generateEntry(String metric) {
    ...
    if (!metric.isEmpty()) {
      ...
      if (idx == 3) {
        LOG.info("generating entry, timestamp: " + timestamp.getTime() + ",memory: " + memory + "hostname: " + hostname);
        entry = new TimeseriesTable.Entry(Bytes.toBytes("memory"), Bytes.toBytes(memory), timestamp.getTime(), Bytes.toBytes(hostname));
      }
      else {
