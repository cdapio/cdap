...
final class MetricEntityCodec {
  ...
  private final EnumMap<MetricEntityType, LoadingCache<String, byte[]>> entityCaches;


  MetricEntityCodec(EntityTable entityTable, int contextDepth, int metricDepth, int tagDepth) {
    this.entityTable = entityTable;
    this.contextDepth = contextDepth;
    this.metricDepth = metricDepth;
    this.tagDepth = tagDepth;

    this.entityCaches = Maps.newEnumMap(MetricEntityType.class);
    for (MetricEntityType type : MetricEntityType.values()) {
   ...
   */
  public byte[] encode(MetricEntityType type, String entity) {
   ...
   * would be padded by the given padding.
   * @param type Type of the entity.
   * @param entity Value of the entity.
   ...
   */
  public byte[] paddedEncode(MetricEntityType type, String entity, int padding) {
    int idSize = entityTable.getIdSize();
    int depth = getDepth(type);
    String[] entityParts = ENTITY_SPLITTER.split(entity, depth);
   ...
   */
  public ImmutablePair<byte[], byte[]> paddedFuzzyEncode(MetricEntityType type, String entity, int padding) {
    int idSize = entityTable.getIdSize();
    int depth = getDepth(type);
    String[] entityParts = ENTITY_SPLITTER.split(entity, depth);
    byte[] result = new byte[depth * idSize];
  ...
  public String decode(MetricEntityType type, byte[] encoded) {
    return decode(type, encoded, 0, encoded.length);
  }

  public String decode(MetricEntityType type, byte[] encoded, int offset, int len) {
    StringBuilder builder = new StringBuilder();
    int idSize = entityTable.getIdSize();
    int length = len / idSize;
   ...
   * @return
   */
  private CacheLoader<String, byte[]> createCacheLoader(final MetricEntityType type) {
  ...
  private int getDepth(MetricEntityType type) {
    switch (type) {
      case CONTEXT:
        return contextDepth;
      case METRIC:
