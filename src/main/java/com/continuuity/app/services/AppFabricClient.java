...
public class AppFabricClient {
  private static final Logger Log =
    LoggerFactory.getLogger(AppFabricClient.class);
   ...
   */
  private DeploymentService.Client deploymentServiceClient = null;

  /**
   * Instance of client for Runtime Service.
   */
  private RuntimeService.Client RuntimeServiceClient = null;

  /**
   * Instance of client for Metrics frontend service.
   */
  private MetricsFrontendService.Client metricsServiceClient = null;
  ...
  public AppFabricClient(CConfiguration configuration, boolean remoteDeploymentService)
    ...
    // Starting a service discovery client.
    try {
      serviceDiscoveryClient = new ServiceDiscoveryClient(
        configuration.get(
        ...
        public void run() {
          try {
            ...
            if(RuntimeServiceClient != null) {
              RuntimeServiceClient.getInputProtocol().getTransport().close();
              RuntimeServiceClient.getOutputProtocol().getTransport().close();
            }
            if(deploymentServiceClient != null) {
              deploymentServiceClient.getInputProtocol().getTransport().close();
              deploymentServiceClient.getOutputProtocol().getTransport().close();
            }
            if(metricsServiceClient != null) {
              metricsServiceClient.getInputProtocol().getTransport().close();
              metricsServiceClient.getOutputProtocol().getTransport().close();
            }
          } catch (IOException e) {
            Log.error("Failed to shutdown service discovery client. " +
                        "Reason : {}.", e.getMessage());
          }
        }
      })
  ...
  public boolean promote(String accountId, String applicationId, String entityId,
    ...
    // Connects to remote FAR service.
    ...
    try {
      deploymentServiceClient.promote(getAuthToken(), null); // TODO
      return true;
  ...
  public ImmutablePair<Boolean, String> deploy(File file, String accountId,
    ...
    try {
      ...
      ResourceIdentifier identifier = deploymentServiceClient.init(getAuthToken(), info);
      if(identifier == null) {
        return new ImmutablePair<Boolean, String>(false,
      ...
      while(true) {
        ...
        deploymentServiceClient.chunk(getAuthToken(),
          identifier, ByteBuffer.wrap(toSubmit));
      }
      is.close();

      // Finalize the resource deployment
      deploymentServiceClient.deploy(getAuthToken(), identifier);


      // If a flow within the resource is running, then we don't
      // successfully deploy the resource
      DeploymentStatus status = deploymentServiceClient.status(getAuthToken(), identifier);
      if(status.getOverall()
        == DeployableStatus.ALREADY_RUNNING) {
      ...
      while(status.getOverall()
        ...
        Thread.sleep(1000);
        count++;
        status = deploymentServiceClient.status(getAuthToken(), identifier);
      }
      if(status.getOverall()
        == DeployableStatus.VERIFICATION_FAILURE) {
  ...
  private void connectToFARService(boolean remote) throws IOException {
    ...
    if(deploymentServiceClient == null ||
       ! deploymentServiceClient.getInputProtocol().getTransport().isOpen()) {
      ...
      TProtocol protocol = new TBinaryProtocol(transport);
      deploymentServiceClient = new DeploymentService.Client(protocol);
    }
  }
  ...
  public void remove(String accountId, String applicationId, String entityId,
    ...
    try {
      ...
      identifier.setType(type);
      identifier.setAccountId(accountId);
      deploymentServiceClient.remove(getAuthToken(), identifier);
    } catch (DeploymentServiceException e) {
      throw new IOException(e);
    } catch (TException e) {
      throw new IOException(e);
  ...
  public boolean doMain(String[] args) throws Exception {
    ...
    if("remove-all".equals(command)) {
      connectToFARService(remoteDeploymentService);
      try {
        deploymentServiceClient.removeAll(getAuthToken(), acctid);
      } catch (DeploymentServiceException e) {
        throw new IOException(e);
      } catch (TException e) {
        throw new IOException(e);
    ...
    if("reset".equals(command)) {
      ...
      connectToFARService(remoteDeploymentService);
      try {
        deploymentServiceClient.reset(getAuthToken(), acctid);
      } catch (DeploymentServiceException e) {
        throw new IOException(e);
      } catch (TException e) {
        throw new IOException(e);
