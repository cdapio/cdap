...
public class TaskHandler extends AbstractScheduledService {
  ...
  public void startUp() {
    ...
    if(containerMgr == null) {
      LOG.warn("Failed connecting to container manager for container {}",
               container.toString());
      stop();
      return;
    }
    ...
    LOG.trace("Starting task {} in container {}.", specification.getId(),
              container);
    LOG.trace("Task {} run start request : {}", specification.getId(), startRequest.toString());

    try {
      containerMgr.startContainer(startRequest);
      LOG.trace("Started task {}", specification.getId());
    } catch (YarnRemoteException e) {
      LOG.error("Failed starting container {}. Reason : {}",
               container.toString(), e.getMessage());
      LOG.error(StackTraceUtil.toStringStackTrace(e));
      stop();
    } catch (Throwable e) {
      LOG.error(StackTraceUtil.toStringStackTrace(e));
      LOG.error("Horrible thing happened. Reason : {}", e.getMessage());
      stop();
    }
  }
  ...
  public void shutDown() {
    LOG.trace("Stopping task {} running in container {}", specification.getId(), container);
  ...
  protected void runOneIteration() throws Exception {
    LOG.trace("Checking status of task {}, Container {}", specification.getId(), container.getId().toString());
    ...
    try {
      ...
      LOG.trace("Container {} status {}, Diganostics {}.",
               new Object[]{container.getId().toString(), status.toString(), status.getDiagnostics()});

      if (status != null && status.getState() == ContainerState.COMPLETE) {
        LOG.trace("Container {} has completed. Stopping the container.", container.getId());
        stop();
      }
    } catch (YarnRemoteException e) {
      LOG.trace("There was problem receiving the status of container {}. " +
                 "Container might be shutting down.", container.toString());
      checkFailures++;

    } catch (Throwable e) {
      LOG.trace("An error was thrown while checking the status of a container. Reason : {}", e.getMessage());
    }

    if(checkFailures > MAX_CHECK_FAILURES) {
      LOG.trace("Failed after max retry to get status of container {}", container.toString());
      stop();
    }
