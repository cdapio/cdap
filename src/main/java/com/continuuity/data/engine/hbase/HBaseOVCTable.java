...
public class HBaseOVCTable implements OrderedVersionedColumnarTable {
  ...
  public HBaseOVCTable(Configuration conf,
    ...
    try {
      ...
      this.writeTables.add(new HTable(conf, tableName));
      initAsyncWriteTalbes(conf, tableName);
      this.conf = conf;
  ...
  @Override
  public void persistAllFor(long version) throws OperationException {
    persistAllAsyncWrites(version);
    // No other action needed: all other operations are persisted synchronously
  }
  ...
  // Async operations. Logic, briefly:
  // * to execute async operations we use separate pool of HTable objects
  // * current first cut of implementation flushes all HTables in this pool when *any* tx (that have written anything
  //   into this table) commits

  // todo: make configurable the size of the pool
  // all async operations are executed by this executor
  private ExecutorService executorService = Executors.newFixedThreadPool(10);

  /** pool of available HTables that can be used for writing async operations */
  private LinkedList<HTable> asyncWriteTables;
  /** list of all HTables used for writing async operations. Needed to make sure all HTables are flushed */
  private List<HTable> allAsyncWriteTables;

  /** lock to guard accessing pooled HTable instances */
  private Lock asyncWriteTablesLock = new ReentrantLock();
  /** lock to sync requests for persisting async writes */
  private Lock persistAsyncWritesLock = new ReentrantLock();
  ...
  public void asyncPut(byte[] row, byte[][] columns, long version, byte[][] values) throws OperationException {
    ...
    // executing put operation asynchronously
    executorService.execute(new PutCommand(put));
  }

  @Override
  public void asyncIncrement(byte[] row, byte[][] columns, long[] amounts, ReadPointer readPointer, long writeVersion)
    throws OperationException {
    // executing inc operation asynchronously
    executorService.execute(new IncrementCommand(row, columns, amounts, readPointer, writeVersion));
  }

  /**
   * Persists all async write/inc operations performed with given write version. Blocks until all async write/inc
   * operations are persisted.
   *
   * @param version version of operations to persist.
   * @throws OperationException
   */
  private void persistAllAsyncWrites(long version) throws OperationException {
    Log.trace("Persisting all writes, table: {}, # of HTable clients: {}, version: {}",
              Bytes.toString(tableName), allAsyncWriteTables.size(), version);
    // NOTE: we using different from asyncWriteTablesLock to avoid deadlock with returnAsyncWriteTable(...)
    persistAsyncWritesLock.lock();
    try {
      // todo: consider better approach: we don't really need to be in the end of the task queue to
      //       persist up to "version".

      // by adding command to the end of the queue we ensure that all writes from this transaction happened already
      // todo: the above comment ("we ensure ...") is not really true: while adding operation to the end makes it highly
      //       unlikely to persist command acquire one of the tables that are used by previously scheduled commands,
      //       this still may happen. Fix it.
      Future<?> submit = executorService.submit(new PersistAllForVersionCommand(version));
      try {
        // waiting for flush to be done
        submit.get();
        Log.trace("Persisting all writes DONE, table: {}, # of HTable clients: {}, version: {}",
                  Bytes.toString(tableName), allAsyncWriteTables.size(), version);
      } catch (Exception e) {
        throw new OperationException(StatusCode.INTERNAL_ERROR,
                                     "Failed flushing data for tx with version: " + version +
                                       ", table: " + Bytes.toStringBinary(tableName), e);
      }
    } finally {
      persistAsyncWritesLock.unlock();
    }
  }

  private void initAsyncWriteTalbes(Configuration conf, byte[] tableName) throws IOException {
    this.allAsyncWriteTables = new ArrayList<HTable>();
    this.asyncWriteTables = new LinkedList<HTable>();
    this.asyncWriteTables.add(createAsyncWriteTable(conf, tableName));
    this.asyncWriteTables.add(createAsyncWriteTable(conf, tableName));
  }

  private HTable createAsyncWriteTable(Configuration conf, byte[] tableName) throws IOException {
    HTable hTable = new HTable(conf, tableName);
    // Using client-side buffer increases performance a lot in cases with many write/inc operations per tx.
    // The client-side buffer is assured to be flushed (i.e. persisted) by persistAllAsyncWrites(...) operations.
    hTable.setAutoFlush(false);
    // todo: make size of client-side buffer configurable
    hTable.setWriteBufferSize(2 * 1024 * 1024);
    allAsyncWriteTables.add(hTable);
    return hTable;
  }

  private HTable getAsyncWriteTable() throws IOException {
    // Similar to writeTables pool: since HTable is not thread-safe get/return table of the pool is locked to ensure
    // access to single HTable from one thread only at any given time.
    asyncWriteTablesLock.lock();
    try {
      HTable table = this.asyncWriteTables.pollFirst();
      return table == null ?
        createAsyncWriteTable(this.conf, this.tableName) : table;
    } finally {
      asyncWriteTablesLock.unlock();
    }
  }

  private void returnAsyncWriteTable(HTable table) {
    asyncWriteTablesLock.lock();
    try {
      this.asyncWriteTables.add(table);
    } finally {
      asyncWriteTablesLock.unlock();
    }
  }
  ...
  /**
   * Performs put operation using one of the asyncWriteTables instances
   */
  private final class PutCommand implements Runnable {
    Put put;

    private PutCommand(Put put) {
      this.put = put;
    }
    ...
    public void run() {
      try {
        // acquiring table as early as possible to be "before" persist command.
        // todo: do better job here wrt to syncing with persist op (see other todo in persist command on that)
        HTable table = getAsyncWriteTable();
        try {
          table.put(put);
        } finally {
          returnAsyncWriteTable(table);
        }
      } catch (IOException e) {
        try {
          exceptionHandler.handle(e);
        } catch (OperationException e1) {
          throw new RuntimeException("Failed writing put", e1);
        }
      }
    }
  }
  ...
  /**
   * Performs increment operation using one of the asyncWriteTables instances
   */
  private class IncrementCommand implements Runnable {
    private byte[] row;
    ...
    private IncrementCommand(byte[] row, byte[][] columns, long[] amounts, ReadPointer readPointer, long writeVersion) {
      this.row = row;
    ...
    private void doInc() throws OperationException {
      HTable table = null;
      try {
        // acquiring table as early as possible to be "before" persist command.
        // todo: do better job here wrt to syncing with persist op (see other todo in persist command on that)
        table = getAsyncWriteTable();
        ...
        for (int i = 0; i < columns.length; i++) {
          ...
          put.add(family, columns[i], writeVersion, prependWithTypePrefix(DATA, Bytes.toBytes(l)));
          puts.add(put);
        }
  ...
  /**
   * Performs persisting of write/inc operations which were done using one of the asyncWriteTables instances
   * for given version.
   */
  private class PersistAllForVersionCommand implements Runnable {
    private long version;

    private PersistAllForVersionCommand(long version) {
      this.version = version;
    }

    @Override
    public void run() {
      // todo: persist only based on version, not all
      // todo: consider flushing HTables asynchronously.
      try {
        Log.trace("Flushing all \"async-writing\" HTable clients, table: {}, # of HTable clients: {}, version: {}",
                  Bytes.toString(tableName), allAsyncWriteTables.size(), version);

        // NOTE: since getAsyncWriteTable() gets from the start of the list and returnAsyncWriteTable(t) returns to the
        //       end this logic will work. Otherwise we could loop forever.
        Set<HTable> toFlush = new HashSet<HTable>(allAsyncWriteTables);
        while (toFlush.size() > 0) {
          HTable t = getAsyncWriteTable();
          try {
            if (toFlush.contains(t)) {
              t.flushCommits();
              toFlush.remove(t);
            }
          } finally {
            returnAsyncWriteTable(t);
          }
        }
        Log.trace("Flushing all \"async-writing\" HTable clients DONE, table: {}, # of HTable clients: {}",
                  Bytes.toString(tableName), allAsyncWriteTables.size());
      } catch (IOException e) {
        // it will be caught later on future.get() and transformed to OperationException
        throw new RuntimeException("Failed flushing data for tx with version: " + version +
                                     ", table: " + Bytes.toStringBinary(tableName), e);
  ...
  private KeyValue[] getLatestVisible(final byte [] row, final byte [][] qualifiers, ReadPointer readPointer)
    ...
    try {
      ...
      for (int i = 0; i < qualifiers.length; i++) {
        get.addColumn(this.family, qualifiers[i]);
      }
      // read rows that were written up until the start of the current transaction (=getMaxStamp(readPointer))
      get.setTimeRange(0, getMaxStamp(readPointer));
      get.setMaxVersions();
