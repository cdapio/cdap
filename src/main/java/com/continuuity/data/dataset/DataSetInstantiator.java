...
public class DataSetInstantiator {
  ...
  public void setDataSets(List<DataSetMeta> metas) {
    for (DataSetMeta meta : metas) {
      this.datasets.put(meta.getName(), meta);
    }
  }
  ...
  <T extends DataSet> T instantiate(String dataSetName)
    ...
    DataSetMeta meta = this.datasets.get(dataSetName);
    if (meta == null) {
    ...
    try {
      ...
      throw logAndException(e, "Data set class %s does not declare constructor from DataSetMeta", className);

    } catch (InstantiationException e) {
      throw logAndException(e, "Data set class %s is not instantiable", className);

    } catch (IllegalAccessException e) {
      throw logAndException(e, "Constructor from DataSetMeta is not accessible in data set class %s", className);
    }
