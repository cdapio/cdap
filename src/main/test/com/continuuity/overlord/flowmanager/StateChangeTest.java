...
import com.continuuity.overlord.flowmanager.internal.StateChangeFactory;
...
public class StateChangeTest {
  ...
  public void testSimpleEventsOrdered() throws Exception {
    ...
    client.start();
    StateChanger changer = StateChangeFactory.newChange(client, "/continuuity/system/queue");
    StateChangeListener listener = StateChangeFactory.newListener(client);

    for(int i = 0; i < 100; ++i) {
      changer.change(StateChangeFactory.newState(System.currentTimeMillis(), "A:" + i, "B", "C", "[]",
            StateChangeType.DEPLOYED_FLOW));
    }


    listener.monitor("/continuuity/system/queue", new StateChangeCallback() {
      private int i = 0;
      @Override
