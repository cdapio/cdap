...
public class SortedCounterTable extends DataLib {
  ...
  public SortedCounterTable(String dataSetId,SortedCounterConfig config) {
    super(dataSetId, "SortedCounterTable");
    this.config = config;
  }
  ...
  public Increment generatePrimaryCounterIncrement(byte [] counterSet,
    ...
    return new Increment(getDataSetId(), row, counter, amount);
  }
  ...
  public void performSecondaryCounterIncrements(
    ...
    for (long bucket : this.config.buckets) {
      if (value >= bucket) {
        ...
        getCollector().add(new Increment(getDataSetId(), row, counter, localAmount));
      } else break;
    }
  }
  ...
  public List<Counter> readTopCounters(byte [] counterSet, int limit)
    ...
    for (Long bucket : bucketList) {
      ...
      ReadColumnRange bucketRead =
        new ReadColumnRange(getDataSetId(), row, null, null);
      OperationResult<Map<byte[],byte[]>> result = getDataFabric().read(bucketRead);
