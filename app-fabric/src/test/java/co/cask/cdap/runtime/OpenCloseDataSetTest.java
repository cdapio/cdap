...
public class OpenCloseDataSetTest {
  ...
  public void testDataSetsAreClosed() throws Exception {
    ...
    while (TrackingTable.getTracker("cdap.user.foo", "write") < 4) {
      TimeUnit.MILLISECONDS.sleep(50);
    }

    // get the number of writes to the foo table
    Assert.assertEquals(4, TrackingTable.getTracker("cdap.user.foo", "write"));
    // only the flow has started with s single flowlet (procedure is loaded lazily on 1sy request
    Assert.assertEquals(1, TrackingTable.getTracker("cdap.user.foo", "open"));
    ...
    Assert.assertEquals(1, TrackingTable.getTracker("cdap.user.foo", "read"));
    Assert.assertEquals(2, TrackingTable.getTracker("cdap.user.foo", "open"));
    Assert.assertEquals(0, TrackingTable.getTracker("cdap.user.foo", "close"));

    // stop flow and procedure, they shuld both close the data set foo
    for (ProgramController controller : controllers) {
      controller.stop().get();
    }
    Assert.assertEquals(2, TrackingTable.getTracker("cdap.user.foo", "close"));
    ...
    // of times.
    Assert.assertTrue(2 < TrackingTable.getTracker("cdap.user.foo", "open"));
    Assert.assertEquals(TrackingTable.getTracker("cdap.user.foo", "open"),
                        TrackingTable.getTracker("cdap.user.foo", "close"));
