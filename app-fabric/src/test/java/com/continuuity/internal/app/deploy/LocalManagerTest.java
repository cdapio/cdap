...
public class LocalManagerTest {
  ...
  public void testImproperOrNoManifestFile() throws Exception {
    ...
    try {
      AppFabricServiceWrapper.getLocalManager().deploy(DefaultId.ACCOUNT, null, deployedJar).get();
    } finally {
  ...
  public void testGoodPipeline() throws Exception {
    Location deployedJar = lf.create(
      JarFinder.getJar(ToyApp.class, AppFabricServiceWrapper.getManifestWithMainClass(ToyApp.class))
    );

    ListenableFuture<?> p = AppFabricServiceWrapper.getLocalManager().deploy(DefaultId.ACCOUNT, null, deployedJar);
