 ...
 */
final class SparkProgramRunner extends AbstractProgramRunner
  ...
  protected ProgramController startProgram(Program program, ProgramOptions options, RunId runId,
                                           ...
                                           Deque<Closeable> closeables) throws IOException, InterruptedException {
    ...
    SparkRuntimeContext runtimeContext = new SparkRuntimeContext(new Configuration(hConf), program, options,
                                                                 host, txClient, datasetFramework,
    ...
    try {
      ...
      LOG.error("Failed to instantiate Spark class for {}",
                program.getApplicationSpecification().getSpark().get(program.getName()).getClassName(), e);
