...
public abstract class AggregatesTableTestBase {
    ...
    try {
      ...
      for (int i = 1; i <= 10; i++) {
        MetricsRecord metric = new MetricsRecord("simple." + i, "runId", "metric",
                                                 ImmutableList.<TagMetric>of(), 0L, i, MetricContentType.COUNT);
  ...
  public void testGaugeValues() throws OperationException {
    ...
    aggregatesTable.update(ImmutableList.of(new MetricsRecord("context", "runId", "bigmetric",
                                                              ...
                                                              time, value + 10, MetricContentType.GAUGE)));
    ...
    try {
      aggregatesTable.update(ImmutableList.of(
        new MetricsRecord("app1.f.flow1.flowlet1", "0", "metric", ImmutableList.of(
          new TagMetric("tag1", 1), new TagMetric("tag2", 2), new TagMetric("tag3", 3)), 0L, 6, MetricContentType.COUNT)
      ));
  ...
  public void testTags() throws OperationException {
    ...
    try {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        aggregatesTable.update(ImmutableList.of(
          new MetricsRecord("context." + i, "runId", "metric", tags, 0L, i, MetricContentType.COUNT)
        ));
      }
  ...
  public void testDeleteContext() throws OperationException {
    ...
    try {
      ...
      aggregatesTable.update(ImmutableList.of(
        ...
        new MetricsRecord("context.0", "0", "metric.0", tags, 0L, 100, MetricContentType.COUNT),
        new MetricsRecord("context.0", "0", "metric.1", tags, 0L, 100, MetricContentType.COUNT),
        new MetricsRecord("context.1", "0", "metric.0", tags, 0L, 100, MetricContentType.COUNT),
        new MetricsRecord("context.1", "0", "metric.1", tags, 0L, 100, MetricContentType.COUNT),
        new MetricsRecord("context.2", "0", "metric.0", tags, 0L, 100, MetricContentType.COUNT),
        new MetricsRecord("context.2", "0", "metric.1", tags, 0L, 100, MetricContentType.COUNT)
      ));
  ...
  public void testDeletes() throws OperationException {
    ...
    try {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        aggregatesTable.update(ImmutableList.of(
          new MetricsRecord("context." + i, runId, metric, tags, 0L, 100, MetricContentType.COUNT)
        ));
      }
