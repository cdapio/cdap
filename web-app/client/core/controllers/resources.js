  ...
  var Controller = Em.Controller.extend({
    ...
    load: function () {
      ...
      this.HTTP.post('metrics', [
        '/cdap/cluster/resources.total.memory?start=' + start + '&count=1&interpolate=step',
        '/cdap/cluster/resources.total.storage?start=' + start + '&count=1&interpolate=step'
        ], function (response) {
              ...
              object.getSubPrograms(function (programs) {
                ...
                for (var type in programs) {
                  ...
                  for (var i = 0; i < programs[type].length; i ++) {
                    ...
                    context = '/cdap/' + program.get('context') + '/';
    ...
    updateStats: function () {
      ...
      var queries = [
        '/cdap/resources.used.memory?count=' + C.__timeRange + '&start=' + start + '&end=' +
          end + '&interpolate=step',
        '/cdap/resources.used.containers?count=' + C.__timeRange + '&start=' + start + '&end=' +
          end +'&interpolate=step',
        '/cdap/resources.used.vcores?count=' + C.__timeRange + '&start=' + start + '&end=' +
          end + '&interpolate=step',
        '/cdap/resources.used.storage?count=' + C.__timeRange + '&start=' + start + '&end=' +
          end + '&interpolate=step'
      ], self = this;
