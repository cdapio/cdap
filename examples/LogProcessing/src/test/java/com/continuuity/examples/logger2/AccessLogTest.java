...
public class AccessLogTest extends ReactorTestBase {
  ...
  public void test() throws Exception {
    ...
    try {
      ...
      RuntimeMetrics metrics = RuntimeStats.getFlowletMetrics("accessLogIntermediate", "requestCountFlow", "collector");
      metrics.waitForProcessed(3, 5, TimeUnit.SECONDS);

      // Start a MapReduce job.
      mrManager = appManager.startMapReduce("requestCountMR");
  ...
  private void sendData(ApplicationManager appManager, long now) throws IOException {
    ...
    StreamWriter streamWriter = appManager.getStreamWriter("logEventsIntermediate");
    ...
    // The log is out of the time range.
  ...
  private void verifyCountProcedure(ApplicationManager appManager)
    throws IOException {
    // Start a procedure.
    ProcedureManager procedureManager = appManager.startProcedure(AccessLogApp.RequestProcedure.class.getSimpleName());
