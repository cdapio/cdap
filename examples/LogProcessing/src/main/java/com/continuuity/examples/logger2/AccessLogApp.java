package com.continuuity.examples.logger2;
...
/**
 * AccessLogApp is an application that analyze Apache access log data to aggregate the number of HTTP
...
public class AccessLogApp implements Application {
  ...
  /**
   * The flow parses the log data from the stream and stores them in SimpleTimeseriesTable.
  */
  public static class LogAnalyticsFlow implements Flow {
    ...
    public FlowSpecification configure() {
      return FlowSpecification.Builder.with()
        ...
        .setDescription("read log data and store in dataset")
        .withFlowlets()
          .add("collector", new LogEventStoreFlowlet())
        .connect()
           // Log data sent to the stream is sent to the collector flowlet
          .fromStream("logEventsIntermediate").to("collector")
        .build();
    }
  }

  /**
   * Parse time field of log data and store log data in SimpleTimeseriesTable.
   */
    ...
    public void processFromStream(StreamEvent event) throws CharacterCodingException, ParseException {
     ...
     // Parse time fields of a log.
      ...
      if (matcher.matches() && matcher.groupCount() >= 4) {
        ...
        // Store logs in one row. A log is kept in the value of an table entry. MD5 hash code of the log is used as tag
        // to distinguish logs with the same timestamp.
   ...
   */
  public static class LogCount extends AbstractMapReduce {
    // Annotation indicates that the dataset logs is used in the MapReduce.
    @UseDataSet("logs")
    private SimpleTimeseriesTable logs;
    ...
    public MapReduceSpecification configure() {
      return MapReduceSpecification.Builder.with()
        .setName("requestCountMR")
        .setDescription("log count MapReduce job")
        // Specify the dataset for Mapper to read
        .useInputDataSet("logs")
        // Specify the dataset for Reducer to write
        .useOutputDataSet("counts")
        .build();
    }
    ...
    /**
     * A Mapper that transforms log data into key value pairs, where key is the timestamp on the hour scale and value
     * the occurrence of a log. The Mapper receive a log in a key value pair (<byte[], TimeseriesTable.Entry>) from
     * the input dataset and output data in another key value pair (<LongWritable, IntWritable>) to the Reducer.
     */
    public static class LogMapper extends Mapper<byte[], TimeseriesTable.Entry, LongWritable, IntWritable> {
      ...
      /**
       * Generate key value pairs with key as timestampe by hour and value as occurrence of the log events.
       * @param key the key of the input data. It doesn't contain valuable information for the Mapper.
       * @param entry the value of the input data. An entry of the SimpleTimeSeriesTable contains key, value, timestamp
       *              and tags.
       * @param context the context of a MapReduce job
       * @throws IOException
    ...
    /**
     * Aggregate the number of requests per hour and store the results in SimpleTimeseriesTable.
     */
    public static class LogReducer extends Reducer<LongWritable, IntWritable, byte[], TimeseriesTable.Entry> {
      /**
       * Aggregate the number of requests by hour and store the results in output dataset.
      ...
      public void reduce(LongWritable key, Iterable<IntWritable> values, Context context)
        ...
        // Store aggregated results in output dataset. All aggregated results are store in one row. Each result,
        // the number of HTTP requests per hour, is an entry of the row. The value of the entry is the number of
        // the requests per hour. The timestamp is one hour in millisecond.
  ...
  /**
   * LogProcedure is used to query the number of requests per hour in a time of range, by default, the last 24 hours.
   */
  public static class RequestProcedure extends AbstractProcedure {
    // Annotation indicates that counts dataset is used in the procedure.
    @UseDataSet("counts")
    private SimpleTimeseriesTable counts;
    ...
    public void getAllCounts(ProcedureRequest request, ProcedureResponder responder)
      ...
      // Get the start time of the time range from the query parameters. By default, end time is 24 hours ago from now.
      ...
      for (TimeseriesTable.Entry entry : counts.read(ROW_KEY, startTime, endTime)) {
