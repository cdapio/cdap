...
function usage() {
  echo "Application lifecycle management tool for the CountTokens application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the CountTokens application."
...
if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/CountTokens-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "CountTokens" "flow" $action $gateway
fi
