...
function usage() {
  ...
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the SentimentAnalysis application."
...
if [ "x$action" == "xdeploy" ]; then
  ...
  program_action $app "analysis" "flow" $action $gateway
  program_action $app "sentiment-query" "procedure" $action $gateway
fi
