...
function usage() {
  echo "Application lifecycle management tool for the SimpleWriteAndRead application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the SimpleWriteAndRead application."
...
app="SimpleWriteAndRead"

if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/SimpleWriteAndRead-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "SimpleWriteAndRead" "flow" $action $gateway
fi
