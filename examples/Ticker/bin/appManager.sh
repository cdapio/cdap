...
function usage() {
  echo "Application lifecycle management tool for the Ticker application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the Ticker application."
...
if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/Ticker-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "TickTimeseriesFlow" "flow" $action $gateway
  program_action $app "OrderDataFlow" "flow" $action $gateway
  program_action $app "Timeseries" "procedure" $action $gateway
  program_action $app "Orders" "procedure" $action $gateway
  program_action $app "Summary" "procedure" $action $gateway
fi
