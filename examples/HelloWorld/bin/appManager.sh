...
function usage() {
  echo "Application lifecycle management tool for the HelloWorld application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the HelloWorld application."
...
if [ "x$action" == "xdeploy" ]; then
  ...
  program_action $app "WhoFlow" "flow" $action $gateway
  program_action $app "Greeting" "procedure" $action $gateway
fi
