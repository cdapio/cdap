...
function usage() {
  echo "Application lifecycle management tool for the ResourceSpammer application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the ResourceSpammer application."
...
app="ResourceSpammerApp"

if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/ResourceSpammer-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "CPUSpammerFlow" "flow" $action $gateway
  program_action $app "spamProcedure" "procedure" $action $gateway
fi
