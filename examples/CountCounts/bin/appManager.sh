...
function usage() {
  echo "Application lifecycle management tool for the CountCounts application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the CountCounts application."
...
if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/CountCounts-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "CountCounts" "flow" $action $gateway
  program_action $app "CountCountsProcedure" "procedure" $action $gateway
fi
