...
function usage() {
  echo "Application lifecycle management tool for the ResponseCodeAnalyticsApp."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the ResponseCodeAnalytics application."
...
app="ResponseCodeAnalytics"

if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/ResponseCodeAnalytics-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "LogAnalyticsFlow" "flow" $action $gateway
  program_action $app "StatusCodeProcedure" "procedure" $action $gateway
fi
