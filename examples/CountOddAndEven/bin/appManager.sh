...
function usage() {
  echo "Application lifecycle management tool for the CountOddAndEven application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the CountOddAndEven application."
...
if [ "x$action" == "xdeploy" ]; then
  jar_path=`ls $dir/../target/CountOddAndEven-*.jar`
  deploy_action $app $jar_path $gateway
else
  program_action $app "CountOddAndEven" "flow" $action $gateway
fi
