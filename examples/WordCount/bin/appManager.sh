...
function usage() {
  echo "Application lifecycle management tool for the WordCount application."
  echo "Usage: $script --action <deploy|start|stop|status> [--gateway <hostname>]"
  echo ""
  echo "  Options"
  echo "    --action    Specifies the action to be taken on the WordCount application."
...
if [ "x$action" == "xdeploy" ]; then
  ...
  program_action $app "WordCounter" "flow" $action $gateway
  program_action $app "RetrieveCounts" "procedure" $action $gateway
fi
