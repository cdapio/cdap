...
public interface RuntimeJobManager {
   ...
   */
  CompletableFuture<RuntimeJobId> stop(RuntimeJobId runtimeJobId) throws Exception;

  /**
   * Forcefully kills a running job. If the job is already in terminal status, then this method should be a no-op.
   *
   * @param runtimeJobId job to be killed
   * @throws Exception thrown if any exception while killing the job
   */
  CompletableFuture<RuntimeJobId> kill(RuntimeJobId runtimeJobId) throws Exception;
}
