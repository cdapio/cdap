...
A MapReduce job uses the output of the Spark program from *ranks* dataset and computes the total number of pages for every unique page rank and tabulates its results in another ObjectStore dataset, *rankscount*.

The *PageRankWorkflow* ties the Spark and MapReduce to run together sequentially in this application.
...
*rankscount* dataset by using ``total`` endpoint of the *TotalPagesPR*. It will send the total number of pages which has the page rank queried for as a string.
...
The calculated page rank data is stored in an ObjectStore dataset, *ranks* and the total number of pages for a page rank is stored in ObjectStore dataset, *rankscount*

The ``RanksService`` and ``TotalPagesPR`` Service
----------------------------

This ``RanksService`` service has a ``rank`` endpoint to obtain the page rank of a given URL.
This ``TotalPagesPR`` service has a ``total`` endpoint to obtain the total number of pages for a given page rank.
...
- Go to the *SparkPageRank* `application overview page 
  ...
  and then do the same for the *GoogleTypePR* and *TotalPagesPR* service; or
- From the Standalone CDAP SDK directory, use the Command Line Interface::

    $ cdap-cli.sh start service SparkPageRank.RanksService
    $ cdap-cli.sh start service SparkPageRank.GoogleTypePR
    $ cdap-cli.sh start service SparkPageRank.TotalPagesPR
    ...
    Successfully started service 'TotalPagesPR' of application 'SparkPageRank' with stored runtime arguments '{}'
...
Similarily to query *rankscount* ObjectStore through the ``TotalPagesPR``

curl::

  $ curl -w'\n' http://localhost:10000/v3/namespaces/default/apps/SparkPageRank/services/TotalPagesPR/methods/total/10

Command Line Interface::

  $ cdap-cli.sh call service SparkPageRank.TotalPagesPR GET 'total/10'
...
- Go to the *SparkPageRank* `application overview page 
  ...
  doing the same for the ``GoogleTypePR`` and ``TotalPagesPR`` service; or
- From the Standalone CDAP SDK directory, use the Command Line Interface::

    $ cdap-cli.sh stop service SparkPageRank.RanksService
    $ cdap-cli.sh stop service SparkPageRank.GoogleTypePR
    $ cdap-cli.sh stop service SparkPageRank.TotalPagesPR
