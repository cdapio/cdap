...
- *stopwords.file*: The path to the file containing stopwords to filter in the *ScalaWikipediaAnalyzer* program.
  If unspecified, no words are considered as stopwords.
- *vocab.size*: The size of the vocabulary for the *ScalaWikipediaAnalyzer* program. Defaults to 1000.
...
- Go to the *WikipediaPipelineWorkflow* `application overview page
  ...
  click ``WikipediaPipelineWorkflow`` to get to the flow detail page, then click the *Stop* button; or
- From the Standalone CDAP SDK directory, use the Command Line Interface::

    $ cdap-cli.sh stop flow WikipediaPipeline.WikipediaPipelineWorkflow
