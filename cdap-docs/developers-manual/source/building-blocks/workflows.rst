...
--------------------------------------
When Workflow executes the MapReduce program, MapReduce counters generated by the program are added to the
Workflow token under the *System* scope. The counter value can be retrieved from the Workflow token using
counter group name, followed by ".", followed by the name of the counter as the key in the Workflow token.
For example - to access the the number of input records to the map method of the ``PurchaseHistoryBuilder``
MapReduce program::
