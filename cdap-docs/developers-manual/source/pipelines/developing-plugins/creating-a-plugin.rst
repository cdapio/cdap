...
Lineage:
  ...
  be a valid ``DatasetId`` or optionally the users can also extend from ``ReferenceBatchSource`` which verifies the
  validity of ``inputName`` if the plugin's ``PluginConfig`` extends from ``ReferencePluginConfig``.
  ``ReferenceBatchSource`` is available in the ``hydrator-common`` artifact.
...
Lineage:
  ...
  be a valid ``DatasetId`` or optionally the users can also extend from ``ReferenceBatchSink`` which verifies the
  validity of ``outputName`` if the plugin's ``PluginConfig`` extends from ``ReferencePluginConfig``.
  ``ReferenceBatchSink`` is available in the ``hydrator-common`` artifact.
...
Lineage:
  ...
  ``StreamingContext`` in ``getStream()``. Note that the ``referenceName`` should be a valid ``DatasetId`` or
  optionally the users can also extend from ``ReferenceStreamingSource`` which verifies the validity
  of ``referenceName`` if the plugin's ``PluginConfig`` extends from ``ReferencePluginConfig``.
