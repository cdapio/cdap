...
- :cask-issue:`CDAP-4535` - Authentication server announce address is now configurable
  ...
  ``security.auth.server.announce.address`` takes a single address in the form of
  ``host:port`` or ``host``. Protocol and port (if not specified) will be added by the
  Authentication Server to form the URL. A default URL will be generated by the
  Authentication Serve if either property is not set.
...
- :cask-issue:`CDAP-8997` - Fixed an issue with uncaught exceptions so that they are
  logged through the logger, allowing log collections for those exce
