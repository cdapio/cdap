...
.. _cdap-pipelines-creating-a-plugin-alert-publishers:

.. highlight:: java

Alert Publisher Plugin
======================
An ``AlertPublisher`` plugin is a special type of plugin that consumes alerts emitted from previous stages
...
It is up to the processing engine to decide when to publish alerts.
...
- ``initialize()``: Used to perform any initialization step that might be required during
  the runtime of the ``AlertPublisher``. It is guaranteed that this method will be invoked
