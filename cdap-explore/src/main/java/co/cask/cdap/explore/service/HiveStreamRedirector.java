...
public final class HiveStreamRedirector {
  ...
  static void redirectToLogger(SessionState sessionState, Logger logger) {
    ...
    sessionState.err = new PrintStream(RedirectedPrintStream.createRedirectedOutStream(logOut, null), true);
    sessionState.out = new PrintStream(RedirectedPrintStream.createRedirectedErrStream(logErr, null), true);

    sessionState.childErr = new PrintStream(RedirectedPrintStream.createRedirectedOutStream(logOut, null), true);
    sessionState.childOut = new PrintStream(RedirectedPrintStream.createRedirectedErrStream(logErr, null), true);
  }
