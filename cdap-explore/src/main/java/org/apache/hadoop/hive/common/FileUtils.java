...
public final class FileUtils {
  ...
  public static void checkFileAccessWithImpersonation(final FileSystem fs,
    ...
    LOG.info(String.format("CurrentUser: %s, doAsUser: %s", currentUser, user));

//    if (user == null || currentUser.equals(user) || ugi.getUserName().equals(user)) {
    if (user == null || currentUser.equals(user)) {
      LOG.info("no impersonation");
    ...
    proxyUser.doAs(new PrivilegedExceptionAction<Object>() {
      ...
      public Object run() throws Exception {
        LOG.info(String.format("alianwar0: %s", UserGroupInformation.getCurrentUser().getUserName()));
        LOG.info(String.format("alianwar0: %s", UserGroupInformation.getCurrentUser()));
        LOG.info(String.format("alianwar4: %s", UserGroupInformation.getCurrentUser().getCredentials().getAllTokens()));
        LOG.info(String.format("alianwar5: %s", UserGroupInformation.getCurrentUser().getTokens()));
        LOG.info(String.format("alianwar6: %s", UserGroupInformation.getCurrentUser().getTokenIdentifiers()));
...
//        try {
//          Method method =
//            shim.getClass().getMethod("setFullFileStatus", conf.getClass(), fullFileStatus.getClass(),
//                                      String.class, fs.getClass(), firstNonExistentParent.getClass(), boolean.class);
//          method.invoke(shim, conf, fullFileStatus, null, fs, firstNonExistentParent, true);
...
//      HiveConfUtil.updateJobCredentialProviders(conf);
      throw new IllegalArgumentException("blame ali");
...
//        return true;
  ...
  public static File createTempFile(String lScratchDir, String prefix, String suffix) throws IOException {
    ...
    Runtime.getRuntime().addShutdownHook(new DeleteOnExitHook(tmpFile));
    return tmpFile;
  }
