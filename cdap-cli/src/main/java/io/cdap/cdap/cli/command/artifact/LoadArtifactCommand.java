...
public class LoadArtifactCommand extends AbstractAuthCommand {
  ...
  @Inject
  public LoadArtifactCommand(ArtifactClient artifactClient, CLIConfig cliConfig,
  ...
  public void perform(Arguments arguments, PrintStream output) throws Exception {
    ...
    File artifactFile = resolver.resolvePathToFile(
        arguments.get(ArgumentName.LOCAL_FILE_PATH.toString()));
    ...
    if (name == null && version != null) {
      ...
      throw new IllegalArgumentException(
          "If a name is specified, a version must also be specified.");
    } else if (name == null) {
      artifactId = new ArtifactId(cliConfig.getCurrentNamespace().getNamespace(),
    ...
    if (configPath == null) {
      ...
      ArtifactConfig artifactConfig = configReader.read(Id.Namespace.fromEntityId(namespace),
          configFile);
      artifactClient.add(namespace, artifactId.getEntityName(),
          () -> new FileInputStream(artifactFile), artifactId.getVersion(),
