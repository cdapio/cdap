...
/** Fetch messages directly from {@link MessageTable} and {@link PayloadTable}. */
final class CoreMessageFetcher {
  ...
  public CloseableIterator<RawMessage> fetch(MessageFetchRequest messageFetchRequest)
    ...
    try {
      return new MessageCloseableIterator(messageTable, messageFetchRequest);
    } catch (Throwable t) {
  ...
  private final class MessageCloseableIterator implements CloseableIterator<RawMessage> {
    ...
    MessageCloseableIterator(MessageTable messageTable, MessageFetchRequest messageFetchRequest)
        throws IOException {
