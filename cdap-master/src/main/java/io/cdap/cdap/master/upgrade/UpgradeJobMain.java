...
public class UpgradeJobMain {
  ...
  public static void main(String[] args) {
    ...
    try {
      Retries.callWithRetries(
          () -> {
            suspendSchedulesAndStopPipelines(clientConfig, cConf);
            return null;
  ...
  private static void suspendSchedulesAndStopPipelines(ClientConfig clientConfig,
      CConfiguration conf) throws Exception {
            ...
            for (WorkflowId workflowId : workflowIds) {
              ...
              if (!programClient.getStatus(workflowId).equals(ProgramStatus.STOPPED.toString())) {
                try {
                  ...
                  // There might be race condition between checking if the program is in
                  // RUNNING state and stopping it.
                  // This can cause programClient.stop to throw BadRequestException so
                  // verifying if the program
