...
public class HealthCheckService extends AbstractIdleService {
  ...
  public void initiate(String host, int port, String serviceName) {
    ...
    NettyHttpService.Builder builder =
      new CommonNettyHttpServiceBuilder(cConf, serviceName).setHttpHandlers(handlers)
        .setHost(host)
        .setPort(port);
  ...
  public void helper(String serviceName, CConfiguration cConf, String hostName) {
    String hostAddress = cConf.get(hostName);
    int port = cConf.getInt(Constants.HealthCheck.SERVICE_BIND_PORT);
    initiate(hostAddress, port, serviceName);
  }
}
