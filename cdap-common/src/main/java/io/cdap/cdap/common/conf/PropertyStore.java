...
public interface PropertyStore<T> extends Closeable {
   ...
   * @param updater Updater to provides updated property value. The updater might get called
   *     multiple times.
   * @return A future that will be completed when the update is completed and will carry the
   *     property value being saved to property store.
   */
   ...
   * @return A future that will be completed when the property is saved to property store. The
   *     future will carry the property value being saved when completed.
   */
  ListenableFuture<T> set(String name, T property);

  /**
   * Adds a listener for listening changes on a given property. Calls to listener is guaranteed to
   * be serialized.
   *
   * @param name Name of the property
