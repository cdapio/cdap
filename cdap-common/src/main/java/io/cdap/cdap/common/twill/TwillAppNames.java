...
/**
 * Utility class to convert to and from the Twill application name for a {@link ProgramId}, since
 * Twill application name is a simple String and must encode all identifying information of a
 * program.
 */
public final class TwillAppNames {
  ...
  /**
   * Given a Twill app name, returns the id of the program that was used to construct this Twill app
   * name.
   *
  ...
  public static ProgramId fromTwillAppName(String twillAppName, boolean mustMatch,
    ...
    if (!matcher.matches()) {
      Preconditions.checkArgument(!mustMatch,
          "Twill app name '%s' does not match pattern for programs", twillAppName);
      return null;
    }
