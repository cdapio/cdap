...
/**
 * Generate a class bytecode that implements {@link FieldAccessor} for a given class field. The
 * generated class extends from {@link AbstractFieldAccessor} and overrides the {@link
 * AbstractFieldAccessor#get(Object)} and {@link AbstractFieldAccessor#set(Object, Object)} methods.
 * The primitive getter/setter will be overridden as well if the field it tries to access is of
 * primitive type.
 *
 * The class generated will try to be in the same package as the class enclosing the field, hence
 * directly access to the field if allowed (public/protected/package) to avoid using Java
 * Reflection. For private classes/fields, it will use reflection.
 */
@NotThreadSafe
...
//    io.cdap.cdap.internal.asm.Debugs.debugByteCode(classDefinition, new java.io.PrintWriter(System.out));
  ...
  private void generateConstructor(Field field) {
    ...
    GeneratorAdapter mg = new GeneratorAdapter(Opcodes.ACC_PUBLIC, constructor, null, new Type[0],
        classWriter);
    mg.loadThis();
    mg.loadArg(0);
  ...
  private void initializeReflectionField(GeneratorAdapter mg, Field field) {
    ...
    */
    ...
    // field.setAccessible(true);
    mg.push(true);
    mg.invokeVirtual(Type.getType(Field.class),
        getMethod(void.class, "setAccessible", boolean.class));
  ...
  private void invokeReflection(Method method, String signature) {
    GeneratorAdapter mg = new GeneratorAdapter(Opcodes.ACC_PUBLIC, method, signature, new Type[0],
        classWriter);
    /**
     * try {
     *   // Call method
    ...
    Label endTry = mg.newLabel();
    Label catchHandle = mg.newLabel();
    mg.visitTryCatchBlock(beginTry, endTry, catchHandle,
  ...
  private void directGetter(Field field) {
    GeneratorAdapter mg = new GeneratorAdapter(Opcodes.ACC_PUBLIC,
        getMethod(Object.class, "get", Object.class),
  ...
  private void directSetter(Field field) {
    ...
    mg.putField(Type.getType(field.getDeclaringClass()), field.getName(),
        Type.getType(field.getType()));
    mg.returnValue();
    mg.endMethod();
  }
  ...
  private void primitiveGetter(Field field) {
    ...
    GeneratorAdapter mg = new GeneratorAdapter(Opcodes.ACC_PUBLIC,
        getMethod(field.getType(), methodName, Object.class),
