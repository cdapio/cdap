...
public class AuthenticationChannelHandlerTest {
  ...
  public void testWriteWithAuditLogging() throws Exception {
    ...
    ctx.channel().attr(AttributeKey.valueOf(AUDIT_LOG_QUEUE_ATTR_NAME)).set(getAuditLogContexts());
    ...
    Assert.assertNull(ctx.channel().attr(AttributeKey.valueOf(AUDIT_LOG_QUEUE_ATTR_NAME)).get());
  }
