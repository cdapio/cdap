...
public class SupportBundlePipelineRunLogTask implements SupportBundleTask {
  ...
  private final ConcurrentHashMap<String, RunRecord> runRecordDetailMap;
  ...
  public SupportBundlePipelineRunLogTask(
    ...
    this.runRecordDetailMap = new ConcurrentHashMap<>();
  ...
  public void initializeCollection() {
    List<Future> futureRunLogList = new ArrayList<>();
    try {
      ...
      while (!runRecordDetailQueue.isEmpty()) {
        ...
        runRecordDetailMap.put(runId, runRecord);
        SupportBundleStatusTask task =
            initializeTask(runId + "-log", "PipelineRunLogSupportBundleTask");
        Future<SupportBundleStatusTask> futureRunLogService =
            executor.submit(
                () -> {
                  try {
                    ...
                    ProgramId programId =
                        new ProgramId(
                            namespaceId,
                            appId,
                            ProgramType.valueOfCategoryName("workflows"),
                            workflowName);
                    String runLog =
                    ...
                    LOG.warn(
                        String.format(
                            "Retried three times for this run id %s to generate run log ", runId),
                        e);
        ...
        if (!retryServiceMap.containsKey(runId)) {
          futureRunLogList.add(futureRunLogService);
        }
      }
      completeProcessing(futureRunLogList, basePath, "runlog");
    } catch (InterruptedException e) {
      LOG.warn("Can not process run log into queue ", e);
    }
  }
  ...
  public synchronized void queueTaskAfterFailed(String serviceName, SupportBundleStatusTask task) {
    if (retryServiceMap.getOrDefault(serviceName, 0) >= 3) {
      ...
      try {
        runRecordDetailQueue.put(runRecordDetailMap.get(serviceName));
      } catch (InterruptedException e) {
