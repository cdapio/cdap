...
public class PreviewHttpHandler extends AbstractLogHandler {
  ...
  private void sendLogs(String namespaceId, String previewId, Consumer<LogReaderInfo> logsResponder) throws Exception {
    ...
    RunRecordMeta runRecordMeta = previewManager.getRunner().getRunRecord(applicationId);
    LoggingContext loggingContext = LoggingContextHelper.getLoggingContextWithRunId(runRecordMeta.getProgramRunId(),
                                                                                    runRecordMeta.getSystemArgs());
    LogReader logReader = previewManager.getLogReader();
    logsResponder.accept(new LogReaderInfo(logReader, loggingContext, runRecordMeta));
  }
