...
public class ProgramLifecycleHttpHandler extends AbstractAppFabricHttpHandler {
  ...
  public void getStatus(HttpRequest request, HttpResponder responder,
                        ...
                        @PathParam("program-type") String type,
                        @PathParam("program-id") String programId) throws Exception {
    versionId = replaceDefaultVersionWithLatestIfNotFound(namespaceId, appId, versionId);
  ...
  private void doPerformAction(FullHttpRequest request, HttpResponder responder, String namespaceId, String appId,
    ...
    appVersion = replaceDefaultVersionWithLatestIfNotFound(namespaceId, appId, appVersion);
  ...
  public void programHistory(HttpRequest request, HttpResponder responder,
    ...
    ProgramType programType = getProgramType(type);
    appVersion = replaceDefaultVersionWithLatestIfNotFound(namespaceId, appName, appVersion);
  ...
  public void getProgramStatusSchedules(HttpRequest request, HttpResponder responder,
    ...
    triggerAppVersion = replaceDefaultVersionWithLatestIfNotFound(triggerNamespaceId, triggerAppName, triggerAppVersion);
  ...
  public void getSchedule(HttpRequest request, HttpResponder responder,
    ...
    doGetSchedule(responder, namespaceId, appName,
  ...
  private void doGetSchedule(HttpResponder responder, String namespace,
    ...
    version = replaceDefaultVersionWithLatestIfNotFound(namespace, app, version);
  ...
  public void getProgramSchedules(HttpRequest request, HttpResponder responder,
                                  ...
                                  @QueryParam("trigger-type") String triggerType,
                                  @QueryParam("schedule-status") String scheduleStatus) throws Exception {
    appVersion = replaceDefaultVersionWithLatestIfNotFound(namespace, application, appVersion);
  ...
  private void doAddSchedule(FullHttpRequest request, HttpResponder responder, String namespace, String appName,
    ...
    appVersion = replaceDefaultVersionWithLatestIfNotFound(namespace, appName, appVersion);
  ...
  private void updateLogLevels(FullHttpRequest request, HttpResponder responder, String namespace, String appName,
    ...
    appVersion = appVersion.equals(ApplicationId.DEFAULT_VERSION) ?
      getLatestAppVersion(new NamespaceId(namespace), appName) : appVersion;
    ProgramType programType = getProgramType(type);
    try {
   ...
   * @param appName
   * @param appVersion
   * @return latest app version if input appVersion is -SNAPSHOT
   * @throws ApplicationNotFoundException
   */
  private String replaceDefaultVersionWithLatestIfNotFound(String namespaceId, String appName, String appVersion)
    throws ApplicationNotFoundException {
    if (appVersion.equals(ApplicationId.DEFAULT_VERSION) &&
      store.getApplication(new ApplicationId(namespaceId, appName, appVersion)) == null) {
      // -snapshot version does not exist
      return getLatestAppVersion(new NamespaceId(namespaceId), appName);
    }
    return appVersion;
  }
