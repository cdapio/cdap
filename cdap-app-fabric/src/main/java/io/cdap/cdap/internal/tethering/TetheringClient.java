...
public final class TetheringClient {
   ...
   * @throws IOException if there was an exception while connecting to the server, or the server
   *     returned an error
  ...
  public void createTethering(PeerInfo peerInfo) throws URISyntaxException, IOException {
    ...
    if (response.getResponseCode() != 200) {
      throw new IOException(
          String.format("Failed to initiate tethering with peer %s, error code %s body %s",
              peerInfo.getName(),
  ...
  public TetheringControlResponseV2 pollControlChannel(PeerInfo peerInfo,
    ...
    try {
      return GSON.fromJson(resp.getResponseBodyAsString(StandardCharsets.UTF_8),
  ...
  private HttpResponse sendHttpRequest(RemoteAuthenticator remoteAuthenticator, HttpMethod method,
    ...
    return HttpRequests.execute(builder.build(),
        new HttpRequestConfig(connectionTimeout, readTimeout));
  }
