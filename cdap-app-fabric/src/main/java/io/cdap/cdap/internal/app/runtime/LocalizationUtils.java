...
public final class LocalizationUtils {
  ...
  /**
   * Localizes the specified {@link LocalizeResource} in the specified {@link File targetDir} with
   * the specified file name and returns the {@link File} pointing to the localized file.
   *
  ...
  public static File localizeResource(String fileName, LocalizeResource resource, File targetDir)
    ...
    if (resource.isArchive()) {
      ...
      try {
        ...
        java.nio.file.Files.copy(input.toPath(), localizedResource.toPath(),
  ...
  /**
   * Returns a local {@link File} for the specified {@link LocalizeResource}. If the specified
   * {@link LocalizeResource} points to a local file already, it simply returns a {@link File}
   * object for it. If it is not local, then the method will try to download the resource to a local
   ...
   */
  private static File getFileToLocalize(LocalizeResource resource, File tempDir)
      throws IOException {
  ...
  /**
   * Get the filename part from a {@link URI} to be localized. For a URI "file:///tmp/foo.jar",
   * "foo.jar" will be returned. For a URI "file:///tmp/foo.jar#bar.jar", "bar.jar" will be
   * returned.
   *
