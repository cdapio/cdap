...
public class RemoteArtifactRepositoryReader implements ArtifactRepositoryReader {
  ...
  public ArtifactDetail getArtifact(Id.Artifact artifactId) throws Exception {
    ...
    if (artifactLocalizerClient.isPresent()) {
      ArtifactId id = new ArtifactId(detail.getDescriptor().getNamespace(),
                                             detail.getDescriptor().getArtifactId().getName(),
                                             detail.getDescriptor().getArtifactId().getVersion().getVersion());
      location = Locations.toLocation(artifactLocalizerClient.get().getArtifactLocation(id));
    } else {
  ...
  public List<ArtifactDetail> getArtifactDetails(ArtifactRange range, int limit, ArtifactSortOrder order)
    ...
    for (ArtifactDetail detail : details) {
      ...
      Location location;
      if (artifactLocalizerClient.isPresent()) {
        ArtifactId artifactId = new ArtifactId(detail.getDescriptor().getNamespace(),
                                       detail.getDescriptor().getArtifactId().getName(),
                                       detail.getDescriptor().getArtifactId().getVersion().getVersion());
        location = Locations.toLocation(artifactLocalizerClient.get().getArtifactLocation(artifactId));
      } else {
