...
public class UserServiceProgramMain extends AbstractServiceMain<ServiceOptions> {
  ...
  protected LoggingContext getLoggingContext(ServiceOptions options) {
    ...
    LOG.info("wyzhang: UserServiceProgramMain:getLoggingContext programOptionsFile={}",
             programOptionsFile.getAbsolutePath());
    LOG.info("wyzhang: UserServiceProgramMain:getLoggingContext program options");
    for (Map.Entry<String, String> e :programOptions.getArguments().asMap().entrySet()) {
      LOG.info("wyzhang: {}, {}", e.getKey(), e.getValue());
    }
    for (Map.Entry<String, String> e :programOptions.getUserArguments().asMap().entrySet()) {
      LOG.info("wyzhang: {}, {}", e.getKey(), e.getValue());
    }
