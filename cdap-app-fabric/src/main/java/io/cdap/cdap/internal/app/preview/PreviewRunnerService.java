 ...
 */
public class PreviewRunnerService extends AbstractIdleService {
  ...
  @Inject
  PreviewRunnerService(CConfiguration cConf,
    ...
    NettyHttpService.Builder builder = commonNettyHttpServiceFactory.builder(
      ...
      .setHttpHandlers(new PreviewHttpHandlerInternal(cConf, discoveryService,
                                                      discoveryServiceClient, this::stopService, metricsCollectionService)); // TODO : dbshweta -Check here for Task Worker Http Internal Handler
  ...
  protected void startUp() throws Exception {
    LOG.debug("Starting PreviewRunnerService");
    httpService.start();
    bindAddress = httpService.getBindAddress();
    cancelDiscovery = discoveryService.register(
      ResolvingDiscoverable.of(
        URIScheme.createDiscoverable(Constants.Service.PREVIEW_RUNNER, httpService)));
    LOG.debug("Starting PreviewRunnerService has completed");
  }

  @Override
  protected void shutDown() throws Exception {
    LOG.debug("Shutting down PreviewRunnerService");
    httpService.stop(1, 2, TimeUnit.SECONDS);
    cancelDiscovery.cancel();
    LOG.debug("Shutting down PreviewRunnerService has completed");
  }
