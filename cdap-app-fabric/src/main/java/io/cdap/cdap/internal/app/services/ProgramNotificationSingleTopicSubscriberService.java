...
class ProgramNotificationSingleTopicSubscriberService
  ...
  private void sendStartMessage( RunRecordDetail runRecordDetail){
                ...
                (runRecordDetail) -> {
                  ...
                  try {
                    if (runRecordDetail.getStatus() == ProgramRunStatus.PENDING) {
                      // if operation try launching directly
                      if (runRecordDetail.getProgramRunId().getType() == ProgramType.Operation) {
                        operationLifecycleService.startOperation(runRecordDetail);
                      } else {
  ...
  private void handleProgramEvent(
    ...
    switch (programRunStatus) {
                ...
                try {
                  ...
                  try {
                    if (programRunId.getType() == ProgramType.Operation) {
                      operationLifecycleService.startOperation(prgOptions, programRunId);
                    } else {
                      programLifecycleService.startInternal(prgDescriptor, prgOptions,
                          programRunId);
                    }
                  } catch (Exception e) {
      ...
      case SUSPENDED:
        if (programRunId.getType() == ProgramType.Operation) {
          // ignore suspend messages for operation
          recordedRunRecord = null;
          break;
        }
        long suspendTime =
   ...
   */
  private Optional<Runnable> handleClusterEvent(
      ProgramRunId programRunId,
  ...
  private Optional<Runnable> getEmitMetricsRunnable(
      ProgramRunId programRunId,
      @Nullable RunRecordDetail recordedRunRecord,
      String metricName,
  ...
  private Map<String, String> getAdditionalTagsForProfileMetrics(
  ...
  private Map<String, String> getAdditionalTagsForProgramMetrics(
