...
public class AdminEventPublisher {
  ...
  /**
   * Update the profile metadata for the affected programs/schedules, the metadata of
   * programs/schedules will be updated according to the closest level preference that contains the
   * profile information.
   *
   * @param entityId the entity id to update metadata, all programs/schedules under this entity
   *     id will be affected
  ...
  /**
   * Publish a message when a profile is unassigned to an entity. The metadata of the
   * programs/schedules will be reindexed to the closest level preference that contains the profile
   * information
   *
   * @param entityId the entity id to reindex metadata, all programs/schedules under this entity
   *     id will be affected
   * @param seqId the sequence id of the preferences operation
   */
  ...
  /**
   * Publish a message about app creation, the programs/schedules which get created will get updated
   * with the profile metadata
   *
  ...
  /**
   * Publish a message about schedule creation, the schedules which get created will get updated
   * with the profile metadata
   *
  ...
  /**
   * Remove the profile metadata for the given app id. Note that the app spec is required since we
   * might not find the program/schedule information from the app meta store since they can be
   * deleted. So we must specify app spec here to make sure their profile metadata is removed.
   *
  ...
  /**
   * Remove the profile metadata for the given schedule id. Note that the ProgramSchedule is needed
   * since the schedule might already get deleted when we process the message. So we must specify it
   * to make sure the profile metadata is removed.
   *
  ...
  public void publishScheduleDeletion(ProgramSchedule programSchedule) {
    publishMessage(programSchedule.getScheduleId(), MetadataMessage.Type.ENTITY_DELETION,
        programSchedule);
  }
  ...
  private void publishMessage(EntityId entityId, MetadataMessage.Type type,
    ...
    try {
          ...
          () -> {
            try {
              messagingContext.getMessagePublisher()
                  .publish(NamespaceId.SYSTEM.getNamespace(), topic.getTopic(),
                      GSON.toJson(message));
      ...
      throw new RuntimeException(
          String.format("Failed to publish profile metadata request for entity id %s",
              entityId), e);
    }
  }
}
