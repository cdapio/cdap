...
/**
 * A helper class for dealing with MapReduce framework localization and classpath settings based on
...
public final class MapReduceContainerHelper {
  ...
  /**
   * Adds the classpath to be used in MapReduce job execution based on the given {@link
   * Configuration}.
   *
   ...
   */
  public static <T extends Collection<String>> T addMapReduceClassPath(Configuration hConf,
      T result) {
  ...
  /**
   * Gets the MapReduce framework URI based on the {@code mapreduce.application.framework.path}
   * setting.
   *
   * @param hConf the job configuration
   * @return the framework URI or {@code null} if not present or if the URI in the config is
   *     invalid.
   */
  @Nullable
  private static URI getFrameworkURI(Configuration hConf) {
    ...
    try {
      ...
      LOG.warn(
          "Failed to parse {} as a URI. MapReduce framework path is not used. Check the setting for {}.",
          framework, MRJobConfig.MAPREDUCE_APPLICATION_FRAMEWORK_PATH, e);
    } catch (IOException e) {
      LOG.warn(
          "Failed to resolve {} URI. MapReduce framework path is not used. Check the setting for {}.",
          framework, MRJobConfig.MAPREDUCE_APPLICATION_FRAMEWORK_PATH, e);
    }
    return null;
  }
  ...
  /**
   * Sets the resources that need to be localized to program runner twill container that used as
   * MapReduce client.
   *
   ...
   */
  public static <T extends Map<String, LocalizeResource>> T localizeFramework(Configuration hConf,
      T result) {
    try {
      ...
      if (frameworkURI != null) {
        URI uri = new URI(frameworkURI.getScheme(), frameworkURI.getAuthority(),
            frameworkURI.getPath(), null, null);
