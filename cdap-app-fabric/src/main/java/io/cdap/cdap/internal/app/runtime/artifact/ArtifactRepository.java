...
/**
 * This class manages artifact and artifact metadata. It is mainly responsible for inspecting
 * artifacts to determine metadata for the artifact.
 */
public interface ArtifactRepository extends ArtifactRepositoryReader {
   ...
   * classes, with access to packages that all program type has access to. It delegates to {@link
   * ArtifactClassLoaderFactory#createClassLoader(Location, EntityImpersonator)}.
   *
  ...
  /**
   * Clear all artifacts in the given namespace. This method is only intended to be called by unit
   * tests, and when a namespace is being deleted.
   *
   ...
   *     store
   * @throws ArtifactNotFoundException if the given artifact does not exist
   * @throws PluginNotExistsException if no plugins of the given type and name are available to
   *     the given artifact
   */
   ...
   * @throws InvalidArtifactException if the artifact is invalid. For example, if it is not a
   *     zip file, or the application class given is not an Application.
   * @throws UnauthorizedException if the current user does not have the privilege to add an
   *     artifact in the specified namespace. To add an artifact, a user needs {@link
   *     StandardPermission#UPDATE} privilege on the namespace in which the artifact is being
   *     added.
   */
  ArtifactDetail addArtifact(Id.Artifact artifactId, File artifactFile) throws Exception;

  /**
   * Inspects and builds plugin and application information for the given artifact, adding an
   * additional set of plugin classes to the plugins found through inspection. This method is used
   * when all plugin classes cannot be derived by inspecting the artifact but need to be explicitly
   * set. This is true for 3rd party plugins like jdbc drivers.
   *
   * @param artifactId the id of the artifact to inspect and store
   * @param artifactFile the artifact to inspect and store
   * @param parentArtifacts artifacts the given artifact extends. If null, the given artifact
   *     does not extend another artifact
   * @param additionalPlugins the set of additional plugin classes to add to the plugins found
   *     through inspection. If null, no additional plugin classes will be added
   * @throws IOException if there was an exception reading from the artifact store
   * @throws ArtifactRangeNotFoundException if none of the parent artifacts could be found
   * @throws UnauthorizedException if the user is not authorized to add an artifact in the
   *     specified namespace. To add an artifact, a user must have {@link StandardPermission#UPDATE}
   *     on the namespace in which the artifact is being added.
   */
   ...
   * @throws UnauthorizedException if the current user is not permitted to write properties to
   *     the artifact. To be able to write properties to an artifact, users must have admin
   *     privileges on the artifact
   */
  void writeArtifactProperties(Id.Artifact artifactId, Map<String, String> properties)
      throws Exception;

  /**
   * Writes a property for an artifact. If the property already exists, it will be overwritten. If
   ...
   * @throws IOException if there was an exception writing to the artifact store
   * @throws ArtifactNotFoundException if the artifact does not exist
   * @throws UnauthorizedException if the current user is not permitted to remove a property
   *     from the artifact. To be able to remove a property, users must have admin privileges on the
   *     artifact
   */
   ...
   * @throws IOException if there was an exception writing to the artifact store
   * @throws ArtifactNotFoundException if the artifact does not exist
   * @throws UnauthorizedException if the current user is not permitted to remove properties
   *     from the artifact. To be able to remove properties, users must have admin privileges on the
   *     artifact
   */
  void deleteArtifactProperties(Id.Artifact artifactId) throws Exception;

  /**
   * Scan all files in the local system artifact directory, looking for jar files and adding them as
   * system artifacts. If the artifact already exists it will not be added again unless it is a
   * snapshot version.
   *
   ...
   * @throws ArtifactNotFoundException if the given artifact does not exist
   * @throws UnauthorizedException if the current user is not authorized to delete the artifact.
   *     To delete an artifact, a user needs {@link StandardPermission#DELETE} permission on the
   *     artifact.
   */
