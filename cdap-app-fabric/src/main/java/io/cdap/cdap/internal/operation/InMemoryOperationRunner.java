...
public class InMemoryOperationRunner extends AbstractOperationRunner {
  ...
  @Override
  public OperationController run(OperationRunDetail detail)
      throws OperationTypeNotSupportedException {
    LongRunningOperationContext context = new LongRunningOperationContext(detail.getRunId(),
        detail.getRun().getType(), statePublisher);
    OperationDriver driver = new OperationDriver(getOperation(detail), context);
