...
/**
 * Performs steps to deprovision a cluster for a program run. Before any operation is performed,
 * state is persisted to the ProvisionerStore to record what we are doing. This is done in case we
 * crash in the middle of the task and the task is later restarted. The operation state transition
 * looks like:
 *
 * |-- (state == NOT_FOUND) --> Deleted RequestingDelete -- request delete --> PollingDelete --| |--
 * (state == FAILED || state == ORPHANED) --> Orphaned
 *
 * Some cluster statuses are not expected when polling for delete state. They are handled as
 * follows:
 *
