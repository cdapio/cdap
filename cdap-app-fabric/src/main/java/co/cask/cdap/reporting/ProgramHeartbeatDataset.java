  ...
  public Collection<Notification> scan(byte[] startRowKey, byte[] endRowKey) {
    try (Scanner scanner = table.scan(startRowKey, endRowKey)) {
      ...
      while ((row = scanner.next()) != null) {
        ...
        // TODO add coverage for this condition -- might not be needed, recheck
        if (programRunStatus.isEndState() && result.containsKey(programRunId)) {
          // if program run ended and we have notification from running heart-beat, we merge current properties into
          // existing.
          Map<String, String> mergedMap = new HashMap<>(result.get(programRunId).getProperties());
          mergedMap.putAll(notification.getProperties());
          result.put(programRunId, new Notification(notification.getNotificationType(), mergedMap));
          continue;
        }
