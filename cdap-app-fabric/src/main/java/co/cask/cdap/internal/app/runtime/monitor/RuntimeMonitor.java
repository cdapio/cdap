...
public class RuntimeMonitor extends AbstractExecutionThreadService {
  ...
  protected void run() {
    ...
    callWithRetries(new Retries.Callable<Long, TransactionFailureException>() {
      @Override
      public Long call() throws TransactionFailureException {
        Transactionals.execute(transactional, context -> {
  ...
  private long processResponse(Map<String, List<MonitorMessage>> monitorResponses) throws Exception {
    ...
    for (Map.Entry<String, List<MonitorMessage>> monitorResponse : monitorResponses.entrySet()) {
      ...
      Long publishTime = callWithRetries(new Retries.Callable<Long, TransactionFailureException>() {
        @Override
        public Long call() throws TransactionFailureException {
          Transactionals.execute(transactional, context -> {
            return publish(topicConfig, topic, monitorMessages, context);
          });
          return null;
        }
      });
