 ...
 */
public class CreateStreamsStage extends AbstractStage<ApplicationSpecLocation> {
  private static final Logger LOG = LoggerFactory.getLogger(CreateStreamsStage.class);
  ...
  public void process(ApplicationSpecLocation input) throws Exception {
    ...
    for (Map.Entry<String, StreamSpecification> streamEntry : specification.getStreams().entrySet()) {
      String streamName = streamEntry.getKey();
      // currently unused, but may include a schema and format in the future.
      StreamSpecification streamSpec = streamEntry.getValue();
