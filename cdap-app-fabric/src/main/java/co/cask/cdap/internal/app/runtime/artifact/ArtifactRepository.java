...
public class ArtifactRepository {
  ...
  public ArtifactDetail addArtifact(Id.Artifact artifactId, File artifactFile) throws Exception {
    ...
    authorizerSupplier.get().enforce(namespace, principal, Action.WRITE);
    ...
    authorizerSupplier.get().grant(artifactId.toEntityId(), principal, Collections.singleton(Action.ALL));
  ...
  public void writeArtifactProperties(Id.Artifact artifactId, final Map<String, String> properties) throws Exception {
    authorizerSupplier.get().enforce(artifactId.toEntityId(), SecurityRequestContext.toPrincipal(),
                                                Action.ADMIN);
  ...
  public void addSystemArtifacts() throws Exception {
    ...
    if (Principal.SYSTEM.equals(principal)) {
      ...
      authorizerSupplier.get().enforce(instanceId, principal, Action.WRITE);
    }
  ...
  public void deleteArtifact(Id.Artifact artifactId) throws Exception {
    ...
    if (NamespaceId.SYSTEM.equals(artifactId.getNamespace().toEntityId())) {
      authorizerSupplier.get().enforce(instanceId, principal, Action.ADMIN);
    } else {
      authorizerSupplier.get().enforce(artifactId.toEntityId(), principal, Action.ADMIN);
    }
    // revoke all privileges on the artifact
    authorizerSupplier.get().revoke(artifactId.toEntityId());
