...
public class ApplicationLifecycleService extends AbstractIdleService {
  ...
  private ApplicationWithPrograms deployApp(NamespaceId namespaceId, @Nullable String appName,
    ...
    profileMetadataPublisher.updateProfileMetadata(applicationWithPrograms.getApplicationId());
  ...
  private void deleteApp(final ApplicationId appId, ApplicationSpecification spec) throws Exception {
    ...
    profileMetadataPublisher.removeProfileMetadata(appId, getProgramsWithType(appId, ProgramType.WORKFLOW, appSpec));
  }
  ...
  private Set<ProgramId> getProgramsWithType(ApplicationId appId, ProgramType type, ApplicationSpecification appSpec) {
    Set<ProgramId> result = new HashSet<>();
    Map<ProgramType, Set<String>> programTypeToNames = new HashMap<>();
    programTypeToNames.put(type, appSpec.getFlows().keySet());
  ...
  private Set<ProgramId> getAllPrograms(ApplicationId appId, ApplicationSpecification appSpec) {
    Set<ProgramId> result = new HashSet<>();
    result.addAll(getProgramsWithType(appId, ProgramType.FLOW, appSpec));
    result.addAll(getProgramsWithType(appId, ProgramType.MAPREDUCE, appSpec));
    result.addAll(getProgramsWithType(appId, ProgramType.WORKFLOW, appSpec));
    result.addAll(getProgramsWithType(appId, ProgramType.SERVICE, appSpec));
    result.addAll(getProgramsWithType(appId, ProgramType.SPARK, appSpec));
    result.addAll(getProgramsWithType(appId, ProgramType.WORKER, appSpec));
    return result;
  }
