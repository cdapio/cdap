...
public class ApplicationLifecycleService extends AbstractIdleService {
  ...
  public ApplicationWithPrograms updateApp(Id.Application appId, AppRequest appRequest,
    ...
    authorizerSupplier.get().enforce(appId.toEntityId(), SecurityRequestContext.toPrincipal(), Action.ADMIN);
  ...
  private ApplicationWithPrograms deployApp(Id.Namespace namespace, @Nullable String appName,
    ...
    authorizerSupplier.get().enforce(namespace.toEntityId(), SecurityRequestContext.toPrincipal(),
                                                Action.WRITE);
    ...
    authorizerSupplier.get().grant(namespace.toEntityId().app(applicationWithPrograms.getId().getId()),
  ...
  private void revokePrivileges(ApplicationId appId, ApplicationSpecification appSpec) throws Exception {
    for (ProgramId programId : getAllPrograms(appId, appSpec)) {
      authorizerSupplier.get().revoke(programId);
    }
    authorizerSupplier.get().revoke(appId);
  }
