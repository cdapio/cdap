...
public class ArtifactRepositoryTest {
  ...
  public void testAddSystemArtifacts() throws Exception {
    ...
    Assert.assertEquals(systemAppArtifactId.getName(), appArtifactDetail.getDescriptor().getArtifact().getName());
    Assert.assertEquals(systemAppArtifactId.getVersion(),
                        appArtifactDetail.getDescriptor().getArtifact().getVersion());

    // check plugin artifact added correctly
    ArtifactDetail pluginArtifactDetail = artifactRepository.getArtifact(pluginArtifactId);
    Assert.assertEquals(pluginArtifactId.getName(), pluginArtifactDetail.getDescriptor().getArtifact().getName());
    Assert.assertEquals(pluginArtifactId.getVersion(),
                        pluginArtifactDetail.getDescriptor().getArtifact().getVersion());
  ...
  public void testPluginSelector() throws Exception {
    ...
    Assert.assertEquals(new ArtifactVersion("1.0"), plugin.getKey().getArtifact().getVersion());
    ...
    Assert.assertNotNull(plugin);
    Assert.assertEquals(new ArtifactVersion("2.0"), plugin.getKey().getArtifact().getVersion());
    Assert.assertEquals("TestPlugin2", plugin.getValue().getName());
