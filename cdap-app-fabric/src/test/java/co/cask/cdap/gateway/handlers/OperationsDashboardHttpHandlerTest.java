...
public class OperationsDashboardHttpHandlerTest extends AppFabricTestBase {
  ...
  public void testDashboardDetail() throws Exception {
    ...
    long sourceId = 0;
    // run1
    ProgramRunId runId1 = programId1.run(RunIds.generate());
    store.setProvisioning(runId1, 10, Collections.emptyMap(), Collections.emptyMap(),
                          Bytes.toBytes(++sourceId), artifactId);
    store.setStop(runId1, 50, ProgramRunStatus.COMPLETED, Bytes.toBytes(++sourceId));
    // run2
    ProgramRunId runId2 = programId1.run(RunIds.generate());
    store.setProvisioning(runId2, 60, Collections.emptyMap(), Collections.emptyMap(),
                          Bytes.toBytes(++sourceId), artifactId);
    ...
    // run3
    ProgramRunId runId3 = programId2.run(RunIds.generate());
    store.setProvisioning(runId3, 120, Collections.emptyMap(), Collections.emptyMap(),
                          Bytes.toBytes(++sourceId), artifactId);

    store.setStop(runId3, 200, ProgramRunStatus.COMPLETED, Bytes.toBytes(++sourceId));
    // run4
    ProgramRunId runId4 = programId2.run(RunIds.generate());
    store.setProvisioning(runId4, 2000, Collections.emptyMap(), Collections.emptyMap(),
                          Bytes.toBytes(++sourceId), artifactId);
    ...
    // run5
    ProgramRunId runId5 = programId3.run(RunIds.generate());
    store.setProvisioning(runId5, 120, Collections.emptyMap(), Collections.emptyMap(),
                          Bytes.toBytes(++sourceId), artifactId);
