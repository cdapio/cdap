...
public class TransactionRunners {
   ...
   * @throws RuntimeException where the cause is wrapped with {@link RuntimeException} if it is
   *     not already a {@link RuntimeException}
   */
   ...
   * @throws X if failed to execute the given {@link io.cdap.cdap.api.TxRunnable} in a
   *     transaction. If the TransactionException has a cause in it, the cause is thrown as-is if it
   *     is an instance of X.
   ...
   *     TransactionException has a cause in it, the cause is thrown as-is if it is an instance of
   *     X1.
   ...
   *     TransactionException has a cause in it, the cause is thrown as-is if it is an instance of
   *     X3.
   * @throws RuntimeException if cause is not an instance of X1 or X2 or X3. The cause is
   *     wrapped with {@link RuntimeException} if it is not already a {@link RuntimeException}.
   */
   ...
   *     transaction. If the TransactionException has a cause in it, the cause is propagated.
   */
  ...
  /**
   * Propagates the given {@link TransactionException}. If the {@link
   * TransactionException#getCause()} doesn't return {@code null}, the cause will be used instead
   * for the propagation. This method will throw the failure exception as-is the given propagated
   * type if the type matches or as {@link RuntimeException}.
   *
   ...
   * @param <X> exception type of propagate type
   */
  public static <X extends Throwable> X propagate(TransactionException e, Class<X> propagateType)
      throws X {
   ...
   * TransactionException#getCause()} doesn't return {@code null}, the cause will be used instead
   * for the propagation. This method will throw the failure exception as-is the given propagated
   * types if the type matches or as {@link RuntimeException}.
   *
   * @param e the {@link TransactionException} to propagate
   ...
   * @param propagateType2 if the exception is an instance of this type, it will be rethrown as
   *     is
   * @param propagateType3 if the exception is an instance of this type, it will be rethrown as
   *     is
   * @param <X1> exception type of first propagate type
   * @param <X2> exception type of second propagate type
   * @param <X3> exception type of third propagate type
