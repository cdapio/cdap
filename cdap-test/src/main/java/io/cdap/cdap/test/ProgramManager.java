...
public interface ProgramManager<T extends ProgramManager> {
  ...
  /**
   * Starts the program and waits for one additional run of the specified status. This is
   * essentially a call to {@link #start()} followed by a call to {@link
   * #waitForRuns(ProgramRunStatus, int, long, TimeUnit)}. It should not be used if there is another
   * run in progress.
   *
   ...
   * Starts the program and waits for one additional run of the specified status. This is
   * essentially a call to {@link #start()} followed by a call to {@link
   * #waitForRuns(ProgramRunStatus, int, long, TimeUnit)}. Fails early if run gets into status
   * that's {@link ProgramRunStatus#isUnsuccessful()}. It should not be used if there is another run
   * in progress.
   *
   * @param status the status of the run to wait for
   * @param timeout amount of time units to wait
   ...
   * This method assumes another run is not started by another thread. This is essentially a call to
   * {@link #start(Map)} ()} followed by a call to {@link #waitForRuns(ProgramRunStatus, int, long,
   * TimeUnit)}. It should not be used if there is another run in progress.
   *
   * @param arguments the arguments to start the program with
   * @param status the status of the run to wait for
   * @param timeout amount of time units to wait
   ...
   * @return the program manager itself
   */
  T startAndWaitForRun(Map<String, String> arguments, ProgramRunStatus status, long timeout,
      TimeUnit timeoutUnit)
      throws InterruptedException, ExecutionException, TimeoutException;

  /**
   * Starts the program with arguments and waits for one additional run of the specified status.
   * Fails early if run gets into status that's {@link ProgramRunStatus#isUnsuccessful()}. This
   * method assumes another run is not started by another thread. This is essentially a call to
   * {@link #start(Map)} ()} followed by a call to {@link #waitForRuns(ProgramRunStatus, int, long,
   * TimeUnit)}. It should not be used if there is another run in progress.
   *
   * @param arguments the arguments to start the program with
  ...
  /**
   * Wait for the status of the program with 5 seconds timeout. A started program does not mean all
   * functionality is available. For example, a started service may not yet have registered its url.
   * In most cases, {@link #waitForRun(ProgramRunStatus, long, TimeUnit)} should be used with {@link
   * ProgramRunStatus#RUNNING} to wait for a program to actually be running, and {@link
   * #waitForStopped(long, TimeUnit)} should be used to wait for the program to stop.
   *
   ...
   * @deprecated use {@link #waitForRun(ProgramRunStatus, long, TimeUnit)} or {@link
   *     #waitForStopped(long, TimeUnit)}.
   */
  @Deprecated
   ...
   */
  void waitForStopped(long timeout, TimeUnit timeUnit)
  ...
  /**
   * Blocks until at least {@code runCount} number of run records with the given {@link
   * ProgramRunStatus}.
   *
   * @param status the status of the run record
  ...
  /**
   * Wait for the status of the program, retrying a given number of times with a timeout between
   * attempts.
   *
   * @param status true if waiting for started, false if waiting for stopped.
