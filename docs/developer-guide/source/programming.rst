...
The TrafficAnalytics application can be deployed using the deployApplication method from ReactorTestBase

	// Deploy an app.
    	ApplicationManager appManager = deployApplication(TrafficAnalyticsApp.class);


The MapReduce job reads for TrafficAnalytics application reads from logEventTable DataSet. As a first step the data to logEventTable should be populated
by running the RequestCountFlow and sending data to the logEventStream stream
