...
You add workers to your Service by calling the ``addHandler`` method in the Service's ``configure`` method.

To use Datasets within a handler, specify the Datasets being used by calling the ``useDataset`` method in the Service's
``configure`` method and use the ``@UseDataSet`` annotation to get an instance of the Dataset. By default, operations on
a Dataset are non-transactional. To use Datasets transactionally, annotate the handler method with ``@EnableTransaction``.
Each request to that method is then committed as a single transaction.
