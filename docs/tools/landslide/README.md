...
# Viewing (Continuuity version)

- Press `h` to toggle display of help
- Press `left`, `right`, `up`, `down` arrows, spacebar, shift-spacebar to navigate
- Press `t` to toggle a table of contents for your presentation. Slide titles
  are links
- Press `e` to display the presentation overview (Exposé)
- Press `1` to toggle slide number visibility
- Press `b` to toggle screen blanking
- Press `c` to toggle current slide context (previous and next slides)
- Press `f` to make slides filling the whole screen
- Press `w` to make slides filling the whole available window (space) within the
  document body
- Press `p` to toggle presenter view
- Press `s` to toggle display of link to the source file for each slide
- Press 'n' to toggle notes in your slides (specify with the .notes macro)
...
**NOTE:** Paths *inside* the css and js files must be relative to the css and js files.
