...
class Generator(object):
    ...
    def find_theme_dir(self, theme, copy_theme=False):
        ...
        if os.path.abspath(theme) == os.path.abspath(target_theme_dir):
            self.log(u'Using theme directory %s' % theme)
        elif copy_theme or os.path.exists(target_theme_dir):
            ...
            if os.path.exists(target_theme_dir):
                try:
                    ...
                    self.log(u"Error deleting existing theme folder: %s" % e)
                    pass
            try:
                shutil.copytree(self.theme_dir, target_theme_dir)
                self.log(u'Copied   %s theme directory to %s'
                         % (theme, target_theme_dir))
            except Exception, e:
