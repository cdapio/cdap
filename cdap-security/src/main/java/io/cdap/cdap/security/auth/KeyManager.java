...
public interface KeyManager extends Service {
  ...
  /**
   * Represents the combination of a digest computed on a message using a secret key, and the ID of
   * the secret key used to compute the digest.  Both elements are needed in order to later
   * recompute (validate) the digest.
   */
  class DigestId {
   ...
   * @throws java.security.InvalidKeyException If the internal {@code Mac} implementation does
   *     not accept the given key.
   */
   ...
   *
   * @param codec The serialization utility to use in serializing the message when recomputing
   *     the digest
