...
public class AuthorizerWrapper implements AccessController {
  ...
  public void grant(Authorizable authorizable, Principal principal, Set<? extends Permission> permissions)
    ...
    try {
      authorizer.grant(authorizable, principal, getActionSet(permissions));
    } catch (Exception e) {
      throw AuthEnforceUtil.propagateAccessException(e);
    }
  }
  ...
  public void revoke(Authorizable authorizable, Principal principal, Set<? extends Permission> permissions)
    throws AccessException {
    try {
      authorizer.revoke(authorizable, principal, getActionSet(permissions));
    } catch (Exception e) {
      throw AuthEnforceUtil.propagateAccessException(e);
    }
  }
  ...
  public void enforce(EntityId entity, Principal principal, Permission permission) throws AccessException {
    try {
      authorizer.enforce(entity, principal, getAction(permission));
    } catch (Exception e) {
      throw AuthEnforceUtil.propagateAccessException(e);
    }
  }
  ...
  public void enforce(EntityId entity, Principal principal, Set<? extends Permission> permissions)
    throws AccessException {
    try {
      authorizer.enforce(entity, principal, getActionSet(permissions));
    } catch (Exception e) {
      throw AuthEnforceUtil.propagateAccessException(e);
    }
  }
  ...
  private Set<Action> getActionSet(Set<? extends Permission> permissions) {
    return permissions.stream().map(AuthorizerWrapper::getAction).collect(Collectors.toCollection(LinkedHashSet::new));
  }

  static Action getAction(Permission permission) {
    if (StandardPermission.GET == permission) {
      return Action.READ;
    } else if (StandardPermission.UPDATE == permission) {
      return Action.ADMIN;
