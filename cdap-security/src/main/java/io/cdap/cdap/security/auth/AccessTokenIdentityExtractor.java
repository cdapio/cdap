...
/**
 * The AccessTokenIdentityExtractor attempts to extract an {@link AccessToken} included in the
 * Authorization header, validate it, then transform it into a {@link UserIdentityPair}.
 */
public class AccessTokenIdentityExtractor implements UserIdentityExtractor {
  ...
  @Inject
  public AccessTokenIdentityExtractor(TokenValidator tokenValidator,
  ...
  /**
   * Extracts a {@link UserIdentityPair} from an HTTP request by validating its {@link AccessToken}
   * in the Authorization header. The access token is an instance of {@link AccessToken} which is
   * base64-serialized.
   *
  ...
  public UserIdentityExtractionResponse extract(HttpRequest request)
    ...
    if (auth != null) {
      ...
      if (idx < 0) {
        return new UserIdentityExtractionResponse(
            UserIdentityExtractionState.ERROR_MISSING_CREDENTIAL,
    ...
    try {
      ...
      throw new UserIdentityExtractionException("Failed to decode access token due to codec error",
          e);
    }
  }
}
