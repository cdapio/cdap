...
public class MasterAuthenticationContext implements AuthenticationContext {
  ...
  public Principal getPrincipal() {
    ...
    System.out.println("wyzhang: " + this.getClass().getName() + " cur cred=" + userCredential);

    if (userCredential != null &&
      sConf.getBoolean(Constants.Security.Authentication.USER_CREDENTIAL_ENCRYPTION_ENABLED)) {
      try {
        String key = sConf.get(Constants.Security.Authentication.USER_CREDENTIAL_ENCRYPTION_KEY);
        userCredential = aesCipher.decrypt(key, userCredential);
        System.out.println("wyzhang: " + this.getClass().getName() + " decrypted cred=" + userCredential);
