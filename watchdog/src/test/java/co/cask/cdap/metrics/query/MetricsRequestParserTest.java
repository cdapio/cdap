...
public class MetricsRequestParserTest {
  ...
  public void testPathStrip() {
    String expected = "/cdap/apps/app1/flows/flow1/metric?aggregate=true";
  ...
  public void testQueryArgs() throws MetricsPathException {
    MetricsRequest request = MetricsRequestParser.parse(URI.create("/cdap/apps/app1/reads?count=60"));
    Assert.assertEquals(MetricsRequest.Type.TIME_SERIES, request.getType());
    Assert.assertEquals(60, request.getCount());

    request = MetricsRequestParser.parse(URI.create("/cdap/apps/app1/reads?summary=true"));
    Assert.assertEquals(MetricsRequest.Type.SUMMARY, request.getType());

    request = MetricsRequestParser.parse(URI.create("/cdap/apps/app1/reads?aggregate=true"));
    Assert.assertEquals(MetricsRequest.Type.AGGREGATE, request.getType());

    request = MetricsRequestParser.parse(URI.create("/cdap/apps/app1/reads?count=60&start=1&end=61"));
    Assert.assertEquals(1, request.getStartTime());
    Assert.assertEquals(61, request.getEndTime());
    Assert.assertEquals(MetricsRequest.Type.TIME_SERIES, request.getType());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=60&start=1&end=61"));
    Assert.assertEquals(1, request.getStartTime());
    Assert.assertEquals(61, request.getEndTime());
    Assert.assertEquals(MetricsRequest.Type.TIME_SERIES, request.getType());
    ...
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?start=1&end=61"));
    Assert.assertEquals(1, request.getStartTime());
    Assert.assertEquals(61, request.getEndTime());
    Assert.assertEquals(MetricsRequest.Type.TIME_SERIES, request.getType());
    Assert.assertNull(request.getInterpolator());
    ...
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=60&start=1&end=61&interpolate=step"));
    Assert.assertEquals(1, request.getStartTime());
    Assert.assertEquals(61, request.getEndTime());
    Assert.assertEquals(MetricsRequest.Type.TIME_SERIES, request.getType());
    ...
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=60&start=1&end=61&interpolate=linear"));
    Assert.assertEquals(1, request.getStartTime());
    Assert.assertEquals(61, request.getEndTime());
    Assert.assertEquals(MetricsRequest.Type.TIME_SERIES, request.getType());
  ...
  public void testRelativeTimeArgs() throws MetricsPathException  {
    ...
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&end=now-5s"));
    ...
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&start=now-65s"));
    assertTimestamp(now - 5, request.getEndTime());
    assertTimestamp(now - 65, request.getStartTime());

    now = TimeUnit.SECONDS.convert(System.currentTimeMillis(), TimeUnit.MILLISECONDS);
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&start=now-1m"));
    assertTimestamp(now, request.getEndTime());
    assertTimestamp(now - 60, request.getStartTime());

    now = TimeUnit.SECONDS.convert(System.currentTimeMillis(), TimeUnit.MILLISECONDS);
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&start=now-1h"));
    assertTimestamp(now - 3600 + 60, request.getEndTime());
    assertTimestamp(now - 3600, request.getStartTime());

    now = TimeUnit.SECONDS.convert(System.currentTimeMillis(), TimeUnit.MILLISECONDS);
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&start=now-1d"));
    assertTimestamp(now - 86400 + 60, request.getEndTime());
    assertTimestamp(now - 86400, request.getStartTime());

    now = TimeUnit.SECONDS.convert(System.currentTimeMillis(), TimeUnit.MILLISECONDS);
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&start=now-1m&end=now"));
    assertTimestamp(now, request.getEndTime());
    assertTimestamp(now - 60, request.getStartTime());

    now = TimeUnit.SECONDS.convert(System.currentTimeMillis(), TimeUnit.MILLISECONDS);
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/reads?count=61&start=now-2m%2B20s"));
  ...
  public void testScope() throws MetricsPathException {
    MetricsRequest request = MetricsRequestParser.parse(URI.create("/cdap/apps/app1/reads?summary=true"));
    Assert.assertEquals(MetricsScope.CDAP, request.getScope());
  ...
  public void testOverview() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(URI.create("/cdap/reads?aggregate=true"));
  ...
  public void testApps() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(URI.create("/cdap/apps/app1/reads?aggregate=true"));
  ...
  public void testFlow() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/flow1/flowlets/flowlet1/process.bytes?count=60&start=1&end=61"));
    ...
    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/flow1/some.metric?summary=true"));
    Assert.assertEquals("app1.f.flow1", request.getContextPrefix());
    Assert.assertEquals("some.metric", request.getMetricPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/loads?aggregate=true"));
  ...
  public void testQueues() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/flow1/flowlets/flowlet1/queues/queue1/process.bytes.in?aggregate=true"));
    Assert.assertEquals("app1.f.flow1.flowlet1", request.getContextPrefix());
    Assert.assertEquals("process.bytes.in", request.getMetricPrefix());
    Assert.assertEquals("queue1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/flow1/flowlets/flowlet1/queues/queue1/process.bytes.out?aggregate=true"));
    Assert.assertEquals("app1.f.flow1.flowlet1", request.getContextPrefix());
    Assert.assertEquals("process.bytes.out", request.getMetricPrefix());
    Assert.assertEquals("queue1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/flow1/flowlets/flowlet1/queues/queue1/process.events.in?aggregate=true"));
    Assert.assertEquals("app1.f.flow1.flowlet1", request.getContextPrefix());
    Assert.assertEquals("process.events.in", request.getMetricPrefix());
    Assert.assertEquals("queue1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/flows/flow1/flowlets/flowlet1/queues/queue1/process.events.out?aggregate=true"));
    Assert.assertEquals("app1.f.flow1.flowlet1", request.getContextPrefix());
  ...
  public void testMapReduce() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/mapreduce/mapred1/mappers/reads?summary=true"));
    Assert.assertEquals("app1.b.mapred1.m", request.getContextPrefix());
    Assert.assertEquals("reads", request.getMetricPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/mapreduce/mapred1/reducers/reads?summary=true"));
    Assert.assertEquals("app1.b.mapred1.r", request.getContextPrefix());
    Assert.assertEquals("reads", request.getMetricPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/mapreduce/mapred1/reads?summary=true"));
    Assert.assertEquals("app1.b.mapred1", request.getContextPrefix());
    Assert.assertEquals("reads", request.getMetricPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/mapreduce/reads?summary=true"));
  ...
  public void testProcedure() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/procedures/proc1/reads?summary=true"));
    Assert.assertEquals("app1.p.proc1", request.getContextPrefix());
    Assert.assertEquals("reads", request.getMetricPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/procedures/reads?summary=true"));
  ...
  public void testUserServices() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/services/serve1/reads?summary=true"));
    Assert.assertEquals("app1.s.serve1", request.getContextPrefix());
    Assert.assertEquals("reads", request.getMetricPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/services/serve1/runnables/run1/reads?summary=true"));
  ...
  public void testInvalidUserServices() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/service/serve1/reads?summary=true"));
  }

  @Test(expected = MetricsPathException.class)
  public void testInvalidUserServicesTooManyPath() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/apps/app1/services/serve1/runnables/run1/random/reads?summary=true"));
  }

  @Test
  public void testDataset() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/datasets/dataset1/apps/app1/flows/flow1/flowlets/flowlet1/store.reads?summary=true"));
    Assert.assertEquals("app1.f.flow1.flowlet1", request.getContextPrefix());
    Assert.assertEquals("store.reads", request.getMetricPrefix());
    Assert.assertEquals("dataset1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/datasets/dataset1/apps/app1/flows/flow1/store.reads?summary=true"));
    Assert.assertEquals("app1.f.flow1", request.getContextPrefix());
    Assert.assertEquals("store.reads", request.getMetricPrefix());
    Assert.assertEquals("dataset1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/datasets/dataset1/apps/app1/flows/store.reads?summary=true"));
    Assert.assertEquals("app1.f", request.getContextPrefix());
    Assert.assertEquals("store.reads", request.getMetricPrefix());
    Assert.assertEquals("dataset1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/datasets/dataset1/apps/app1/store.reads?summary=true"));
    Assert.assertEquals("app1", request.getContextPrefix());
    Assert.assertEquals("store.reads", request.getMetricPrefix());
    Assert.assertEquals("dataset1", request.getTagPrefix());

    request = MetricsRequestParser.parse(
      URI.create("/cdap/datasets/dataset1/store.reads?summary=true"));
    Assert.assertNull(request.getContextPrefix());
    Assert.assertEquals("store.reads", request.getMetricPrefix());
    Assert.assertEquals("dataset1", request.getTagPrefix());
  }
  ...
  public void testStream() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/streams/stream1/collect.events?summary=true"));
    Assert.assertNull(request.getContextPrefix());
  ...
  public void testService() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/services/appfabric/request.received?aggregate=true"));
  ...
  public void testHandler() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/services/appfabric/handlers/AppFabricHttpHandler/response.server-error?aggregate=true"));
  ...
  public void testMethod() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/services/metrics/handlers/MetricsQueryHandler/methods/handleComponent/" +
  ...
  public void testInvalidRequest() throws MetricsPathException {
    ...
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/services/metrics/handler/MetricsQueryHandler/" +
                   "response.successful?aggregate=true"));
  }
  ...
  public void testCluster() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/cluster/resources.total.storage?count=1&start=12345678&interpolate=step"));
  ...
  public void testTransactions() throws MetricsPathException  {
    MetricsRequest request = MetricsRequestParser.parse(
      URI.create("/cdap/transactions/invalid?count=1&start=12345678&interpolate=step"));
  ...
  public void testBadPathsThrowExceptions() {
    int numBad = 0;
    String[] validPaths = {
      "/cdap/metric?aggregate=true",
      "/cdap/apps/appX/metric?aggregate=true",
      "/cdap/apps/appX/flows/metric?aggregate=true",
      "/cdap/apps/appX/flows/flowY/metric?aggregate=true",
      "/cdap/apps/appX/flows/flowY/flowlets/flowletZ/metric?aggregate=true",
      "/cdap/apps/appX/procedures/metric?aggregate=true",
      "/cdap/apps/appX/procedures/procedureY/metric?aggregate=true",
      "/cdap/apps/appX/mapreduce/metric?aggregate=true",
      "/cdap/apps/appX/mapreduce/mapreduceY/metric?aggregate=true",
      "/cdap/apps/appX/mapreduce/mapreduceY/mappers/metric?aggregate=true",
      "/cdap/apps/appX/mapreduce/mapreduceY/reducers/metric?aggregate=true",
      "/cdap/datasets/datasetA/metric?aggregate=true",
      "/cdap/datasets/datasetA/apps/appX/metric?aggregate=true",
      "/cdap/datasets/datasetA/apps/appX/flows/flowY/metric?aggregate=true",
      "/cdap/datasets/datasetA/apps/appX/flows/flowY/flowlets/flowletZ/metric?aggregate=true",
      "/cdap/streams/streamA/metric?aggregate=true"
    };
    // check that miss-spelled paths and the like throw an exception.
    String[] invalidPaths = {
      "/reacto/metric?aggregate=true",
      "/cdap/app/appX/metric?aggregate=true",
      "/cdap/apps/appX/flow/metric?aggregate=true",
      "/cdap/apps/appX/flows/flowY/flowlet/flowletZ/metric?aggregate=true",
      "/cdap/apps/appX/procedure/metric?aggregate=true",
      "/cdap/apps/appX/procedure/procedureY/metric?aggregate=true",
      "/cdap/apps/appX/mapreduces/metric?aggregate=true",
      "/cdap/apps/appX/mapreduces/mapreduceY/metric?aggregate=true",
      "/cdap/apps/appX/mapreduce/mapreduceY/mapper/metric?aggregate=true",
      "/cdap/apps/appX/mapreduce/mapreduceY/reducer/metric?aggregate=true",
      "/cdap/dataset/datasetA/metric?aggregate=true",
      "/cdap/datasets/datasetA/app/appX/metric?aggregate=true",
      "/cdap/datasets/datasetA/apps/appX/flow/flowY/metric?aggregate=true",
      "/cdap/datasets/datasetA/apps/appX/flows/flowY/flowlet/flowletZ/metric?aggregate=true",
      "/cdap/stream/streamA/metric?aggregate=true"
    };
