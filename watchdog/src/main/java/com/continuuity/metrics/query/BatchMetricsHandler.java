...
public final class BatchMetricsHandler extends AbstractHttpHandler {
  ...
  private MetricsScanQuery createScanQuery(MetricsRequest request) {
    ...
    // if we're interpolating, expand the time window a little to allow interpolation
    ...
    if (interpolator != null) {
      ...
      // super big windows.  Only really matters for smaller windows anyway
      start -= Math.min(interpolator.getMaxAllowedGap(), 30);
      end += Math.min(interpolator.getMaxAllowedGap(), 30);
