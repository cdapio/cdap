...
public class WikipediaService extends AbstractService {
  ...
  public static final class WikipediaHandler extends AbstractHttpServiceHandler {
    ...
    @UseDataSet(WikipediaPipelineApp.SPARK_LDA_OUTPUT_DATASET)
    private Table ldaTable;
    ...
    /**
     * The {@link SparkWikipediaAnalyzer} program generates a list of topics for the input data. This API returns the
     * list of topics that were generated by the {@link SparkWikipediaAnalyzer} program.
     */
    @GET
    @Path("/lda/topics")
    public void getTopics(HttpServiceRequest request, HttpServiceResponder responder) {
      List<Integer> topics = new ArrayList<>();
      Scanner scanner = ldaTable.scan(null, null);
      Row row;
    ...
    public void getTopic(HttpServiceRequest request, HttpServiceResponder responder,
      ...
      Row row = ldaTable.get(Bytes.toBytes(topic));
      if (row.isEmpty()) {
