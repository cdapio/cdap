...
echo:
echo Options
echo     events    Number of events to send (min 1)
echo     delay     Number of seconds to sleep between each event-call (min 1; in integers)
...
for /L %%G IN (1,1,%$EVENTS%) DO call :CURL_CALL %%G
...
curl -H "Authorization: Bearer %$ACCESS_TOKEN%" -X POST -d"%$BODY%" "http://%$GATEWAY%:10000/v3/namespaces/default/streams/%$STREAM%"
