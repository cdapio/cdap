 ...
 * based upon the impressionId of the records. It then keeps track of how many clicks a particular view resulted in.
 */
public class ClicksAndViewsMapReduce extends AbstractMapReduce {
  protected static final String NAME = "ClicksAndViewsMapReduce";
   ...
   * It also keys all of the output records with the impressionId, so that a single reducer gets all of the records
   * for a particular impressionId.
   */
  public static class ImpressionKeyingMapper extends Mapper<LongWritable, Text, LongWritable, Text>
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
      ...
      context.write(new LongWritable(Long.valueOf(split[0])), new Text(TAB_JOINER.join(inputName, value.toString())));
    }
  }

  /**
   * Reducer class which looks at all the records for the impressionId.
  ...
  public static class JoiningReducer extends Reducer<LongWritable, Text, NullWritable, String> {
    ...
    public void reduce(LongWritable key, Iterable<Text> values,
      ...
      // the impressionId (which we key on from the mapper output) is already in the viewData
