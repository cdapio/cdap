...
public class StructuredRecordBuilderTest {
  ...
  @Test
  public void testFixedLogicalTypeWithNull() {
    Schema schema = Schema.recordOf(
      Schema.Field.of("name", Schema.of(Schema.Type.STRING)),
      Schema.Field.of("Fixed", Schema.nullableOf(Schema.fixedOf(4, "fixedName"))));
    StructuredRecord record = StructuredRecord.builder(schema)
      .set("Fixed", null)
      .set("name", "test").build();
    Assert.assertNull(record.get("Fixed"));
  }
  ...
  public void testFixedLogicalTypeWithByteBuffer() {
    ...
    StructuredRecord record = StructuredRecord.builder(schema)
      ...
      .set("name", "test").build();
    Assert.assertEquals(byteBuffer, record.get("Fixed"));
  }
  ...
  public void testFixedLogicalTypeWithByteArray() {
    ...
    StructuredRecord record = StructuredRecord.builder(schema)
      .set("Fixed", bytes)
      .set("name", "test").build();
    Assert.assertEquals(ByteBuffer.wrap(bytes), record.get("Fixed"));
  }
