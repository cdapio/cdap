...
public class ArtifactClientTestRun extends ClientTestBase {
    ...
    public void setUp() throws Throwable {
        ...
        for (ArtifactSummary artifactSummary : artifactClient.list(NamespaceId.DEFAULT)) {
            artifactClient
                    .delete(NamespaceId.DEFAULT.artifact(artifactSummary.getName(), artifactSummary.getVersion()));
        }
    }
    ...
    public void testNotFound() throws Exception {
        ...
        Set<ArtifactRange> parents = Sets.newHashSet(new ArtifactRange(ghostId.getParent().getNamespace(),
                ghostId.getArtifact(), new ArtifactVersion("1"), new ArtifactVersion("2")));
        try {
    ...
    public void testAddSelfExtendingThrowsBadRequest() throws Exception {
        try {
            artifactClient.add(NamespaceId.DEFAULT, "abc", DUMMY_SUPPLIER, "1.0.0",
                    Sets.newHashSet(new ArtifactRange(NamespaceId.DEFAULT.getNamespace(), "abc",
    ...
    public void testArtifacts() throws Exception {
        ...
        Set<ArtifactRange> parents = Sets.newHashSet(new ArtifactRange(myapp2Id.getParent().getNamespace(),
                myapp2Id.getArtifact(), new ArtifactVersion(myapp2Id.getVersion()), new ArtifactVersion("3.0.0")));
        Set<PluginClass> additionalPlugins = Sets.newHashSet(new PluginClass("jdbc", "mysql", "", null, null, null,
                "com.mysql.jdbc.Driver", null, Collections.<String, PluginPropertyField>emptyMap()));
        artifactClient.add(pluginId.getParent(), pluginId.getArtifact(), contentProvider, pluginId.getVersion(),
                parents, additionalPlugins);
        ...
        ArtifactClasses myAppClasses = ArtifactClasses.builder()
                .addApp(new ApplicationClass(MyApp.class.getName(), "", myAppConfigSchema)).build();

        // test get myapp-1.0.0
        ArtifactInfo myapp1Info = new ArtifactInfo(myapp1Id.getArtifact(), myapp1Id.getVersion(), ArtifactScope.USER,
                myAppClasses, myapp1Properties);
        Assert.assertEquals(myapp1Info, artifactClient.getArtifactInfo(myapp1Id));

        // test get myapp-2.0.0
        ArtifactInfo myapp2Info = new ArtifactInfo(myapp2Id.getArtifact(), myapp2Id.getVersion(), ArtifactScope.USER,
        ...
        Map<String, PluginPropertyField> props = ImmutableMap.of("x",
                new PluginPropertyField("x", "", "int", true, false));
        ArtifactClasses pluginClasses = ArtifactClasses.builder().addPlugin(new PluginClass("callable", "plugin1",
                "p1 description", null, null, null, Plugin1.class.getName(), "conf", props))
                .addPlugins(additionalPlugins).build();
        ArtifactInfo pluginArtifactInfo = new ArtifactInfo(pluginId.getArtifact(), pluginId.getVersion(),
                ArtifactScope.USER, pluginClasses, ImmutableMap.<String, String>of());
        ...
        Set<ApplicationClassSummary> appClassSummaries = Sets
                .newHashSet(artifactClient.getApplicationClasses(NamespaceId.DEFAULT));
        ...
        Set<ApplicationClassInfo> appClassInfos = Sets
                .newHashSet(artifactClient.getApplicationClasses(NamespaceId.DEFAULT, MyApp.class.getName()));
        ...
        // test get plugin types for myapp-1.0.0. should be empty, since plugins only
        // extends versions [2.0.0 - 3.0.0)
        ...
        PluginSummary pluginSummary = new PluginSummary("plugin1", "callable", "p1 description", null, null, null,
                Plugin1.class.getName(), pluginArtifactSummary);
