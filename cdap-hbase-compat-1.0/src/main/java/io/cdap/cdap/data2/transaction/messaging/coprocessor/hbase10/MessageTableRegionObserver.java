...
package io.cdap.cdap.data2.transaction.messaging.coprocessor.hbase10;
...
import io.cdap.cdap.data2.util.hbase.HBase10ScanBuilder;
 ...
 * generation. iii) Mark data entries with invalidated transactions (loaded from tx.snapshot) so
 * that invalid list can be pruned.
 */
public class MessageTableRegionObserver extends BaseRegionObserver {
  ...
  private static final class LoggingInternalScanner implements InternalScanner {
    ...
    public void close() throws IOException {
      if (state != null) {
        LOG.info("During " + operation + ", Filter " + filterName
            + " completed using tx.snapshot of timestamp "
            + state.getTimestamp() + " which had the visibility bound of "
            + state.getVisibilityUpperBound());
      }
      delegate.close();
    }
  }
  ...
  private static final class MessageDataFilter extends FilterBase {
    ...
    public ReturnCode filterKeyValue(Cell cell) throws IOException {
      ...
      long publishTimestamp = MessagingUtils.getPublishTimestamp(cell.getRowArray(), rowKeyOffset,
          sizeOfRowKey);
      int topicIdLength =
