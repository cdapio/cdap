...
/**
 * Builder for scheduling a program. The schedule must be triggered by a certain event and wait for
 * all constraints to be satisfied to launch the program. Build the schedule by setting the
 * schedule's trigger with methods {@link #triggerByTime(String)} or {@link
 * #triggerOnPartitions(String, int)}. To set the constraints for the schedule, use methods {@link
 * #withConcurrency(int)}, {@link #withDelay(long, TimeUnit)}, {@link #withTimeWindow(String,
 * String)}, {@link #withTimeWindow(String, String, TimeZone)}, and {@link
 * #withDurationSinceLastRun(long, TimeUnit)}. If no constraint is specified, the schedule will
 * immediately launch the program once the schedule is triggered.
 */
public interface ScheduleBuilder {
  ...
  /**
   * Sets a timeout on the schedule. Once a scheduled job is created and the specified number of
   * seconds has passed, the job will be removed without execute. For instance, if the timeout is
   * 21600 seconds (6 hours), and the configured constraints of the schedule are not met within 6
   * hours, then the job will be dropped, without execution.
   *
   ...
   * @return {@link ConstraintProgramScheduleBuilder} containing the given max number of concurrent
   *     runs
   */
   ...
   * program.
   *
   * @param delay delay in the given <tt>unit</tt> to wait after the schedule is triggered
   *     before launching the program
   * @param unit the time unit of the <tt>delay</tt> argument
   * @return {@link ScheduleBuilder} containing the given delay. Note that the delay constraint does
   *     not have the option to abort the schedule if the constraint is not met.
   */
  ScheduleBuilder withDelay(long delay, TimeUnit unit);

  /**
   * Set a time range in a day starting from {@code startTime} and ending at {@code endTime},
   * between which the program is allowed to be launched. {@code endTime} must be later than {@code
   * startTime}. JVM's default time zone will be used to interpret {@code startTime} and {@code
   * endTime}.
   *
   * @param startTime the start time (inclusive) in the format of "HH:mm", for instance 2am
   *     should be "02:00"
   * @param endTime the end time (exclusive) in the format of "HH:mm", for instance 5pm should
   *     be "17:00"
  ...
  /**
   * Set a time range in a day starting from {@code startTime} and ending at {@code endTime} in the
   ...
   * program again.
   *
   * @param duration duration in the given <tt>unit</tt> to wait after the last launch of the
   *     program before launching the program
   ...
   * @param numPartitions the minimum number of new partitions added to the dataset to trigger
   *     the schedule
   * @return a {@link ScheduleCreationSpec}
   */
   ...
   * @return a {@link ScheduleCreationSpec}
   */
  ScheduleCreationSpec triggerOnPartitions(String datasetNamespace, String datasetName,
      int numPartitions);
   ...
   * @param programStatuses the set of statuses to trigger the schedule. The schedule will be
   *     triggered if the status of the specific program transitioned to one of these statuses.
   * @return a {@link ScheduleCreationSpec}
   */
   ...
   *
   * @param trigger the {@link Trigger} to be satisfied to trigger the schedule. {@link Trigger}
   *     can be created by calling methods from {@link TriggerFactory} obatined from {@link
   *     AbstractApplication#getTriggerFactory()}
   * @return a {@link ScheduleCreationSpec}
   */
