 ...
 * Tests Netty Router.
 */
public class SecureNettyRouterTest {
  private static final Logger LOG = LoggerFactory.getLogger(SecureNettyRouterTest.class);
  ...
  public void testRouterAsync() throws Exception {
    ...
    for (int i = 0; i < numElements; ++i) {
      ...
      final Request request = new RequestBuilder("GET")
        .setUrl(String.format("https://%s:%d%s/%s-%d",
  ...
  public void testHostForward() throws Exception {
    // Test defaultService
    HttpResponse response = get(String.format("https://%s:%d%s/%s",
    ...
    // Test webappService
    response = get(String.format("https://%s:%d%s/%s",
  ...
  public void testUpload() throws Exception {
    ...
    final Request request = new RequestBuilder("POST")
      .setUrl(String.format("https://%s:%d%s", HOSTNAME, ROUTER.getServiceMap().get(DEFAULT_SERVICE), "/v1/upload"))
  ...
  private void testSync(int numRequests) throws Exception {
    for (int i = 0; i < numRequests; ++i) {
      ...
      HttpResponse response = get(String.format("https://%s:%d%s/%s-%d",
                                                HOSTNAME, ROUTER.getServiceMap().get(DEFAULT_SERVICE),
                                                "/v1/ping", "sync", i));
      Assert.assertEquals(HttpResponseStatus.OK.getCode(), response.getStatusLine().getStatusCode());
    }
...
// apache HttpClient version >4.2 should use BasicClientConnectionManager
  ...
  public static class ServerResource extends ExternalResource {
    ...
    protected void before() throws Throwable {
      ...
      builder.addHttpHandlers(ImmutableSet.<HttpHandler>of(new ServerHandler()));
