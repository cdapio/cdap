...
public class MetricsQueryTest extends MetricsSuiteTestBase {
  ...
  public void testQueueLength() throws Exception {
    ...
    MetricsCollector enqueueCollector = collectionService.getCollector(MetricsScope.CDAP,
                                                                       "WordCount.f.WordCounter.counter", "0");
    enqueueCollector.increment("process.events.out", 10, queueName.getSimpleName());

    // Insert ack metrics
    MetricsCollector ackCollector = collectionService.getCollector(MetricsScope.CDAP,
    ...
    MetricsCollector streamCollector = collectionService.getCollector(MetricsScope.CDAP,
    ...
    post.setEntity(new StringEntity(
      "[\"/cdap/apps/WordCount/flows/WordCounter/flowlets/unique/process.events.pending?aggregate=true\"]"));
  ...
  public void testingSystemMetrics() throws Exception {
    ...
    MetricsCollector collector = collectionService.getCollector(MetricsScope.CDAP,
    ...
    String methodRequest =
      "/cdap/services/appfabric/handlers/AppFabricHttpHandler/methods/getAllApps/" +
        "request.received?aggregate=true";
    String handlerRequest =
      "/cdap/services/appfabric/handlers/AppFabricHttpHandler/request.received?aggregate=true";
    String serviceRequest =
      "/cdap/services/appfabric/request.received?aggregate=true";
  ...
  public void testingInvalidSystemMetrics() throws Exception {
    ...
    String methodRequest =
      "/cdap/services/appfabrics/handlers/AppFabricHttpHandler/methods/getAllApps/" +
        "request.received?aggregate=true";
  ...
  public void testingTransactoinMetrics() throws Exception {
    ...
    MetricsCollector collector = collectionService.getCollector(MetricsScope.CDAP, "transactions", "0");
    collector.increment("inprogress", 1);

    // Wait for collection to happen
    TimeUnit.SECONDS.sleep(2);

    String request = "/cdap/transactions/inprogress?aggregate=true";
  ...
  public void testGetMetric() throws Exception {
    ...
    collector = collectionService.getCollector(MetricsScope.CDAP, "-.cluster", "0");
