...
public final class Loggers {
  ...
  /**
   * Returns the closest parent {@link Logger} of the given {@link Logger} that is already defined in the
   * {@link LoggerContext} associated with the {@link Logger}.
   *
   * @return the closet parent {@link Logger} or {@code null} if the given {@link Logger} is the root logger.
   */
  @Nullable
  public static Logger getParentLogger(Logger logger) {
    LoggerContext context = logger.getLoggerContext();
    if (logger == context.getLogger(Logger.ROOT_LOGGER_NAME)) {
      return null;
    }

    Logger parent = context.getLogger(Logger.ROOT_LOGGER_NAME);
    String loggerName = logger.getName();
    int idx = LoggerNameUtil.getFirstSeparatorIndexOf(loggerName);
    while (idx >= 0) {
      String parentName = loggerName.substring(0, idx);
      if (parentName.equals(loggerName)) {
        break;
      }

      Logger newParent = context.exists(parentName);
      if (newParent != null) {
        parent = newParent;
      }
