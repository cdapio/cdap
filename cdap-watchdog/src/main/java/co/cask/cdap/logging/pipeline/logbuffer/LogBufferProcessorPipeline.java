...
public class LogBufferProcessorPipeline extends AbstractExecutionThreadService {
  ...
  private final CheckpointManager<LogBufferCheckpoint> checkpointManager;
  private final MetricsContext metricsContext;
  private final TimeEventQueueProcessor<FileOffset> eventQueueProcessor;
  private final LogBufferReader reader;
  private final LoggingEventSerializer serializer;
  private MutableLogBufferCheckpoint checkpoint;
  ...
  public LogBufferProcessorPipeline(LogProcessorPipelineContext context,
                                    CheckpointManager<LogBufferCheckpoint> checkpointManager, LogBufferReader reader) {
  ...
  protected void run() throws Exception {
    ...
    try {
      ...
      while (!stopped) {
        List<LogBufferEvent> events = reader.read(offset, BATCH_SIZE);
   ...
   */
  private boolean processMessages(List<LogBufferEvent> logEvents) {
    boolean processed = false;
    for (LogBufferEvent logEvent : logEvents) {
      ...
      if (checkpoint == null) {
        checkpoint = new MutableLogBufferCheckpoint(smallestOffset.getFileId(), smallestOffset.getFilePos(),
   ...
   */
  private static final class MutableLogBufferCheckpoint extends LogBufferCheckpoint {
    private String fileId;
    private long filePos;
    private long maxEventTs;

    MutableLogBufferCheckpoint(String fileId, long filePos, long maxEventTs) {
    ...
    MutableLogBufferCheckpoint setFileId(String fileId) {
      this.fileId = fileId;
      return this;
    }

    MutableLogBufferCheckpoint setFilePos(long filePos) {
      this.filePos = filePos;
      return this;
    }

    MutableLogBufferCheckpoint setMaxEventTs(long maxEventTs) {
      this.maxEventTs = maxEventTs;
      return this;
    }
