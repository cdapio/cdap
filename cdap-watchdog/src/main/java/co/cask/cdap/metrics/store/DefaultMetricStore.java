...
public class DefaultMetricStore implements MetricStore {
  ...
  public Map<String, MetricProcessorStat> getMetricsProcessorStats() throws Exception {
    ...
    Map<String, MetricProcessorStat> processMap = new HashMap<>();
    for (TopicId topicId : metricsTopics) {
      ...
      // todo maybe add messageId to watchdog-api?
      String messageId = null;
      if (metricProcessorStat.getMessageId() != null) {
        messageId = new MessageId(metricProcessorStat.getMessageId()).toString();
      }
      processMap.put(topicId.getTopic(), new MetricProcessorStat(messageId == null ? "null" : messageId,
