...
public class LevelDBFilterableOVCTableTest {
  ...
  public void testTimeseriesQuery() throws OperationException {
    ...
    for (int i = -2; i < secondsToQuery + 2; i++) {
      records.add(new MetricsRecord(context, "0", "reads", tags, ts + i, i, MetricContentType.COUNT));
    }
    ...
    records.add(new MetricsRecord("app1.p.procedure1", "0", "reads", tags, ts, 5, MetricContentType.COUNT));
    records.add(new MetricsRecord("app1.f.flow2.flowlet1", "0", "reads", tags, ts, 10, MetricContentType.COUNT));

    // this one should get returned
    records.add(new MetricsRecord("app1.f.flow1.flowlet2", "0", "reads", tags, ts, 15, MetricContentType.COUNT));
