...
public class FileMetadataManagerTest {
  ...
  public void testScanFiles() throws Exception {
    ...
    Map<URI, NamespaceId> filesToRemove = new HashMap<>();
...
//    Assert.assertEquals(collectedMetaFiles.size(), fileMetaDataManager.cleanMetadata(filesToRemove));
  ...
  private void listToMap(List<URI> collectedMetaFiles, String namespace, Map<URI, NamespaceId> filesToRemove) {
    for (URI uri : collectedMetaFiles) {
      filesToRemove.put(uri, new NamespaceId(namespace));
    }
  }
