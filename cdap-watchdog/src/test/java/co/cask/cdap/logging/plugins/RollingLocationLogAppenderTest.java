...
public class RollingLocationLogAppenderTest {
  ...
  public void testRollingLocationLogAppender() throws Exception {
    ...
    ingestLogs(logger);
    Map<LocationIdentifier, Location> activeFilesToLocation = rollingAppender.activeFilesToLocation;
    Assert.assertEquals(1, activeFilesToLocation.size());
    verifyFileOutput(activeFilesToLocation);

    // different program should go to different directory
    addTagsToMdc("testNs", "testApp1");
    ingestLogs(logger);
    activeFilesToLocation = rollingAppender.activeFilesToLocation;
  ...
  private void ingestLogs(Logger logger) {
    for (int i = 0; i < 5; i++) {
      logger.info("Testing Application log: " + i);
    }
  }

  private void verifyFileOutput(Map<LocationIdentifier, Location> activeFilesToLocation) throws IOException {
