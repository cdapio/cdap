...
public class LogBufferWriterTest {
  ...
  public void testLogBufferWriter() throws Exception {
    ...
    ImmutableList<ILoggingEvent> events = getLoggingEvents();
    Iterator<LogBufferEvent> writtenEvents = writer.write(events.iterator());
    writer.close();
  ...
  public void testWritesOnClosedWriter() throws IOException {
    ...
    writer.write(ImmutableList.of(createLoggingEvent("test.logger", Level.INFO, "0", 1,
  ...
  private ImmutableList<ILoggingEvent> getLoggingEvents() {
    ...
    return ImmutableList.of(
      createLoggingEvent("test.logger", Level.INFO, "0", now - 1000, loggingContext),
      createLoggingEvent("test.logger", Level.INFO, "1", now - 900, loggingContext),
      createLoggingEvent("test.logger", Level.INFO, "2", now - 700, loggingContext),
      createLoggingEvent("test.logger", Level.DEBUG, "3", now - 600, loggingContext),
      createLoggingEvent("test.logger", Level.INFO, "4", now - 500, loggingContext),
      createLoggingEvent("test.logger", Level.INFO, "5", now - 100, loggingContext));
  }
