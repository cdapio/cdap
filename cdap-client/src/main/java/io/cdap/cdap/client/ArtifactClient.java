...
public class ArtifactClient {
  ...
  private static final Type ARTIFACT_SUMMARIES_TYPE = new TypeToken<List<ArtifactSummary>>() {
  }.getType();
   ...
   *
   * @param namespace the namespace to list artifacts in
   * @param scope the scope of the artifacts to get. If {@code null}, both user and system
  ...
  public List<ArtifactSummary> list(NamespaceId namespace, @Nullable ArtifactScope scope)
    ...
    return ObjectResponse.<List<ArtifactSummary>>fromJsonBody(response, ARTIFACT_SUMMARIES_TYPE)
        .getResponseObject();
  }
   ...
   * @param namespace the namespace to list artifact versions in
   * @param artifactName the name of the artifact
   * @param scope the scope of artifacts to get. If none is given, the scope defaults to the
   *     user scope
   * @return list of {@link ArtifactSummary}
   * @throws IOException if a network error occurred
   * @throws UnauthenticatedException if the request is not authorized successfully in the
  ...
  public List<ArtifactSummary> listVersions(NamespaceId namespace, String artifactName,
    ...
    URL url = scope == null ? config.resolveNamespacedURLV3(namespace,
        String.format("artifacts/%s", artifactName)) :
  ...
  /**
   * Get summaries of all application classes in the given namespace, including classes from system
   * artifacts.
   *
   * @param namespace the namespace to list application classes from
   * @return summaries of all application classes in the given namespace, including classes from
   *     system artifacts
   * @throws IOException if a network error occurred
   * @throws UnauthenticatedException if the request is not authorized successfully in the
   *     gateway server
  ...
  /**
   * Get summaries of all application classes in the given namespace, optionally including classes
   * from system artifacts.
   *
   * @param namespace the namespace to list application classes from
   * @param scope the scope to list application classes in. If null, classes from all scopes are
   *     returned
   * @return summaries of all application classes in the given namespace, including classes from
   *     system artifacts
   * @throws IOException if a network error occurred
  ...
  public List<ApplicationClassSummary> getApplicationClasses(NamespaceId namespace,
    ...
    String path =
        scope == null ? "classes/apps" : String.format("classes/apps?scope=%s", scope.name());
  ...
  /**
   * Get information about all application classes in the specified namespace, of the specified
   * class name.
   *
   * @param namespace the namespace to list application classes from
   * @return summaries of all application classes in the given namespace, including classes from
  ...
  public List<PluginSummary> getPluginSummaries(ArtifactId artifactId, String pluginType,
    ...
    return ObjectResponse.<List<PluginSummary>>fromJsonBody(response, PLUGIN_SUMMARIES_TYPE)
        .getResponseObject();
  }
   ...
   * @throws NotFoundException if the given artifact does not exist or plugins for that artifact
   *     do not exist
   * @throws IOException if a network error occurred
   * @throws UnauthenticatedException if the request is not authorized successfully in the
   *     gateway server
  ...
  public List<PluginInfo> getPluginInfo(ArtifactId artifactId, String pluginType, String pluginName,
    ...
    return ObjectResponse.<List<PluginInfo>>fromJsonBody(response, PLUGIN_INFOS_TYPE)
        .getResponseObject();
  }
   ...
   * @throws BadRequestException if the request is invalid. For example, if the artifact name or
   *     version is invalid
   ...
   * @param artifactVersion the version of the artifact to add. If null, the version will be
   ...
   *     derived from the manifest of the artifact
   * @param parentArtifacts the set of artifacts this artifact extends
   * @param additionalPlugins the set of plugins contained in the artifact that cannot be
   *     determined through jar inspection. This set should include any classes that are plugins but
   *     could not be annotated as such. For example, 3rd party classes like jdbc drivers fall into
   *     this category.
   * @throws ArtifactAlreadyExistsException if the artifact already exists
   * @throws BadRequestException if the request is invalid. For example, if the artifact name or
   *     version is invalid
  ...
  /**
   * Write a property for an artifact. If the property already exists, it will be overwritten. If
   * the property does not exist, it will be added.
   *
