...
public class ProgramClient {
  ...
  private static final Gson GSON = new GsonBuilder()
      ...
      .create();
  private static final Type BATCH_STATUS_RESPONSE_TYPE = new TypeToken<List<BatchProgramStatus>>() {
  }.getType();
  private static final Type BATCH_RESULTS_TYPE = new TypeToken<List<BatchProgramResult>>() {
  }.getType();
   ...
   */
  public void start(ProgramId program, boolean debug, @Nullable Map<String, String> runtimeArgs)
      throws IOException,
      ProgramNotFoundException, UnauthenticatedException, UnauthorizedException {
    String action = debug ? "debug" : "start";
    String path = String.format("apps/%s/versions/%s/%s/%s/%s", program.getApplication(),
        program.getVersion(),
   ...
   * @param startTimeSeconds the lower bound of when programs were stopped to restart
   *     (inclusive)
   * @param endTimeSeconds the upper bound of when programs were stopped to restart (exclusive)
   * @throws IOException if a network error occurred
   * @throws UnauthenticatedException if the request is not authenticated successfully in the
   *     gateway server
  ...
  public void restart(ApplicationId applicationId, long startTimeSeconds, long endTimeSeconds)
    ...
    String path = String.format(
        "apps/%s/versions/%s/restart-programs?start-time-seconds=%d&end-time-seconds=%d",
  ...
  public void stop(ProgramId programId)
    ...
    String path = String.format("apps/%s/versions/%s/%s/%s/stop", programId.getApplication(),
   ...
   */
  public void stopAll(NamespaceId namespace, Set<ProgramType> programTypesToSkip)
    ...
    for (ApplicationRecord applicationRecord : allApps) {
      ...
      for (ProgramRecord programRecord : programRecords) {
        try {
          if (programTypesToSkip.contains(programRecord.getType())) {
  ...
  public String getStatus(ProgramId programId)
    ...
    String path = String.format("apps/%s/versions/%s/%s/%s/status", programId.getApplication(),
        programId.getVersion(),
        programId.getType().getCategoryName(), programId.getProgram());
    URL url = config.resolveNamespacedURLV3(programId.getNamespaceId(), path);
  ...
  public List<BatchProgramStatus> getStatus(NamespaceId namespace, List<BatchProgram> programs)
    ...
    return ObjectResponse.<List<BatchProgramStatus>>fromJsonBody(response,
            BATCH_STATUS_RESPONSE_TYPE, GSON)
        .getResponseObject();
  }
   ...
   * @throws UnauthenticatedException if the request is not authorized successfully in the
   *     gateway server
   * @throws TimeoutException if the program did not achieve the desired program status before
   *     the timeout
   * @throws InterruptedException if interrupted while waiting for the desired program status
   */
  public void waitForStatus(final ProgramId program, ProgramStatus status, long timeout,
    ...
    try {
      Tasks.waitFor(status.name(), new Callable<String>() {
          ...
          String.format("Timeout occurred. ProgramId: %s. Expected %s but found %s.",
  ...
  /**
   * Gets the live information of a program. In distributed CDAP, this will contain IDs of the YARN
   * applications that are running the program.
   *
   * @param program the program
  ...
  public DistributedProgramLiveInfo getLiveInfo(ProgramId program)
    ...
    return ObjectResponse.fromJsonBody(response, DistributedProgramLiveInfo.class)
        .getResponseObject();
  }
  ...
  public int getWorkerInstances(ProgramId worker) throws IOException, NotFoundException,
    ...
    return ObjectResponse.fromJsonBody(response, Instances.class).getResponseObject()
        .getInstances();
  }
  ...
  public void setWorkerInstances(ProgramId worker, int instances)
    ...
    URL url = config.resolveNamespacedURLV3(worker.getNamespaceId(),
        String.format("apps/%s/workers/%s/instances",
            worker.getApplication(), worker.getProgram()));
    HttpRequest request = HttpRequest.put(url).withBody(GSON.toJson(new Instances(instances)))
        .build();
   ...
   *     gateway server
   */
  public List<RunRecord> getAllProgramRuns(ProgramId program, long startTime, long endTime,
      int limit)
      throws IOException, NotFoundException, UnauthenticatedException, UnauthorizedException {
