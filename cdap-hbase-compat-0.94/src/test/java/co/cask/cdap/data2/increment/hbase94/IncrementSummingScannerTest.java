...
public class IncrementSummingScannerTest {
  ...
  public void testMultiColumnFlushAndCompact() throws Exception {
    ...
    try {
      ...
      long ts = now;
      for (int i = 0; i < 50; i++) {
        doPut(region, generateIncrementPut(familyBytes, columnBytes, row1, ts));
        doPut(region, generateIncrementPut(familyBytes, columnBytes, row2, ts));
        doPut(region, generateIncrementPut(familyBytes, columnBytes2, row1, ts));
        ts++;
      }
      ...
      RegionScanner scanner =
        new IncrementSummingScanner(region, -1, region.getScanner(new Scan().setMaxVersions()),
                                    ScanType.MINOR_COMPACT, now + 15, -1);
