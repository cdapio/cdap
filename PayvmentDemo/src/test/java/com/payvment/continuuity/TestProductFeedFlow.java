...
public class TestProductFeedFlow extends PayvmentBaseFlowTest {
  ...
  public void testProductFeedFlow() throws Exception {
    ...
    // Start the flow
    TestFlowHandle flowHandle = startFlow(ProductFeedFlow.class);
    ...
    ProductFeedEntry productMeta = new ProductFeedEntry(product_id, store_id, now, country, category, name, 3.5);
    ...
    ProductFeedEntry readProductMeta = productTable.readObject(Bytes.toBytes(product_id));
    assertEqual(productMeta, readProductMeta);

    // Verify the product update count has been incremented
    assertEquals(new Long(1), productUpdateCountTable.readSingleKey(Bytes.toBytes(product_id)));

    // Verify the product total score has increased to 1
    assertEquals(new Long(1), allTimeScoreTable.readSingleKey(Bytes.toBytes(product_id)));

    // Verify the hourly score has been incremented to type score increase
    List<Counter> counters = topScoreTable.readTopCounters(Bytes.add(Bytes.toBytes(Helpers.hour(now)),
  ...
  public void testProductMetaSerialization() throws Exception {
    ...
    String exampleProductMetaJson = "{\"@id\":\"6709879\"," +
      "\"country\":[\"US\",\"UK\"]," +
      "\"category\":\"Dresses\"," +
    ...
    String processedJsonString = ProductFeedParserFlowlet.preProcessSocialActionJSON(exampleProductMetaJson);

    Gson gson = new Gson();
    ProductFeedEntry productMeta = gson.fromJson(processedJsonString, ProductFeedEntry.class);
    ...
    processedJsonString = ProductFeedParserFlowlet.preProcessSocialActionJSON(exampleProductMetaJson);
    ProductFeedEntry productMeta2 = gson.fromJson(processedJsonString, ProductFeedEntry.class);
