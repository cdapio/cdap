...
public final class SparkContainerLauncher {
  ...
  private static <T extends Collection<URL>> T addSystemClassPath(T urls) throws IOException {
    for (String path : System.getProperty("java.class.path").split(File.pathSeparator)) {
      if (path.endsWith("*")) {
        ...
        if (jarFiles != null) {
          for (File f : jarFiles) {
            urls.add(f.toPath().toRealPath().toUri().toURL());
          }
        }
      } else {
  ...
  public static void launch(String mainClassName, String[] args, boolean removeMainClass,
    ...
    if (systemClassLoader instanceof URLClassLoader) {
      urls = ClassLoaders.getClassLoaderURLs(systemClassLoader, new LinkedHashSet<>());
    } else {
      urls = addSystemClassPath(new LinkedHashSet<>());
    }
    ...
    if (removeMainClass) {
      ...
      if (urls.remove(urlByClass)) {
        LOG.info("Removed url {}", urlByClass);
      } else {
    ...
    try {
      ...
      try {
        ...
        URL resource = classLoader.getResource(mainClassName.replace('.', '/') + ".class");
        log(logger, "info", "Main class from URL {}", resource);
      ...
      log(logger, "error", "Exception raised when calling {}.main(String[]) method", mainClassName,
          t);
      throw t;
    } finally {
      if (sparkRuntimeContext instanceof Closeable) {
