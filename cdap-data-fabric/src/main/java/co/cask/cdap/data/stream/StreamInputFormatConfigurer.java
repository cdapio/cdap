 ...
 */
public class StreamInputFormatConfigurer {
  ...
  /**
   * Tries to set the {@link StreamInputFormatConfigurer#DECODER_TYPE} depending upon the supplied value class
   *
   * @param conf   the conf to modify
  ...
  public static <T> List<T> getSplits(Configuration conf, long currentTime, StreamInputSplitFactory<T> splitFactory)
    throws IOException, InterruptedException {
    long ttl = conf.getLong(STREAM_TTL, Long.MAX_VALUE);
    long endTime = conf.getLong(EVENT_END_TIME, Long.MAX_VALUE);
    long startTime = Math.max(conf.getLong(EVENT_START_TIME, 0L), currentTime - ttl);
   ...
   */
  private static Collection<StreamDataFileSplitter> collectBuckets(FileSystem fs, Path partitionPath)
    throws IOException {
