...
public class SyslogParser {
   ...
   */
  public Map<String, String> parseMessage(String msg, Charset charset) {
    ...
    // Remember priority
    headers.put(SYSLOG_PRIORITY, priority);

    // put fac / sev into header
    headers.put(SYSLOG_FACILITY, String.valueOf(facility));
    headers.put(SYSLOG_SEVERITY, String.valueOf(severity));
    ...
    if (msgLen > curPos + 2 && "1 ".equals(msg.substring(curPos, curPos + 2))) {
      ...
      headers.put(SYSLOG_VERSION, version);
      curPos += 2;
    }
  ...
  protected long parseRfc5424Date(String msg) {
    ...
    ts = timeParser.parseMillis(timestampPrefix);
    ...
    if (msg.charAt(curPos) == '.') {
      ...
      // ODO: ensure we handle all bad formatting cases
      while (!foundEnd) {
