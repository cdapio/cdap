  ...
  public Row next() {
    ...
    try {
      ...
      while (true) {
        ...
        if (rowMap.size() > 0) {
          byte[] row = rowKeyDistributor == null ? result.getRow() : rowKeyDistributor.getOriginalKey(result.getRow());
          return new co.cask.cdap.api.dataset.table.Result(row, rowMap);
        }
      }
