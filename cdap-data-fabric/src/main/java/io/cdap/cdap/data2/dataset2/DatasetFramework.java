 ...
 * <tt>
 * DatasetFramework datasetFramework = ...; datasetFramework.addModule("myDatasets",
 * MyDatasetModule.class); datasetFramework.addInstance("myTable", "table",
 * DatasetProperties.EMPTY); TableAdmin admin = datasetFramework.getAdmin("myTable");
 * admin.create(); Table table = datasetFramework.getDataset("myTable"); try { table.write("key",
 * "value"); } finally { table.close(); }
 * </tt>
 */
...
public interface DatasetFramework {
  ...
  /**
   * Adds dataset types by adding dataset module to the system. Calling this method to add {@link
   * DatasetModule} may result in tracing class dependencies if the {@link DatasetModule} is not a
   * system dataset, which can take couple seconds for the tracing. If the jar {@link Location}
   * containing the {@link DatasetModule} is known, it's better to call {@link
   * #addModule(DatasetModuleId, DatasetModule, Location)} instead.
   *
   * @param moduleId dataset module id
   * @param module dataset module
   * @throws ModuleConflictException when module with same name is already registered or this
   *     module registers a type with a same name as one of the already registered by another module
   *     types
  ...
  /**
   * Adds dataset types by adding dataset module to the system with a jar location containing all
   * dataset classes needed by the module.
   *
   * @param moduleId dataset module id
   * @param module dataset module
   * @param jarLocation location of the jar file that contains the dataset classes needed by the
   *     module
   * @throws ModuleConflictException when module with same name is already registered or this
   *     module registers a type with a same name as one of the already registered by another module
   *     types
   ...
   *
   * @param moduleId dataset module id
   * @throws ModuleConflictException when module cannot be deleted because of its dependant
   ...
   * @throws ModuleConflictException when some of modules can't be deleted because of its
   *     dependant modules or instances
   * @throws ServiceUnavailableException when the dataset service is not running
   */
   ...
   *
   * This uses {@link DatasetDefinition#configure(String, DatasetProperties)} method to build {@link
   * DatasetSpecification} which describes dataset instance and later used to initialize {@link
   * DatasetAdmin} and {@link Dataset} for the dataset instance.
   *
   ...
   * DatasetAdmin} and {@link Dataset} for the dataset instance.
   *
   * Note this method can only be used when app impersonation is enabled, since it will assume the
   * keytab url is in the default keytab path.
   *
   * @param datasetTypeName dataset instance type name
   * @param datasetInstanceId dataset instance name
   * @param props dataset instance properties
   ...
   *
   * This uses {@link Reconfigurable#reconfigure}, or if that method is not implemented by the
   * dataset type, {@link DatasetDefinition#configure}, to build a new {@link DatasetSpecification}.
   * If the {@link DatasetAdmin} implements {@link Updatable}, it is called to update the dataset
   * instance.
   *
   * @param datasetInstanceId dataset instance name
   * @param props dataset instance properties
   ...
   * @return a collection of {@link DatasetSpecification}s for all datasets in the specified
   *     namespace
   */
   ...
   * @return a collection of {@link DatasetSpecification}s for all datasets in the specified
   *     namespace having specified properties
   */
  Collection<DatasetSpecificationSummary> getInstances(NamespaceId namespaceId,
   ...
   *
   * @param datasetInstanceId the {@link DatasetId} for which the {@link DatasetSpecification}
   *     is desired
   ...
   *
   * @param datasetInstanceId dataset instance name
   * @throws InstanceConflictException if dataset instance cannot be deleted because of its
   *     dependencies
   * @throws InstanceNotFoundException if dataset instance does not exist
  ...
  /**
   * Gets dataset instance admin to be used to perform administrative operations. The given
   * classloader must be able to load all classes needed to instantiate the dataset admin. This
   * means if the system classloader is used, only system dataset admins can fetched.
   *
   * @param <T> dataset admin type
   * @param datasetInstanceId dataset instance name
   * @param classLoader classLoader to be used to load classes or {@code null} to use system
   *     classLoader
   * @return instance of dataset admin or {@code null} if dataset instance of this name doesn't
   *     exist.
  ...
  /**
   * Gets dataset instance admin to be used to perform administrative operations. The class loader
   * provider is used get classloaders for any dataset modules used by the specified dataset admin.
   * This is because the classloader(s) for a dataset admin may create some resources that need to
   * be cleaned up on close.
   *
   * @param <T> dataset admin type
   * @param datasetInstanceId dataset instance name
   * @param classLoader parent classLoader to be used to load classes or {@code null} to use
   *     system classLoader
   ...
   * @throws DatasetManagementException when there's trouble getting dataset meta info
   * @throws IOException when there's trouble to instantiate {@link
   *     io.cdap.cdap.api.dataset.Dataset}
   * @throws ServiceUnavailableException when the dataset service is not running
   */
  @Nullable
  default <T extends Dataset> T getDataset(DatasetId datasetInstanceId,
  ...
  /**
   * Gets dataset to be used to perform data operations. This one is used when the classloader(s)
   * for a dataset may create some resources that need to be cleaned up on close, and an access type
   * is specified.
   *
   * @param <T> dataset type to be returned
   * @param datasetInstanceId dataset instance id
