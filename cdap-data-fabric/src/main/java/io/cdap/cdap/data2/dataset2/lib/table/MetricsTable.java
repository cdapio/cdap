...
public interface MetricsTable extends Dataset {
  ...
  /**
   * Atomically compare a single column of a row with an expected value, and if it matches, replace
   * it with a new value.
   *
   * @param oldValue the expected value of the column. If null, this means that the column must
   *     not exist.
   * @param newValue the new value of the column. If null, the effect to delete the column if
   *     the comparison succeeds.
   * @return whether the write happened, that is, whether the existing value of the column matched
   *     the expected value.
   */
  boolean swap(byte[] row, byte[] column, byte[] oldValue, byte[] newValue);

  /**
   * Atomic increment of multiple columns of a row. If a column does not exist prior to the
   * increment, its value is assumed zero. Increments are only guaranteed atomic with respect to
   * other increments.
   *
   ...
   * @param row row to delete from
   * @param columns names of columns to delete
   * @param fullRow if the whole row is deleted. Note that this parameter is just an
   *     optimization and list of columns must still be passed in case when this optimization is not
   *     implemented
   ...
   *
   * @param start the row key of the first row to scan. If null, the scan begins at the first
   *     row of the table.
   * @param stop the row key of the last row to scan. If null, the scan goes to the last row of
   *     the table.
