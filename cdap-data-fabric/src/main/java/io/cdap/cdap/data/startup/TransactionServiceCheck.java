...
public class TransactionServiceCheck extends Check {
  ...
  public void run() throws Exception {
    ...
    int defaultTxTimeout = getConfiguredInteger(TxConstants.Manager.CFG_TX_TIMEOUT,
        "Default transaction timeout");
    int maxTxTimeout = getConfiguredInteger(TxConstants.Manager.CFG_TX_MAX_TIMEOUT,
    ...
    if (defaultTxTimeout > maxTxTimeout) {
      throw new IllegalArgumentException(String.format(
          ...
          TxConstants.Manager.CFG_TX_TIMEOUT, defaultTxTimeout,
          TxConstants.Manager.CFG_TX_MAX_TIMEOUT, maxTxTimeout));
    }
  ...
  private int getConfiguredInteger(String propertyName, String description) {
    try {
      ...
      throw new IllegalArgumentException(
          String.format("%s (%s) is not configured.", description, propertyName));
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(
          String.format("%s (%s) is not a number.", description, propertyName), e);
    }
  }
