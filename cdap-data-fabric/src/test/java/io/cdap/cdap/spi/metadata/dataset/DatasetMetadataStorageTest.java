...
public class DatasetMetadataStorageTest extends MetadataStorageTest {
  ...
  public static void teardown() throws IOException {
    ...
    try {
      ...
      // Ignored because we are performing resource cleanup in teardown.
    }
  }
  ...
  public void testSearchWeight() throws IOException {
    ...
    Map<String, String> userProps = ImmutableMap
        .of("key1", "value1", "key2", "value2", "multiword", multiWordValue);
    ...
    Map<String, String> dataset2UserProps = ImmutableMap
        .of("sKey1", "sValue1 sValue2", "Key1", "Value1");
    Set<String> sysTags = ImmutableSet.of("sysTag1");

    MetadataRecord service1Record = new MetadataRecord(
        service1,
        union(new Metadata(USER, userTags, userProps), new Metadata(SYSTEM, sysTags, systemProps)));
    ...
    MetadataRecord dataset1Record = new MetadataRecord(dataset1,
        new Metadata(USER, tags(), dataset1UserProps));
    MetadataRecord dataset2Record = new MetadataRecord(dataset2,
        new Metadata(USER, tags(), dataset2UserProps));
    ...
    assertInOrder(mds,
        SearchRequest.of("value1 sValue*").addNamespace(ns).setLimit(Integer.MAX_VALUE).build(),
  ...
  public void testCrossNamespacePagination() throws IOException {
    ...
    SearchResponse response =
        assertResults(mds,
            SearchRequest.of("*").setLimit(Integer.MAX_VALUE).setCursorRequested(true).build(),
    ...
    MetadataRecord[] results = {
        resultIter.next(), resultIter.next(), resultIter.next(), resultIter.next(),
        resultIter.next()};

    // get 4 results (guaranteed to have at least one from each namespace), offset 1
    assertResults(mds,
        SearchRequest.of("*").setCursorRequested(true).setOffset(1).setLimit(4).build(),
        results[1], results[2], results[3], results[4]);

    // get the first four
    assertResults(mds,
        SearchRequest.of("*").setCursorRequested(true).setOffset(0).setLimit(4).build(),
        results[0], results[1], results[2], results[3]);

    // get middle 3
    assertResults(mds,
        SearchRequest.of("*").setCursorRequested(true).setOffset(1).setLimit(3).build(),
        results[1], results[2], results[3], results[3]);

    // clean up
    mds.batch(ImmutableList.of(
        new Drop(ns1app1), new Drop(ns1app2), new Drop(ns1app3), new Drop(ns2app1),
        new Drop(ns2app2)),
        MutationOptions.DEFAULT);
  }
  ...
  public void testNsScopes() {
    ...
    // system only
    testNsScopes(ImmutableSet.of("system"), NamespaceId.SYSTEM, EnumSet.of(EntityScope.SYSTEM),
        false);
    // user namespace only
    testNsScopes(ImmutableSet.of("myns"), new NamespaceId("myns"), EnumSet.of(EntityScope.USER),
        false);
    // user and system namespace
    testNsScopes(ImmutableSet.of("myns", "system"), new NamespaceId("myns"),
        EnumSet.allOf(EntityScope.class), false);
  ...
  private void testNsScopes(Set<String> namespaces,
    ...
    ImmutablePair<NamespaceId, Set<EntityScope>> pair = DatasetMetadataStorage
        .determineNamespaceAndScopes(namespaces);
    Assert.assertEquals("namespace does not match for " + namespaces, expectedNamespace,
        pair.getFirst());
