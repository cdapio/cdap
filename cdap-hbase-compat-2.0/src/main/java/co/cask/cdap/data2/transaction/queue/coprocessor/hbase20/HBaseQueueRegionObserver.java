 ...
 */
public final class HBaseQueueRegionObserver implements RegionCoprocessor, RegionObserver {
  ...
  public void start(CoprocessorEnvironment env) {
    if (env instanceof RegionCoprocessorEnvironment) {
      TableDescriptor tableDesc = ((RegionCoprocessorEnvironment) env).getRegion().getTableDescriptor();
      String hTableName = tableDesc.getTableName().getNameAsString();
      ...
      namespaceId = from(tableDesc).getNamespace();
  ...
  private final class EvictionInternalScanner implements InternalScanner {
	...
	private static TableId fromHBaseTableName(String namespace, String qualifier) {
		...
		if (NamespaceId.DEFAULT.getEntityName().equals(namespace)) {
			...
			// strip 'user.' from the beginning of table name since we prepend it in
			// getBackwardCompatibleTableName
	...
	public static TableId from(TableDescriptor htd) {
