...
package io.cdap.cdap.data2.increment.hbase20;
 ...
 */
public class IncrementHandler implements Coprocessor, RegionObserver {
  ...
  public void start(CoprocessorEnvironment e) throws IOException {
    if (e instanceof RegionCoprocessorEnvironment) {
      ...
      this.state = new IncrementHandlerState(env, (HTableDescriptor) tableDesc);
      for (HColumnDescriptor columnDesc : ((HTableDescriptor) tableDesc).getFamilies()) {
        state.initFamily(columnDesc.getName(), columnDesc.getValues().entrySet().stream().collect(Collectors.toMap(
          entry -> entry.getKey().get(), entry -> entry.getValue().get())));
      }
    }
  }
  ...
  public InternalScanner preFlush(ObserverContext<RegionCoprocessorEnvironment> c, Store store,
    ...
    byte[] family = Bytes.toBytes(store.getColumnFamilyName());
