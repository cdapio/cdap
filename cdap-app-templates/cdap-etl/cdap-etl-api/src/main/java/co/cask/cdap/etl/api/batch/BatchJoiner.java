 ...
 * with supported join key and value classes. Join keys and values can be a
 ...
 * If the join value is not one of those types and is being used in mapreduce,
 * it must implement Hadoop's org.apache.hadoop.io.Writable interface.
 * If the joiner is being used in spark, both the join key and value must implement the
 * {@link java.io.Serializable} interface.
 *
 *
 * @param <JOIN_KEY> join key type. Must be a supported type
 * @param <JOIN_VALUE> join value type. Must be a supported type
...
public abstract class BatchJoiner<JOIN_KEY, JOIN_VALUE, OUT> extends BatchConfigurable<BatchJoinerContext>
   ...
   * before any calls to {@link #joinOn(String, Object)} and {@link #merge(Object, List, Emitter)} are made.
   *
