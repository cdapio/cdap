...
/**
 * Batch Sink forms the last stage of a Batch ETL Pipeline. In addition to configuring the Batch
 * run, it also transforms a single input object into a key value pair that the Batch run will
 * output. By default, the input object is used as both the key and value.
 *
 * {@link BatchSink#initialize}, {@link BatchSink#transform} and {@link BatchSink#destroy} methods
 * are called inside the Batch Run while {@link BatchSink#prepareRun} and {@link
 * BatchSink#onRunFinish} methods are called on the client side, which launches the Batch run,
...
public abstract class BatchSink<IN, KEY_OUT, VAL_OUT> extends BatchConfigurable<BatchSinkContext>
  ...
  /**
   * Initialize the Batch Sink stage. Executed inside the Batch Run. This method is guaranteed to be
   * invoked before any calls to {@link BatchSink#transform} are made.
   *
   ...
   * consumed by the output, as set in {@link BatchSink#prepareRun}. By default, the input object is
   * used as both key and value. This method is invoked inside the Batch run.
   *
