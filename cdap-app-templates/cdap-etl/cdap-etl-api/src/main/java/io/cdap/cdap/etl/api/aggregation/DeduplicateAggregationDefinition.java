...
/**
 * Specifies how a deduplicate operation should be executed.
 * A deduplication is an operation that takes in a set of rows and for each group of rows for which the values for
 * certain group by fields are the same, outputs a single row.
 * This single row can be determined by performing an operation on the set of rows which returns a single row.
 * For instance, to select the record with the latest <code>creation_time</code>, the filter expression used would be
 * (<code>creation_time</code>, <code>MAX</code>).
 * Currently the two supported operations are returning the row with the MIN or MAX value of a particular expression.
 * If the first filter expression in the list returns more than one row, the next filter expression will be used as
 * a tie breaker, and so on until the list of filter expressions is exhausted.
 */
public class DeduplicateAggregationDefinition extends AggregationDefinition {
  ...
  /**
   * An enumeration that specifies the function to be executed on the filter expression for each group of rows
  ...
  public static class FilterExpression {
    ...
    public boolean equals(Object o) {
      ...
      return Objects.equals(getExpression(), that.getExpression()) && getFilterFunction() == that.getFilterFunction();
    }
  ...
  public static class Builder {
    ...
    /**
     * Sets the list of filter expressions to the specified list of filter functions and filter expressions.
    ...
    public DeduplicateAggregationDefinition build() {
      if (selectExpressions.isEmpty()) {
        throw new IllegalStateException("Can't build a DeduplicateAggregationDefinition without select fields");
      } else if (groupByExpressions.isEmpty()) {
        throw new IllegalStateException("Can't build a DeduplicateAggregationDefinition without fields to dedup on");
      }

      return new DeduplicateAggregationDefinition(groupByExpressions, selectExpressions, filterExpressions);
    }
  }
}
