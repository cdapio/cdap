...
/**
 * Test for {@link RecordWritableConverter}.
 */
public class RecordWritableConverterTest {
  private Schema schema;

  @Test
  public void checkConversion() throws Exception {
    StructuredRecord initial = getStructuredRecord();
    MapWritable mapWritable = RecordWritableConverter.covertToWritable(initial);
    StructuredRecord recordOfMapWritable = RecordWritableConverter.convertToRecord(mapWritable, schema);
    assertRecordsEqual(initial, recordOfMapWritable);
  }
  ...
  private void assertRecordsEqual(StructuredRecord one, StructuredRecord two) {
    ...
    for (Schema.Field field : one.getSchema().getFields()) {
      if (one.get(field.getName()) == null) {
        Assert.assertNull(two.get(field.getName()));
      } else if (one.get(field.getName()).getClass().equals(StructuredRecord.class)) {
  ...
  private StructuredRecord getStructuredRecord() {
    ...
    Schema.Field nameField = Schema.Field.of("name", Schema.nullableOf(Schema.of(Schema.Type.STRING)));
