...
public final class ExternalDatasets {
  ...
  /**
   * If the input is an external source then an external dataset is created for tracking purpose and
   * returned. If the input is a regular dataset or a stream then it is already trackable, hence
   * same input is returned.
   *
   * @param admin {@link Admin} used to create external dataset
   * @param input input to be tracked
   * @return an external dataset if input is an external source, otherwise the same input that is
      ...
      if (!admin.datasetExists(inputName)) {
        ...
        if (!EXTERNAL_DATASET_TYPE.equals(datasetType)) {
          throw new IllegalArgumentException(
              "An external source cannot have the same name as an existing CDAP Dataset instance "
                  + inputName);
        }
      }
  ...
  /**
   * If the output is an external sink then an external dataset is created for tracking purpose and
   * returned. If the output is a regular dataset then it is already trackable, hence same output is
   * returned.
   *
   * @param admin {@link Admin} used to create external dataset
   * @param output output to be tracked
   * @return an external dataset if output is an external sink, otherwise the same output is
   *     returned
   */
  public static Output makeTrackable(Admin admin, Output output) {
    ...
    try {
      ...
      if (!admin.datasetExists(outputName)) {
        ...
        if (!EXTERNAL_DATASET_TYPE.equals(datasetType)) {
          throw new IllegalArgumentException(
              "An external sink cannot have the same name as an existing CDAP Dataset instance "
                  + outputName);
        }
      }
  ...
  public static void registerLineage(Admin admin, String referenceName,
    ...
    if (schema == null) {
      ...
      datasetProperties = DatasetProperties.of(
  ...
  private static void invokeMethod(String referenceName, Dataset ds,
      Class<? extends Dataset> dsClass,
