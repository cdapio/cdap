...
/**
 * This is run at application configure time to take an application config {@link ETLConfig} and
 * call {@link PipelineConfigurable#configurePipeline(PipelineConfigurer)} on all plugins in the
 * pipeline. This generates a {@link PipelineSpec} which the programs understand.
 *
...
public abstract class PipelineSpecGenerator<C extends ETLConfig, P extends PipelineSpec> {
  ...
  private static final Set<String> VALID_ERROR_INPUTS = ImmutableSet.of(
      BatchSource.PLUGIN_TYPE, Transform.PLUGIN_TYPE, BatchAggregator.PLUGIN_TYPE,
  ...
  protected <T extends PluginConfigurer & DatasetConfigurer> PipelineSpecGenerator(
      String namespace, T configurer, @Nullable RuntimeConfigurer runtimeConfigurer,
  ...
  /**
   * Validate the user provided ETL config and generate a pipeline specification from it. It will
  ...
  /**
   * Performs most of the validation and configuration needed by a pipeline. Handles stages,
   * connections, resources, and stage logging settings.
   *
   * @param config user provided ETL config
  ...
  protected void configureStages(ETLConfig config, PipelineSpec.Builder specBuilder)
    ...
    for (ETLStage stage : traversalOrder) {
      ...
      pluginConfigurers.put(stageName,
          new DefaultPipelineConfigurer(pluginConfigurer, datasetConfigurer,
              stageName, engine,
              new DefaultStageConfigurer(stageName),
              featureFlagsProvider
          ));
    }
    SchemaPropagator schemaPropagator = new SchemaPropagator(pluginConfigurers,
        validatedPipeline::getOutputs,
        pluginTypes::get);
    ...
    for (String propertyName : propertiesFromStages.rowKeySet()) {
      ...
      if (propertyValues.size() > 1) {
        ...
        for (Map.Entry<String, String> valueEntry : propertyValues.entrySet()) {
          ...
          errMsg.append(" stage '").append(fromStage).append("' = '").append(propertyValue)
              .append("',");
        }
  ...
  protected ConfiguredStage configureStage(ETLStage stage, ValidatedPipeline validatedPipeline,
    ...
    if (pluginType.equals(SplitterTransform.PLUGIN_TYPE)) {
      ...
      for (Map.Entry<String, String> outputEntry : validatedPipeline.getOutputPorts(stageName)
          .entrySet()) {
        String outputStage = outputEntry.getKey();
        String outputPort = outputEntry.getValue();
        if (outputPort == null) {
          throw new IllegalArgumentException(
              String.format("Connection from Splitter '%s' to '%s' must specify a port.",
  ...
  public StageSpec.Builder configureStage(String stageName, ETLPlugin etlPlugin,
    ...
    try {
      ...
      collector.addFailure(
              String.format("Null error occurred while configuring the stage %s.", stageName), null)
      ...
      collector.addFailure(
          String.format("Index out of bounds error occurred while configuring the stage %s.",
  ...
  protected void configureSourcePlugin(String stageName, Object plugin,
      DefaultStageConfigurer stageConfigurer,
      FailureCollector collector) {
    //no-op
  }

  protected void validateJoinCondition(String stageName, JoinCondition condition,
      FailureCollector collector) {
  ...
  private void configureAutoJoiner(String stageName, AutoJoiner autoJoiner,
      DefaultStageConfigurer stageConfigurer,
      FailureCollector collector) {
    ...
    if (!missingInputs.isEmpty()) {
      collector.addFailure(
          String.format("Joiner stage '%s' did not include input stage %s in the join.",
    ...
    if (!extraInputs.isEmpty()) {
      collector.addFailure(
          String.format("Joiner stage '%s' is trying to join stage %s, which is not an input.",
              stageName, String.join(", ", missingInputs)),
          "Check with the plugin developer to make sure it is implemented correctly.");
    }
  }
   ...
   */
  private Object getPlugin(String stageName, ETLPlugin etlPlugin,
      TrackedPluginSelector pluginSelector, String type,
    ...
    try {
      ...
      if (runtimeEvaluator != null) {
        pluginProperties = pluginConfigurer.evaluateMacros(etlPlugin.getProperties(),
            runtimeEvaluator, options);
      }
      ...
      for (String missingProperty : e.getMissingProperties()) {
        collector.addFailure(String.format("Required property '%s' has no value.", missingProperty),
                null)
    ...
    if (plugin == null) {
      ...
      if (suggestion != null) {
        suggestedArtifactId = new ArtifactId(suggestion.getName(),
  ...
  protected ValidatedPipeline validateConfig(ETLConfig config) {
    ...
    for (Connection connection : config.getConnections()) {
      if (!stageNames.contains(connection.getFrom())) {
        throw new IllegalArgumentException(
            String.format("Invalid connection %s. %s is not a stage.", connection,
                connection.getFrom()));
      }
    ...
    for (ETLStage stage : config.getStages()) {
      ...
      if (isSource) {
        ...
        if (nonControlParents.size()
            > 1) { // the stage's nonControlParents should only contain itself
          throw new IllegalArgumentException(
              String.format(
                  "%s %s is invalid. %s stages can only be placed at the start of the pipeline.",
      ...
      if (!isAction && !stageType.equals(Condition.PLUGIN_TYPE) && !isSource
          && stageInputs.isEmpty()) {
        throw new IllegalArgumentException(
    ...
    for (String actionStage : actionStages) {
      ...
      if (!nonControlChildren.isEmpty() && !nonControlParents.isEmpty()) {
        throw new IllegalArgumentException(
            String.format(
                "Action stage '%s' is invalid. Actions can only be placed at the start or end of the pipeline.",
                actionStage));
      }
    }
  ...
  private boolean isAction(String pluginType) {
    return Action.PLUGIN_TYPE.equals(pluginType) || Constants.SPARK_PROGRAM_PLUGIN_TYPE.equals(
        pluginType);
  }
   ...
   * @throws IllegalArgumentException if there is an error in the configurations, for example
   *     condition stage is not configured correctly to have 'true' and 'false' branches or there
   *     are multiple incoming connections on the stages which are on condition branches
   */
  ...
  /**
   * Current stage can only have either one incoming connection if its one of the stage on the
   * condition branch or two incoming connections if it is condition stopper stage. We do not allow
   * cross connections from sources.
   */
