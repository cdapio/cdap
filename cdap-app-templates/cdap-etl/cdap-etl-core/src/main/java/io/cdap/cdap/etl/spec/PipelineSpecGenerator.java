...
public abstract class PipelineSpecGenerator<C extends ETLConfig, P extends PipelineSpec> {
  ...
  public StageSpec.Builder configureStage(String stageName, ETLPlugin etlPlugin,
    ...
    try {
      ...
      stageConfigurer.addFailure(e.getMessage(),
        ...
        .withPluginConfigCause(e.getProperty());
    } catch (InvalidStageException e) {
      if (e.getReasons().isEmpty()) {
        stageConfigurer.addFailure(e.getMessage(),
                                   String.format("Provide valid configurations to the stage '%s'.", stageName));
      }
      ...
      for (InvalidStageException reason : e.getReasons()) {
        if (reason instanceof InvalidConfigPropertyException) {
          ...
          stageConfigurer.addFailure(configException.getMessage(),
            ...
            .withPluginConfigCause(configException.getProperty());
        } else {
          stageConfigurer.addFailure(reason.getMessage(),
                                     String.format("Provide valid configurations to the stage '%s'.", stageName));
        }
      }
      ...
      stageConfigurer.addFailure(getNpeErrorMessage(stageName, e),
                                 "Make sure nullable configurations are handled by the plugin.");
    } catch (Exception e) {
      stageConfigurer.addFailure(String.format("Error while configuring stage '%s': '%s'.", stageName,
                                               e.getMessage()), null);
    }
  ...
  private Object getPlugin(String stageName, ETLPlugin etlPlugin, PluginSelector pluginSelector, String type,
    ...
    try {
      ...
      stageConfigurer.addFailure(e.getMessage(), String.format("Provide unique plugin id for plugin '%s'.", stageName));
    } catch (InvalidPluginConfigException e) {
      stageConfigurer.addFailure(e.getMessage(), "Provide valid plugin config properties.");
    }
    ...
    if (plugin == null) {
      ...
      String correctiveAction = String.format("Make sure plugin '%s' of type '%s' is already deployed.",
                                              pluginName, type);
      stageConfigurer.addFailure(errorMessage, correctiveAction).withPluginNotFoundCause(stageName, pluginName, type);
  ...
  private String getNpeErrorMessage(String stageName, NullPointerException e) {
    ...
    return String.format("Null error occurred: stage - '%s', classname - '%s', method - '%s', line - '%s'.",
                         stageName, simpleClassName, methodName, line);
  }
