 ...
 * The key is of the form '__cs|original connector name|input stage name.
...
public class SchemaPropagator {
  ...
  private final Function<String, String> connectorOriginalNameProvider;
  ...
  public SchemaPropagator(Map<String, DefaultPipelineConfigurer> pluginConfigurers,
                          ...
                          Function<String, String> connectorOriginalNameProvider,
    ...
    this.connectorOriginalNameProvider = connectorOriginalNameProvider;
  }
  ...
  public void propagateSchema(StageSpec stageSpec) {
    ...
    if (Constants.Connector.PLUGIN_TYPE.equals(stageType) && nextStages.isEmpty()) {
      writeInputSchemas(connectorOriginalNameProvider.apply(stageName), stageSpec.getInputSchemas());
      return;
    }
    ...
    for (String nextStageName : nextStages) {
      ...
      for (Schema nextStageInputSchema : nextStageInputSchemas.values()) {
        if (!BatchJoiner.PLUGIN_TYPE.equals(nextStageType)
          ...
          && !Constants.Connector.PLUGIN_TYPE.equals(nextStageType)
  ...
  private Map<String, Schema> getNextStageInputSchema(StageSpec currentStageSpec, String nextStageName,
    ...
    if (isConnectorSource(currentStageSpec)) {
      ...
      return readInputSchemas(connectorOriginalNameProvider.apply(currentStageSpec.getName()));
      ...
      String portName = nextStageName;
      if (Constants.Connector.PLUGIN_TYPE.equals(nextStageType)) {
        portName = connectorOriginalNameProvider.apply(nextStageName);
      }
      ...
      StageSpec.Port portSpec = currentStageSpec.getOutputPorts().get(portName);
  ...
  private Map<String, Schema> readInputSchemas(String stageName) {
    ...
    for (Map.Entry<String, List<NodeValue>> entry : workflowToken.getAll().entrySet()) {
      ...
      String inputStageName = key.substring(sepIndex + 1);
