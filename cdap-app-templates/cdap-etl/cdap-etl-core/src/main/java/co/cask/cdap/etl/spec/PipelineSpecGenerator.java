...
/**
 * This is run at application configure time to take an application config
 * {@link ETLConfig} and call
 * {@link PipelineConfigurable#configurePipeline(PipelineConfigurer)} on all
 * plugins in the pipeline. This generates a {@link PipelineSpec} which the
 * programs understand.
 *
 * @param <C>
 *            the type of user provided config
 * @param <P>
 *            the pipeline specification generated from the config
 * @param <T>
 *            the type of the platform configurer
 */
public abstract class PipelineSpecGenerator<C extends ETLConfig, P extends PipelineSpec, T extends 
        PluginConfigurer & DatasetConfigurer> {
    private static final Set<String> VALID_ERROR_INPUTS = ImmutableSet.of(BatchSource.PLUGIN_TYPE,
            Transform.PLUGIN_TYPE, BatchAggregator.PLUGIN_TYPE, ErrorTransform.PLUGIN_TYPE);
    protected final T configurer;
    protected final Engine engine;
    private final Set<String> sourcePluginTypes;
    private final Set<String> sinkPluginTypes;

    protected PipelineSpecGenerator(T configurer, Set<String> sourcePluginTypes, Set<String> sinkPluginTypes,
            Engine engine) {
    ...
    /**
     * Validate the user provided ETL config and generate a pipeline specification
     * from it. It will also register all plugins used by the pipeline and create
     * any error datasets used by the pipeline.
     *
     * A valid pipeline has the following properties:
     *
     * All stages in the pipeline have a unique name. Source stages have at least
     * one output and no inputs. Sink stages have at least one input and no outputs.
     * There are no cycles in the pipeline. All inputs into a stage have the same
     * schema.
     *
     * @param config
     *            user provided ETL config
     */
     ...
     *            user provided ETL config
     * @param specBuilder
     *            builder for creating a pipeline spec.
     */
    protected void configureStages(ETLConfig config, PipelineSpec.Builder specBuilder) {
        // validate the config and determine the order we should configure the stages
        // in.
        ...
        // row = property name, column = property value, val = stage that set the
        // property
        // this is used so that we can error with a nice message about which stages are
        // setting conflicting properties
        ...
        for (ETLStage stage : traversalOrder) {
            ...
            for (String nextStageName : validatedPipeline.getOutputs(stageName)) {
                ...
                // if the output stage is an error transform, it takes the error schema of this
                // stage as its input.
                // if the current stage is a splitter transform, it takes the output schema of
                ...
                if (ErrorTransform.PLUGIN_TYPE.equals(nextStageType)) {
                    ...
                    if (portSpec == null) {
                        ...
                        // Should not happen since it should have been validated earlier, but check here
                        // just in case
                        throw new IllegalArgumentException(String.format(
                                "Must specify a port when connecting Splitter '%s' to '%s'", stageName, nextStageName));
                    } else {
                ...
                // if (BatchJoiner.PLUGIN_TYPE.equals(nextStageType) && nextStageInputSchema ==
                // null) {
                // throw new IllegalArgumentException(String.format("Joiner cannot have any null
                // input schemas, but stage %s " +
                // "outputs a null schema.", stageName));
                // }

                // Do not allow more than one input schema for stages other than Joiner and
                // Action
        ...
        for (String propertyName : propertiesFromStages.rowKeySet()) {
            // go through all values set for the property name. If there is more than one,
            // we have a conflict.
            Map<String, String> propertyValues = propertiesFromStages.row(propertyName);
            if (propertyValues.size() > 1) {
                StringBuilder errMsg = new StringBuilder("Pipeline property '").append(propertyName)
        ...
        specBuilder.addConnections(config.getConnections()).setResources(config.getResources())
                .setDriverResources(config.getDriverResources()).setClientResources(config.getClientResources())
                .setStageLoggingEnabled(config.isStageLoggingEnabled())
                .setNumOfRecordsPreview(config.getNumOfRecordsPreview()).setProperties(pipelineProperties).build();
    }
     ...
     *
     * @param stage
     *            the user provided configuration for the stage
     * @param validatedPipeline
     *            the validated pipeline config
     * @param pluginConfigurer
     *            configurer used to configure the stage
    ...
    private ConfiguredStage configureStage(ETLStage stage, ValidatedPipeline validatedPipeline,
        ...
        if (pluginSpec.getType().equals(SplitterTransform.PLUGIN_TYPE)) {
            ...
            for (Map.Entry<String, String> outputEntry : validatedPipeline.getOutputPorts(stageName).entrySet()) {
                ...
                if (outputPort == null) {
                    throw new IllegalArgumentException(String.format(
                            "Connection from Splitter '%s' to '%s' must specify a port.", stageName, outputStage));
                }
            ...
            // conditions handled specially since they can have an action and a transform as
            // input, where they might have
            // different input schemas. picking the first non-null schema, or null if
            // they're all null
            // this isn't perfect, as we should really be checking all non-action input
            // schemas and validating that they're
            // all the same
            if (Condition.PLUGIN_TYPE.equals(pluginSpec.getType())) {
                outputSchema = null;
                for (Schema schema : inputSchemas.values()) {
                    if (schema != null) {
                        // this check isn't perfect, as we should still error if 2 transforms are
                        // inputs,
                        ...
                        if (outputSchema != null && !outputSchema.equals(schema)) {
                            throw new IllegalArgumentException(
                                    "Cannot have different input schemas going into stage " + stageName);
                        }
                        outputSchema = schema;
                    }
                }
        ...
        StageSpec stageSpec = StageSpec.builder(stageName, pluginSpec).addInputSchemas(inputSchemas)
                .addOutputPortSchemas(outputSchemas).setErrorSchema(stageConfigurer.getErrorSchema())
                .setProcessTimingEnabled(validatedPipeline.isProcessTimingEnabled())
                .setStageLoggingEnabled(validatedPipeline.isStageLoggingEnabled()).build();
     ...
     *            default pipeline configurere to configure the plugin
    ...
    protected PluginSpec configurePlugin(String pluginId, ETLPlugin etlPlugin,
        ...
        Object plugin = configurer.usePlugin(etlPlugin.getType(), etlPlugin.getName(), pluginId,
                etlPlugin.getPluginProperties(), pluginSelector);

        if (plugin == null) {
            throw new IllegalArgumentException(
                    String.format("No plugin of type %s and name %s could be found for stage %s.", etlPlugin.getType(),
                            etlPlugin.getName(), pluginId));
        }
        try {
        ...
        return new PluginSpec(etlPlugin.getType(), etlPlugin.getName(), etlPlugin.getProperties(),
                pluginSelector.getSelectedArtifact());
    }

    /**
     * Validate that this is a valid pipeline. A valid pipeline has the following
     * properties:
     *
     * All stages in the pipeline have a unique name. Source stages have at least
     * one output and no inputs. Sink stages have at least one input and no outputs.
     * There are no cycles in the pipeline. All inputs into a stage have the same
     * schema. ErrorTransforms only have BatchSource, Transform, or BatchAggregator
     * as input stages. AlertPublishers have at least one input and no outputs and
     * don't have SparkSink or BatchSink as input. Action stages can only be at the
     * start or end of the pipeline. Condition stages have at most 2 outputs. Each
     * stage on a condition's output branch has at most a single input.
     *
     * Returns the stages in the order they should be configured to ensure that all
     * input stages are configured before their output.
     *
     * @param config
    ...
    private ValidatedPipeline validateConfig(ETLConfig config) {
        ...
        for (ETLStage stage : config.getStages()) {
            if (!stageNames.add(stage.getName())) {
                throw new IllegalArgumentException(String.format(
                        "Invalid pipeline. Multiple stages are named %s. Please ensure all stage names are unique",
                        stage.getName()));
            }
        ...
        // also check that conditions have at most 2 outgoing connections each label
        // with true or
        // false but not both
        ...
        for (Connection connection : config.getConnections()) {
            ...
            if (conditionStages.contains(connection.getFrom())) {
                ...
                // check if connection from the condition node is marked as true or false
                // multiple times
                if (conditionBranch.containsKey(connection.getFrom())
                    ...
                    String msg = String.format(
                            "For condition stage '%s', more than one outgoing connections are marked as %s.",
        ...
        if (config.getConnections().isEmpty()) {
            if (actionStages.size() == 1 && stageNames.size() == 1) {
                ...
                throw new IllegalArgumentException("Invalid pipeline. There are no connections between stages. "
        ...
        for (ETLStage stage : config.getStages()) {
            ...
            if (isSource) {
                if (!stageInputs.isEmpty() && !controlStages.containsAll(stageInputs)) {
                    throw new IllegalArgumentException(String.format(
                            "%s %s has incoming connections from %s. %s stages cannot have any incoming connections.",
                            stageType, stageName, stageType, Joiner.on(',').join(stageInputs)));
                }
            } else if (isSink) {
                if (!stageOutputs.isEmpty() && !controlStages.containsAll(stageOutputs)) {
                    throw new IllegalArgumentException(String.format(
                            "%s %s has outgoing connections to %s. %s stages cannot have any outgoing connections.",
                ...
                for (String inputStage : stageInputs) {
                    ...
                    if (!VALID_ERROR_INPUTS.contains(inputType)) {
                        throw new IllegalArgumentException(String.format(
                                "ErrorTransform %s cannot have stage %s of type %s as input. Only %s stages can emit "
                                + "errors.", stageName, inputStage, inputType, Joiner.on(',')
                                .join(VALID_ERROR_INPUTS)));
                    }
                }
            }
    ...
    /**
     * Make sure that the stages on the condition branches do not have more than one
     * incoming connections
     * 
     * @param conditionStages
     *            the set of condition stages in the pipeline
     * @param dag
     *            the dag of connections
     * @throws IllegalArgumentException
     *             if there is an error in the configurations, for example condition
     *             stage is not configured correctly to have 'true' and 'false'
     *             branches or there are multiple incoming connections on the stages
     *             which are on condition branches
     */
        ...
        for (String conditionStage : conditionStages) {
            ...
            if (outputs == null || outputs.size() > 2) {
                String msg = String.format("Condition stage in the pipeline '%s' should have at least 1 and at max 2 "
                        + "outgoing connections corresponding to 'true' and 'false' branches " + "but found '%s'.",
                        conditionStage, outputs == null ? 0 : outputs.size());
                throw new IllegalArgumentException(msg);
            }
            for (String output : outputs) {
                // Check that each stage in this branch starting from output has only one
                // incoming connection
    ...
    /**
     * Current stage can only have either one incoming connection if its one of the
     * stage on the condition branch or two incoming connections if it is condition
     * stopper stage. We do not allow cross connections from sources.
     */
            ...
            if (parents.size() > 0) {
                ...
                String msg = String.format(
                        "Stage in the pipeline '%s' is on the branch of condition '%s'. However it also "
                                + "has following incoming paths: '%s', which is not supported.",
                        currentStage, currentCondition, paths);
                throw new IllegalArgumentException(msg);
            }
        }
