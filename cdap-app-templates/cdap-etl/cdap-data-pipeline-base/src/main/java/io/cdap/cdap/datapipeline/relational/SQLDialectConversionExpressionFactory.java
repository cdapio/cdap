...
public class SQLDialectConversionExpressionFactory implements ExpressionFactory<String> {
    ...
    public SQLDialectConversionExpressionFactory(SQLDialect src,
        ...
        Capability srcCapability = getCapabilityFromDialect(src);
        Capability destCapability = getCapabilityFromDialect(dest);

        if (srcCapability != null) {
            capabilities.add(srcCapability);
        }
        if (destCapability != null) {
            capabilities.add(destCapability);
        }
    ...
    public Expression compile(String expression) {
        String compiledSql;
        try {
            converter.validate(expression, sourceDialect, schema);
    ...
    private Capability getCapabilityFromDialect(SQLDialect dialect) {
        switch (dialect) {
            case SPARKSQL:
                return StandardSQLCapabilities.SPARK;
            case BIGQUERYSQL:
                return StandardSQLCapabilities.BIGQUERY;
            default:
                return null;
        }
    }
