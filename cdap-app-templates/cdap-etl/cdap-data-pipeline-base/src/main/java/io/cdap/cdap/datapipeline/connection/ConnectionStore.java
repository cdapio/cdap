...
public class ConnectionStore {
  ...
  public Connection getConnection(ConnectionId connectionId) throws ConnectionNotFoundException {
    return TransactionRunners.run(transactionRunner, context -> {
      ...
      return getConnectionInternal(table, connectionId, true).get();
  ...
  public void saveConnection(ConnectionId connectionId, Connection connection) {
    TransactionRunners.run(transactionRunner, context -> {
      StructuredTable table = context.getTable(TABLE_ID);
      Optional<Connection> oldConnection  = getConnectionInternal(table, connectionId, false);
      ...
      if (oldConnection.isPresent()) {
        newConnection = new Connection(connection.getName(), connection.getConnectionType(),
                                       ...
                                       oldConnection.get().getCreatedTimeMillis(), connection.getPlugin());
      }
  ...
  // use this method to fetch connection outside a transaction
  private Optional<Connection> getConnectionInternal(
    ...
    if (!row.isPresent() && failIfNotFound) {
      throw new ConnectionNotFoundException(connectionId);
    }
    return row.map(structuredRow -> GSON.fromJson(structuredRow.getString(CONNECTION_DATA_FIELD), Connection.class));
  }
