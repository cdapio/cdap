...
/**
 * Enforces authorization for the current {@link SecurityRequestContext} {@link Principal} to perform an
 * {@link Permission} on an {@link EntityId}. @see AccessEnforcer for enforcer for any principal
 */
@Beta
public interface ContextAccessEnforcer {
   ...
   * @throws UnauthorizedException if the current user is not authorized to perform the specified permission on
   * the entity
   */
  ...
  /**
   * Enforces authorization for the current user for the specified {@link Permission permissions} on the
   * specified {@link EntityId}.
   *
   * @param entity the {@link EntityId} on which authorization is to be enforced
   * @param permissions the {@link Permission permissions} being performed
   * @throws UnauthorizedException if the current user is not authorized to perform the specified permissions on t
   * he entity
   */
  ...
  /**
   * Checks whether the set of {@link EntityId}s are visible to the current user.
   * An entity is visible to a user if the user has {@link StandardPermission#GET} on the entity.
