...
async function downloadSDK(res, fileStream, pathToZipFile) {
...
async function main() {
  ...
  const fileStream = fs.createWriteStream(pathToZipFile);
  try {
    await downloadSDK(res, fileStream, pathToZipFile);
  ...
  try {
    await start(path.join('/workspace', `cdap-sandbox-${cdapversion}`, 'bin'));
  } catch(e) {
    throw e;
  }
}
