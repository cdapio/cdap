...
describe('Plugin properties', () => {
  ...
  it('should be validated and errors found in properties', () => {
    ...
    cy.get(dataCy("plugin-properties-errors-found"))
      .should('have.text', '3 errors found.');
    cy.get(`${dataCy(REFERENCE_NAME_PROP_SELECTOR)} > ${dataCy(WIDGET_WRAPPER_SELECTOR)}`)
      .should('have.css', 'border-color', ERROR_BOUNDARY_COLOR);
    // Testing if the widget is highlighted when there is an error
    // and the error text
    cy.get(`${dataCy(REFERENCE_NAME_PROP_SELECTOR)} ~ ${dataCy("property-row-error")}`)
      .should(
  ...
  it('should be validated and no errors found in schema', () => {
    ...
    cy.get(`input${dataCy('project')}`).clear().type(sourceProperties.project);
    cy.get(`input${dataCy('datasetProject')}`)
      .clear()
      .type(sourceProperties.datasetProject);
    cy.get(`input${dataCy('dataset')}`).clear().type(sourceProperties.dataset);
    cy.get(`input${dataCy('table')}`).clear().type(sourceProperties.table);
    cy.get(`input${dataCy('serviceFilePath')}`)
      .clear()
      .type(sourceProperties.serviceFilePath);
    cy.get(`${dataCy(VALIDATE_BTN_SELECTOR)}`).click();
    cy.get(`${dataCy('plugin-validation-success-msg')}`)
  ...
  it('should be validated and errors found in schema', () => {
    ...
    // modal
    cy.get(`select${dataCy('schema-row-type-select-0')}`)
      .select('string:timestamp');
    cy.get(dataCy(VALIDATE_BTN_SELECTOR)).click();
    cy.get(dataCy('schema-row-0'))
      .should('have.css', 'border', `2px solid ${ERROR_BOUNDARY_COLOR}`)
    cy.get(dataCy('schema-row-error-0'))
      .should(
        'have.text',
  ...
  it('should be validated and nested errors should be highlighted', () => {
    ...
    cy.get(`${dataCy(required_property)} ${dataCy(0)} ${dataCy("multiple-values-input-0")}`).type('field1');
    cy.get(`${dataCy(required_property)} ${dataCy(0)} ${dataCy("multiple-values-input-1")}`).type('field2');
    cy.get(`${dataCy(required_property)} ${dataCy(0)} ${dataCy("multiple-values-input-2")}`).type('field3');
    ...
    cy.get(dataCy(VALIDATE_BTN_SELECTOR)).click();
    cy.get(`${dataCy(required_property)} ${dataCy("error-text-0")}`).should('have.text', "Map key 'field1' must be present in the input schema.");
    cy.get(`${dataCy(nested_property)} ${dataCy("error-text-0")}`).should('have.text', "Defaults key 'key1' must be present as a source in mapping.");
    cy.get(`${dataCy(nested_property)} ${dataCy('error-text-1')}`).should('have.text', "Defaults key 'key2' must be present as a source in mapping.");
  });
});
