...
export default function enableSystemApp({
  ...
  function enableService(observer) {
    ...
    MyArtifactApi.list({ namespace }).subscribe((res) => {
      ...
      api.getApp({ namespace }).subscribe(
        (res) => {
  ...
  function createApp(artifact, observer) {
    ...
    api.createApp({ namespace }, { artifact }).subscribe(
      () => {
        startService(observer);
      },
      (err) => {
  ...
  function startService(observer) {
    ...
    api.startService({ namespace }).subscribe(
      () => {
  ...
  function pollServiceStatus(observer) {
    ...
    let servicePoll = api.pollServiceStatus({ namespace }).subscribe(
      (res) => {
  ...
  function stopService(observer) {
    ...
    api.stopService({ namespace }).subscribe(
      () => {
  ...
  function checkForServiceStatusBeforeStart(observer) {
    api.getServiceStatus({ namespace: getCurrentNamespace() }).subscribe(
      (res) => {
