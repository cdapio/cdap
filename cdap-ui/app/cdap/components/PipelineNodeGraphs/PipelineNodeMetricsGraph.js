...
import NodeMetricsGraph, {
  isDataSeriesHaveSingleDatapoint,
} from 'components/PipelineNodeGraphs/NodeMetricsGraph';
...
export default class PipelineNodeMetricsGraph extends Component {
  ...
  getOutputRecordsForCharting() {
    ...
    if (!this.state.aggregate) {
      recordsOutData = this.formatData(
        recordsOutData,
        Array.isArray(recordsOutData) ? recordsOutData.length : 0
      );
      recordsErrorData = this.formatData(
        recordsErrorData,
    ...
    if (
      (typeof recordsOutData === 'number' && recordsOutData !== 0) ||
      (Array.isArray(recordsOutData) && recordsOutData.length)
    ) {
      return {
        ...outputRecordsObj,
        recordsOut: {
  ...
  getInputRecordsForCharting() {
    let { recordsInData, recordsErrorData } = cloneDeep(this.state);

    if (!this.state.aggregate) {
      recordsInData = this.formatData(
        recordsInData,
        Array.isArray(recordsInData) ? recordsInData.length : 0
      );
      recordsErrorData = this.formatData(
        recordsErrorData,
        Array.isArray(recordsErrorData) ? recordsErrorData.length : 0
  ...
  */
  renderProcesstimeTable = () => {
    ...
    Object.keys(this.state.processTimeMetrics).forEach((metric) => {
    ...
    const renderMicroSeconds = (time) => {
      ...
      if (time < 1000000) {
        return `${parseFloat(time / 1000, 10).toFixed(6)} ${T.translate(
  ...
  renderPortsRecordsCount() {
    if (Object.keys(this.state.totalRecordsOutPorts).length <= 2) {
      return Object.keys(this.state.totalRecordsOutPorts).map((key) => {
        return <span>{this.renderPortCount(key)}</span>;
      });
    }
