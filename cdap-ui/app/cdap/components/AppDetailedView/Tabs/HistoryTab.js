...
export default class HistoryTab extends Component {
  ...
  componentWillMount() {
    this.state.entity.programs.forEach((program) => {
      ...
      this.pollSubscriptions.push(
        MyProgramApi.pollRuns({ namespace, programId, programType, appId }).subscribe((res) => {
          let newRuns;
          if (this.state.history) {
            newRuns = res.filter((runRecord) => {
              return !this.state.history.filter(
                (existingRun) => existingRun.runid === runRecord.runid
              ).length;
            });
          } else {
            newRuns = res;
          ...
          history = history.map((runRecord) => {
            ...
            runRecord.end =
              runRecord.end !== runFromBackend.end ? runFromBackend.end : runRecord.end;
            return runRecord;
          });
          ...
          res = orderBy([...newRuns, ...history], ['start'], ['desc']);
  ...
  componentWillUnmount() {
    this.pollSubscriptions.forEach((subscription) => {
    ...
    const renderHistoryRows = () => {
      if (Array.isArray(this.state.history)) {
        if (this.state.history.length) {
          ...
          return (
            <div className="history-tab">
              <i>{T.translate('features.AppDetailedView.History.emptyMessage')}</i>
            </div>
          );
        }
      }
