...
export default class ConnectionExample extends Component {
  constructor(props) {
    ...
    let streams = this.dataSrc.poll(streamsReqObj).map((response) => {
    ...
    let datasets = this.dataSrc.poll(datasetReqObj).map((response) => {
  ...
  addStream() {
    ...
    this.dataSrc.request(reqObj).subscribe(
      () => {
  ...
  render() {
    let error;
    if (this.state.error) {
      error = <div className="text-danger">{this.state.error}</div>;
    }
    ...
    return (
      <div>
        ...
        <h3>
          {' '}
          Datasets & Streams <small>in Default Namespace</small>
        </h3>
