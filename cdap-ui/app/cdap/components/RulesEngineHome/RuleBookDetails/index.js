...
export default class RuleBookDetails extends Component {
  ...
  fetchRuleMetrics(rulebookDetails = { rules: [] }) {
    if (
      !Array.isArray(rulebookDetails.rules) ||
      (Array.isArray(rulebookDetails.rules) && !rulebookDetails.rules.length)
    ) {
      return;
    }
  ...
  generateAggregateStats = () => {
    ...
    if (
      ...
      this.state.rulebookDetails.rules.length
    ) {
      ...
      <div className="rule-book-details">
        <div className="rule-book-name-header">
          <div className="rule-book-name-version">
            <strong className="rule-book-name" title={rulebookDetails.id}>
              {rulebookDetails.id}
            </strong>
