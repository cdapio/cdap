...
/**
 * (CDAP-17283) - This function exists because in 6.1.4 version of CDAP
 * we introduced a new schema editor that didn't generate names that conforms
 * with AVRO schema specification.
 *
 * This function checks for types in the schema with names generated by UI
 * that doesn't conform to the avro schema spec.
 *
 * If true we regenerate the avro schema which will make sure the names are
 * spec complaint.
 *
 */
const doesRecordTypeNameConformToSpec = (flat: IFlattenRowType[]) => {
  if (!flat.length) {
    return false;
  }
  const regExp = new RegExp(/^[a-zA-Z0-9_]+$/);
  for (const row of flat) {
    if (row.name && !regExp.test(row.name)) {
      return true;
    }
  }
};
