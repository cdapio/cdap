...
const WidgetInfoInputView: React.FC<IWidgetInfoInputProps> = ({ classes, widgetID }) => {
  ...
  function onNameChange() {
    return (name) => {
      setWidgetInfo(fromJS(widgetInfo).setIn([widgetID, 'name'], name));
    };
  }

  function onLabelChange() {
    return (label) => {
      setWidgetInfo(fromJS(widgetInfo).setIn([widgetID, 'label'], label));
    };
  }

  function onWidgetCategoryChange() {
    return (widgetCategory) => {
      setWidgetInfo(fromJS(widgetInfo).setIn([widgetID, 'widgetCategory'], widgetCategory));
    };
  }

  function onWidgetTypeChange() {
    return (widgetType) => {
      setWidgetInfo(fromJS(widgetInfo).setIn([widgetID, 'widgetType'], widgetType));
