...
const AttributeMultipleValuesInputView = ({
  ...
  const [currentInput, setCurrentInput] = React.useState(getUpdatedCurrentInput(selectedType));
  ...
  const onKeyValueAttributeChange = (keyvalue, selectedType: SupportedType) => {
  ...
  const renderAttributeMultipleValuesInput = () => {
    if (selectedType === SupportedType.ValueLabelPair) {
      ...
      // TODO see if there is a dsv for number inputs
      return (
        <PluginInput
          widgetType={'dsv'}
          value={currentInput}
