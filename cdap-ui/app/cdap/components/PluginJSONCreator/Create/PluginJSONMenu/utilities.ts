...
function parsePluginJSON(filename, pluginJSON) {
  ...
  if (pluginJSON['configuration-groups']) {
    pluginJSON['configuration-groups'].forEach((group) => {
      ...
      const groupWidgets = group.properties;
      groupWidgets.forEach((widget) => {
        ...
        widgetInfo = widgetInfo.set(
          ...
          fromJS({
            ...
            label: widget.label,
            name: widget.name,
            ...(widget['widget-category'] && { widgetCategory: widget['widget-category'] }),
          })
        );

        if (widget['widget-attributes'] && Object.keys(widget['widget-attributes']).length > 0) {
          widgetToAttributes = widgetToAttributes.set(
            newWidgetID,
            fromJS(widget['widget-attributes'])
          );
        }
      });
    });
