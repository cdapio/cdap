angular.module(PKG.name + '.feature.flows')
  .controller('FlowsDetailRunController', function($scope, $state, MyDataSource, $timeout, EventPipe, $rootScope) {
    ...
    $scope.$watch('runTabs.activeTab', function(newVal, oldVal) {
      ...
      $timeout(function() {
        $state.go('flows.detail.runs.tabs.' + $scope.runTabs[toState]);
      });
    });
    ...
    $scope.$on('$stateChangeSuccess', function(event, toState) {
      var tab;
      if ($state.includes('flows.detail.runs.tabs.**')) {
