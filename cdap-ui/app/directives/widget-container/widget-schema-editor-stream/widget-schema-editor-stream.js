...
angular.module(PKG.name + '.commons')
  .directive('mySchemaEditorStream', function() {
    return {
      restrict: 'EA',
      scope: {
        model: '=ngModel',
      ...
      templateUrl: 'widget-container/widget-schema-editor-stream/widget-schema-editor-stream.html',
      controller: function($scope, myHelpers, EventPipe, IMPLICIT_SCHEMA, HydratorPlusPlusHydratorService, $timeout, myAlertOnValium) {
        ...
        function addFieldsNecessaryForStream() {
          var headersField = {
            name: 'headers',
          ...
          if (angular.isArray($scope.properties)) {
            if ($scope.properties[1].name === 'headers' && $scope.properties[1].type === 'map<string, string>') {
              $scope.properties[1].readonly = true;
            } else {
              $scope.properties.unshift(headersField);
            }

            if ($scope.properties[0].name === 'ts' && $scope.properties[0].type === 'long') {
              $scope.properties[0].readonly = true;
            } else {
              $scope.properties.unshift(tsField);
            }
          }
        }
